...
c 2020-03-25 jh: require phases Rg and Lg not to be only R and L
c 2020-07-20 jh: st to 9 chars, look for !nt
c 2020-10.24 jh: AZ to BAZ, fix problems with duplicate phases (didi no work well)
c                so now no check for duplicate phases
c 2020-11-20 lo: some changes related to weights and END phase
c 2021-03-17 lo: change to only check station code for S-P (before network and location were included)
c 2021-03-19 lo: fixed bug where res was not set for dtwt=0, not dtwt>0
c
c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      ndata=0    ! lo 19/11/2020
      do ii=1,nphase
c 
c 1/95: added section to get ipcount, the # of duplicate phases at the same stat
c       before oct 2020 there was only one back phase back check so often
c       it did no work, so this has been cut out          

c 2/95: changed ipcount=1 to ipcount=0
c
        ipcount=0  ! not clear if this works
...
c      write(55,*) ' debug lo done with trtime '
...
c    Use time difference derivatives for  L and S phases at this station
            ...
            do j=1,nphase
c              if(st(j).eq.st(i).and.i.ne.j)then
              if(st(j)(1:5).eq.st(i)(1:5).and.i.ne.j)then   ! lo 17 March 2021, ignore network and location
                prm2=ucase(phase(j)(1:1))
                if(phase(j)(1:2).eq.'Lg'.or.phase(j)(1:2).eq.'LG'
     *           .or.prm2.eq.'S')then
...
c  only set the raw data to the differences once
                  if(ips(j).eq.0)then
c         write(55,*) ' debug lo setting S-P ',st(j),phase(j),dtwt(j),
c     &  dtw1(j)
                   ips(j)=1
                   tp(j)=tp(j)-tp(i)
...
c         write(55,*) ' debug lo S-P already set
c     &       ',st(j),phase(j),dtwt(j),dtw1(j)
                  endif
...
c 10/94: exclude difference phases and azimuths
        if(ips(i).ne.1.and.phase(i)(1:3).ne.'BAZ'
     &    .and.dtwt(i).ge.0.0) then    ! lo change to ge 19/3/2021
...
c 10/94: change ge.0 to gt.0 also moved below dt(i) calculation
        if(dtw1(i).gt.0.0.and.phase(i)(1:3).ne.'BAZ'.and.
...
c       write(55,*) ' debug lo dtav,sum,wsum,ndata1 ',
c     &     dtav,sum,wsum,ndata1
c      endif
...
c  don't center PS, PL differences
        if(ips(i).eq.1.or.phase(i)(1:3).eq.'BAZ')dtav2=0.0

c don't use negative weighted phases
        if(dtw1(i).gt.0.0.and.phase(i)(1:3).ne.'BAZ'.and
     &   .ips(i).eq.0.) then
c and.phase(i)(1:2).ne.'AM')then    ! lo removed again 10/3/2021
          ww=dtw1(i)
...
c next
        if(dtw1(i).gt.0.0.and.phase(i)(1:3).eq.'BAZ') then
          ww=dtw1(i)
c lo work out weight for baz
c          ww=dtw1(i)*2*pi/360.    ! gives same as weight shown in hyp ouput
c changed back 30 March 2021 as dt(i) has rad units
        endif

c include everything (that has weight, including azimuths) in weighted rms 
        if(dtw1(i).gt.0.)then   ! lo 10/3/2021
          if (phase(i)(1:3).eq.'BAZ') then   ! use ww**2 if baz, 9/4/2021 lo
c                                            ! to get 1 sec if error=uncertainty
            sum=sum+ww**2*dt(i)**2     ! dtav2=0
          else
            sum=sum+ww*(dt(i)-dtav2)**2 
          endif
          if (phase(i)(1:3).eq.'BAZ') then
            wsum1=wsum1+dtw1(i)*2*pi/360.*test(52) ! lo 25/3/2021
          else
            wsum1=wsum1+ww  !change 9/98, BRL
          endif
          ndata=ndata+1
c          write(55,*) ' debug lo ww,dtwt,dtw1,sum,wsum1,ndata ',
c     &      st(i),phase(i),ww,dtwt(i),
c     &      dtw1(i),sum,wsum1,ndata
        endif

c         if(dtw1(i).gt.0.0) write(122,'(2a8,4f12.2)',iostat=code)st(i),
c     &           phase(i),dtw1(i),dt(i),dtav2,dt(i)-dtav2
        call sei code(fort$,code,iulst,b_flag) ! Process error.
      end do
...
c       write(*,*) ' debug lo ',
c     &   rmsv,test(36),iflagd,test(88),iweightflag
      endif
...
c        write(55,*) ' debug lo phase ',i,st(i),phase(i),dt(i),dtwt(i)
        dt(i)=dt(i)-dtav2    
        if (dtwt(i).lt.0.0) dt(i)=0.    
       end do 
...
c          write(iulst,*,iostat=code)st(i),phase(i),dt(i),dtav2
          call sei code(fort$,code,iulst,b_flag) ! Process error.
           if (phase(i)(1:3).ne.'BAZ'.and. ! lo
     &         phase(i)(1:2).ne.'AM') then  ! lo
             dt2(i)=abs(dt(i))                 
           else 
             dt2(i)=0 ! lo
           endif
...
c     &                   iostat=code)st(i),phase(i),dtw1(i)
            call sei code(fort$,code,iulst,b_flag) ! Process error.
c          removed 6/98, BRL
c          if(dtw1(i).gt.0.0.and.phase(i)(1:3).ne.'BAZ'.and.
c     &    ips(i).eq.0)then
...
c      write(*,*) ' debug lo trtime ',indexfp
...
c 7/94: changed weight from -1 to 0
      if(prm2.ne.prmd.and.prmd.ne.' '.and.minflag.ne.5)then
        dtw1(ii)=-1.0
c        write(*,*) ' debug lo dtw1 ',st(ii),phase(ii)
        return
      endif
...
c        write(55,*) ' debug lo dtwt -1 b '
     ...
     *    write(iulst,*,iostat=code)
     ...
     &    ' moho not defined'
          call sei code(fort$,code,iulst,b_flag) ! Process error.
          trphs(ii)='        '
c        write(55,*) ' debug lo dtwt -1 c ',nmoho
          dtw1(ii)=-1.0
          tmin=0.0
          return
         endif
...
c        write(6,*)prmd,xh(3),dmoho,dcorr
         call sei code(fort$,code,iulst,b_flag)
...
c changed 6/98, BRL <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     *     write(iulst,'('' depth limited to Conrad at '',
     ...
     &     f6.1,'' km'')')xh(3)
           ...
           trphs(ii)='        '
c        write(55,*) ' debug lo dtwt -1 cc ',nmoho
           dtw1(ii)=-1.0
           tmin=0.0
           return
          endif            
...
c           write(55,*) ' debug lo dtdx2 iflag ',iflag
...
c change 10/93 to only print message 1st time through
     ...
     *        ne.0.0.and.(.not.multi_model))then   !nt
              write(iulst,'('' No dtdx2 travel time for '',a9,  
...
c set dtw1=-1.0 for invalid phases
            trphs(ii)='        '
c        write(55,*) ' debug lo dtwt -1 dd ',st(ii),phase(ii)
c the next one removes n phases ????
c lo this was there in the past
            dtw1(ii)=-1.0
            tmin=0.0
         else
...
c lo 19/11/2020 looks like infprpt not used
...
c                write(iulst,*,iostat=code)data(1)(1:78)
              ...
              write(iulst,'('' No IASP91 travel time for '',a9, 
     &        1x,a4)',iostat=code)st(ii),phase(ii) !nt
              call sei code(fort$,code,iulst,b_flag) ! Process error.
            endif
...
c                write(*,*)data(1)(1:78)
     ...
     &        '' for '',a9,1x,a4)')st(ii),phase(ii) !nt
            endif
c            trphs(ii)='        '
            aninc(ii)=0
            dtw1(ii)=-1.0
c        write(55,*) ' debug lo dtwt -1 ccc ',nmoho
...
cjh      elseif(prm1.eq.'L'.or.prm1.eq.'R'.or.prm1.eq.'T')then
      elseif(phase(ii)(1:2).eq.'Lg'.or.phase(ii)(1:2).eq.'LG'
     *.or.phase(ii)(1:2).eq.'Rg'.or.phase(ii)(1:2).eq.'RG'
     *.or.prm1.eq.'T')then
...
c    layer having a  velocity vl
        ...
        if(phase(ii)(1:2).eq.'Lg'.or.phase(ii)(1:2).eq.'LG') then
         vl=test(51)
c jul 2003         trphs(ii)='LG      '
         trphs(ii)='Lg      '
         dtw1(ii)=dtw1(ii)*test(99)
        endif       
        if(phase(ii)(1:2).eq.'Rg'.or.phase(ii)(1:2).eq.'RG')then
         vl=test(44)
...
c    Azimuthal derivatives
      elseif(phase(ii)(1:3).eq.'BAZ')then  ! lo vapp known?
...
c also put dtwt(ii) (original weight) calculation in hypocent
        ...
        dtw1(ii)=dtwt(ii)*wt1
c        write(55,*) ' debug lo dtwt(ii) ',dtwt(ii)
...
c        write(*,*) ' debug lo ',ii,phase(ii),dtw1(ii)
        if(iulst.gt.0.and.dtw1(ii).ge.0.0.and.test(66).
     *    ne.0..and.(.not.multi_model))then
c lo 17/11/2020
c          if(data(1).ne.dataprev)then
c            write(iulst,*,iostat=code)data(1)(1:78)
c           call sei code(fort$,code,iulst,b_flag) ! Process error.
c          endif
          if (phase(ii)(1:2).ne.'AM'.and.
     &        phase(ii)(1:3).ne.'END') then ! lo 17/11/2020
            write(iulst,'('' No travel time for '',a9, 
     &    '' phase '',a8)',iostat=code)st(ii),phase(ii) !nt
            call sei code(fort$,code,iulst,b_flag) ! Process error.
          endif   ! lo 5/9/2012
        endif
        if(dtw1(ii).ge.0.0.and.test(66).ne.0.)then
...
c            write(*,*)data(1)(1:78)
          ...
          if(phase(ii)(1:4).ne.'    '.and.phase(ii)(1:2).ne.'AM'.and.
     &     phase(ii)(1:3).ne.'END')    ! lo 17/11/2020
     &    write(*,'('' No travel time for ''
     &    ,a9,'' phase '',a8)')st(ii),phase(ii) !nt
        endif
        
c set this to blank for unidentified phases
        trphs(ii)='        '                
c        write(55,*) ' debug lo dtwt -1 d ',nmoho
        dtw1(ii)=-1.0
        aninc(ii)=0.0
        tpc1(ii)=0.0
      endif
...
c      if(dtw1(ii).gt.0.)ndata=ndata+1

c   set up the row of the partial derivative matrix, g(ii,j)
      if (dtw1(ii).gt.0.) then    ! lo 19/11/2020 use phases with weight???
        ndata=ndata+1
        do k=1,3
          g(ii,k)=dx(k)
        end do        
        ...
        do k=1,3
c          g(ii,k)=0.
c following is needed, not sure why ??? lo 10/3/2021
          g(ii,k)=dx(k)
c          write(55,*) ' debug lo dtw1=0 g(i) ',k,dx(k)
        end do        
      endif
c      write(55,*) ' debug lo part der ',ii,phase(ii),dtw1(ii),ndata

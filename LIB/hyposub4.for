...
c dec 2018    jh   : change output which were in geocentric lat lon to geographic
c                    lat lon so correspondenceto  location in iterations. was not a
c                    for nearest station start location
c apr 2020    jh   : increase number of phases per station from 20 to 30.
c jul 2020    jh   : 9 char station, variable st2 looks funny, to be checked
c                    look for !nt
c oct 19 2020 jh   : fix start locations when BAZ phases are present
c oct 23 2020 jh   : AZ to BAZ
c nov 20 2020 lo   : fix bug with END phase, and some adjustment of use
c                    of dtwt
c feb 10 2021 jh   : less write out in print.out fopr multi_model
c mar  9 2021 lo   : check for Lg (in addition to LG) in multi phase check
c
...
c    phase identifier PN, PG, P , SN, SG, S, L or BAZ (was AZ),
...
c arrays using it.
      save
      include 'hypparm.inc'
      character*9 stat,stataz,stemp !nt
      character*9 statdp   ! nt
     ...
     &ind3(narriv,30),nmp1(narriv),indx(3),xhtr(3),
         ...
         ifound=0
          ...
          do while (statdp.eq.stemp.and.i2.ge.1)
           if(phase(i1)(2:4).eq.phase(i2)(1:3))then
             ...
             if(.not.multi_model)
     *        write(iulst,'(1x,a9,1x,a4,''-'',a4,f7.1,'' km'')')
     &        st(i),phase(i1),phase(i2),dppp  !nt
             if(dppp.gt.0.0)then
...
c
30    k=k+1   ! k is index, not counter
c
...
c
c        write(*,*) ' debug lo ',phase(k),dtwt(k)
        if(phase(k)(1:1).eq.'p')go to 30
        if(phase(k)(1:1).eq.'s')go to 30
        if(phase(k)(1:3).eq.'BAZ')go to 30
cxx        if(phase(k)(1:3).eq.'AMP') goto 30
...
c
c   indcate a multi phase
c
            ind1(i)=ind1(i)+1

            go to 30  ! check for another phase                            
...
c      write(55,*) ' debug lo nphs ',nphs
      do i=1,nphs
c        write(iulst,*)'phase,ind1,maxno: ',st1(i),ind1(i),maxno
c        write(55,*)'phase,ind1,maxno: ',st1(i),ind1(i),maxno
...
c        write(iulst,*)st1(i)
      ...
      do i=1,nphase
c           write(87,*)'first ',stmax(1:4),tp(i),st(i),phase(i),dtwt(i)

c------------------------------------------------------------
c change 10/93: exclude PS differences
c        if(dtwt(i).ne.0.0.and.ip(i).ne.9.and.ips(i).ne.1)then
        if(dtwt(i).gt.0.0.and.ip(i).ne.9.and.ips(i).ne.1)then ! lo
c------------------------------------------------------------
...
c
        ...
        if(.not.multi_model)write(iulst,'(a,f6.1,a,3f8.2)')
...
c  use closest station solution if test(56) <= 0 (changed so 0 disables 6/94)
      ...
      if(test(56).le.0.0) then
          ...
          if(.not.multi_model) write(iulst,'(a)') 
...
c      write(55,*) ' debug lo maxno ',maxno
      if(maxno.gt.1)then
...
c          if(dtwt(i).ne.0.0.and.ips(i).ne.1.and.ip(i).ne.9)then
          if(dtwt(i).gt.0.0.and.ips(i).ne.1.and.ip(i).ne.9)then   ! lo
            if(phase(i)(1:4).eq.stmax(1:4).and.maxind.gt.0)then
...
c 1/95: Find common phase having maximum delta/abs(tp(i)-tp(icpfirst))
        ...
        do  i=1,nphase 
c          if(dtwt(i).ne.0.0.and.ips(i).ne.1.and.ip(i).ne.9)then
          if(dtwt(i).gt.0.0.and.ips(i).ne.1.and.ip(i).ne.9)then   ! lo 20/11/2020
...
c        write(55,*) ' debug lo imaxx ',imaxx
...
c        write(55,*) ' debug lo first alona,alata ',alona,alata
...
c and try it as a start location
     ...
     &   scorr)
         ...
         if(rms.lt.0.0.and.ndata.gt.2)then
          ...
          call unfold1(xhtr(2),xhtr(1),xlat,xlon)   ! convert to geographical coordinates
          if(iulst.gt.0.and.(.not.multi_model))then
          write(iulst,'('' Starting location from midpoint of'',
     &     '' two most distant stations: '',5x,3f8.2)')xlat,
     &      xlon
...
c  
           call unfold1(alata*degtorad,alona*degtorad,xlat,xlon)   ! convert to geographical coordinates         
           if(iulst.gt.0.and.(.not.multi_model))then
            write(iulst,'('' Starting location from regression '',
     &      ''azimuth and slowness: '',5x,3f8.2)')xlat,xlon
...
c     write(6,*) nphase
      ...
      do  i=1,nphase
        if(phase(i)(1:3).eq.'BAZ'.and.dtwt(i).gt.0.0)naz=naz+1
      end do
...
cccccccccccccccccc  start of long loop counter ii ***********************
...
c
          ...
          if(phase(i)(1:3).eq.'BAZ'.and.dtwt(i).gt.0.0)then
            if(kk.eq.0)then
              nsolz=0
...
cccccccccc end of long loop counter ii cccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        if(nazsol.gt.0.and.k.eq.1)then
          ...
          call unfold1(alataz*degtorad,alonaz*degtorad,xlat,xlon)   ! convert to geographical coordinates
            if(iulst.gt.0.and.(.not.multi_model))then
              write(iulst,'('' Starting location from two'',
     &        '' azimuths:'',14x,2f8.2)')xlat,xlon
...
c  *****************  Find the multi-phase stations *******************
c
c  zero  counters
c
      do i=1,nphase
...
cccc   start of loop to find multi phase stations, counter kk ccccccccccccccccc

130   kk=kk+1          ! next original phase
      k=isort(kk)      ! next phase in time order

       if(kk.le.nphase) then
c
c  only phases P, PG etc will be used
c
          if(                         ! added small letter phases lo 30/11/2020
     *    phase(k)(1:2).ne.'P '.and.
     *    phase(k)(1:2).ne.'PG'.and.
     *    phase(k)(1:2).ne.'Pg'.and.
     *    phase(k)(1:2).ne.'PN'.and.
     *    phase(k)(1:2).ne.'Pn'.and.
     *    phase(k)(1:2).ne.'S '.and.
     *    phase(k)(1:2).ne.'SG'.and.
     *    phase(k)(1:2).ne.'Sg'.and.
     *    phase(k)(1:2).ne.'SN'.and.
     *    phase(k)(1:2).ne.'Sn'.and.
     *    phase(k)(1:2).ne.'Rg'.and. ! added 9/3/2021
     *    phase(k)(1:2).ne.'RG'.and. ! added 9/3/2021
     *    phase(k)(1:2).ne.'Lg'.and. ! added 9/3/2021
     *    phase(k)(1:2).ne.'LG') goto 130
...
c 4/94: changed phase limit to 20
            ...
            if(ind1(i).eq.30) then
              write(iulst,*)' Limit of 30 phases/station exceeded'
              go to 135
            endif  
...
ccccccccccccccccccccccccccccc    end of  loop starting at 130 ccccccccccccccccccccccccccc
...
c            if(phase(j)(1:3).eq.'BAZ'.and.dtwt(j).ne.0.0)then
            if(phase(j)(1:3).eq.'BAZ'.and.dtwt(j).gt.0.0)then  ! lo
              do i=1,nstat
...
c        write(55,*) ' debug lo iaz,kaz ',iaz,kaz
...
c  coordinates (x1,y1) have already been saved
     ...
     &      scorr)
            ...
            if(rms.lt.rmsmin)then
              ...
              call unfold1(alata*degtorad,alona*degtorad,xlat,xlon)   ! convert to geographical coordinates
                write(iulst,'('' Starting location from distance +'',
     &          '' another stat. azimuth: '',5x,2f8.2)')xlat,xlon
                 write(iulst,'('' rms = '',f8.2)')rms
              endif
            xhtr(1)=alonb*degtorad
            xhtr(2)=alatb*degtorad
...
c      write(*,*) ' debug lo start xhtr ',xhtr(1),xhtr(2)
c      alona=xhtr(1) ! lo added 28/22/2020
c      alata=xhtr(2) 
c has this part been working?
      do i=1,nphase

        if((ucase(phase(i)(2:2)).eq.'N'.or.phase(i)(2:2).eq.'K')
     &  .and.dtwt(i).gt.0.0.and.phase(i)(1:3).ne.'END')then   ! lo 20/11/20
c this was given problem when introducing Nordic2 with END phase
...
c          if(phase(i)(2:2).eq.'K')dcrit1=test(59) !removed 6/98 BRL
          ...
          if(naz.gt.0)then
c          write(*,*) ' debug lo naz ',naz
            do j=1,nphase

c change 10/93, BRL: exclude zero weight azimuths
c------------------------------------------------------------------
              if(phase(j)(1:3).eq.'BAZ'.and.st(j).eq.st(i).and.dtwt(j)
     &        .gt.0.0)then
c-----------------------------------------------------------------
c  find the distance, dekm1, to the present starting location from this
c  station
c            write(*,*) ' debug lo baz station ',
c     &    st(j),phase(j),alata,alona
c lo looks like event location not known alata,alona
       
                call delaz(alata*degtorad,alona*degtorad,dekm1,
     &          dedeg,azz,y0(i),x0(i))
c                call delaz(alata,alona,dekm1,
c     &          dedeg,azz,y0(i),x0(i))
c          write(*,*) ' debug lo dekm1.lt.dcrit1 ',dekm1,dcrit1
...
c                 write(iulst,*)'alata,alona ',alata,alona
     ...
     &                ,a9,'' phase '',a4,'' used along azimuth '',f6.1
     &                )')st(i),phase(i),az4   !nt
                      write(iulst,'('' rms = '',f8.2)')rms
                      go to 599
                    endif

...
c    nov 24 23 jh    : if blank station, crash, erro message if errro in
c                      station line
...
c-- Direction indicators                               
      ...
      if(station.eq.' ') return	  ! jh nov 2023		
...
c      write(6,*) station,string

      if(string.eq.' ') goto 1   ! jh nov 2023
      if(station(5:5).eq.' ') then  ! 4 char or less station 
         if(string(2:2).ne.' ') then   ! name starts in 2. column
            if(string(2:5) .ne. station(1:4)) go to 1 
         else                                                      
...
c-------- station found ---
c
      read(string,10,err=555) lat,dlat,ns,lon,dlon,ew,elev                    
10    format(6x,i2,f5.3,a1,i3,f5.3,a1,f4.0) 

      goto 556
555   continue
      write(6,*)'Error in station file'
      write(6,'(a)') string
      goto 999   ! skip
c      stop
556   continue
...
c
      ...
      if(station(k)(5:5).eq.' ') then  ! 4 char or less station 
         if(string(2:2).ne.' ') then   ! name starts in 2. column
            if(string(2:5) .eq. station(k)(1:4)) go to 2 
         else                                                      
            if(string(3:6) .eq. station(k)(1:4)) go to 2
         endif
      else                          ! 5 char station
         if(string(2:6) .eq. station(k)(1:5)) go to 2
      endif     

c         if(station(k)(5:5).eq.' ') then  ! 4 char station                                           
c            if(string(3:6) .eq. station(k)(1:4)) go to 2   ! station found
c         else                          ! 5 char station
c            if(string(2:6) .eq. station(k)(1:5)) go to 2   ! station found
c         endif

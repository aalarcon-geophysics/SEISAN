...
c                  variable ddd
c dec 3  2018 jh : add grid search after iterations to be able to get out of a 
c                  local minimum, look for grid
c                  change output format of iterations, show decimal deg instead of deg min
c                  also do depth grid search
c mar 29 2020 jh:  increase number of messages form 50 to 2000
c jul 20 2020 jh:  st to 9 chars, one change, look for !nt
c oct 24 2020 jh:  AZ to BAZ
c nov 19 2020 lo:  started to change check for duplicates, counting of
c                  stations and phases
c nov    2020 lo:  minor fixes
c feb 10 2021 jh:  less output in print.out when multi_modelc mar 29 2021 jh:  change how test96 works
...
c
     ...
     &         iulst                          ! Output diagnostics.
      ...
      real xxxh(3)                            ! used for grid search at end of iterations
...
c      do i=1,nphase
c        write(55,*) ' inside hl ',i,dtwt(i),dtw1(i)
c      enddo
...
c put -1 weight on END phase, disabled again
c       do i=1,nphase
c         if (phase(i)(1:3).eq.'END') then
c           write(55,*) ' debug lo END dtwt=-1 '
c         endif
c       enddo
c
c mark duplicates
c   
       do i=1,nphase
         dupph(i)=.false.   ! phases
         dupst(i)=.false.   ! stations
       enddo
       do i=1,nphase
         do j=i+1,nphase
           if (dtwt(i).gt.0..and.
     &       phase(i)(1:3).ne.'BAZ') then  ! lo
             if (st(i).eq.st(j).and.
     &           phase(i).eq.phase(j).and.
     &           .not.dupph(j)) then
c               write(*,*) ' duplicate phase: ',j,st(j),phase(j)
               dupph(j)=.true.
             endif
             if (st(i).eq.st(j).and.
     &           .not.dupst(j)) then
c               write(*,*) ' duplicate station: ',j,st(j),phase(j)
               dupst(j)=.true.
             endif
           endif
         enddo
...
c       statchk=' '    !nt
       ...
       do i=1,nphase
c lo 19/11/2020
c        if(st(i).ne.statchk.and.dtwt(i).gt.0.0)then
c         statchk=st(i)
c         write(*,*) ' debug lo ',i,dupst(i),dtwt(i)
         if(.not.dupst(i).and.dtwt(i).ge.0.0)then
           ndiffstat=ndiffstat+1
         endif
       enddo  
c       write(*,*) ' debug lo ndiffstat ',ndiffstat
...
c lo 17/11/2020         call sei code( fort$, code, iulst, b_flag ) ! Process outcome.

c change 10/94: added statchk test to make sure azimuths are from different stations


c         if(phase(i)(1:3).eq.'BAZ'.and.dtwt(i).gt.0.0.and.st(i).ne.
c     &    statchk)then

c           statchk=st(i)

c         write(*,*) ' debug lo ',phase(i),dtwt(i),ip(i),appar_veloc(i)
         if(phase(i)(1:3).eq.'BAZ'.and.dtwt(i).ge.0.0) then   ! lo 21/11/2020
           if(appar_veloc(i).gt.0.0) then    
             nappv=nappv+1
           else
c do not use if no vapp
             dtwt(i)=0.   ! lo 17/11/2020
c             ip(i)=4.   ! lo added ip, is the weight also needed?
           endif
           if (.not.dupph(i).and.dtwt(i).gt.0.0) then
             naz=naz+1       
           endif

         elseif (dtwt(i).ge.0.0.and..not.dupph(i))then
c lo 19/11/2020 add check to exclude non-phases
           if (phase(i)(1:3).ne.'END'.and.
     &         phase(i)(1:3).ne.'IAM'.and.
     &         phase(i)(1:2).ne.'AM'.and.
     &         phase(i)(1:2).ne.'E '.and.
     &         phase(i)(1:3).ne.'BAZ'.and.
     &         phase(i)(1:2).ne.'I ') then
             nvp=nvp+1
           endif
         endif
       end do
c       do i=1,nphase
c         write(*,*) ' debug lo ',phase(i),dtwt(i)
c       enddo

c       write(6,*)'lo debug nz,nv',naz,nvp
...
c lo this assumed first phase would be proper phase
c         do i=2,nphase

c           tst=.false.  ! initially not same station

c           do l=1,i-1

c             if(st(i).eq.st(l).and.dtwt(l).ne.0.0) tst=.true.   ! already counted

c           enddo

c           if(dtwt(i).gt.0.0.and..not.tst) k=k+1

cc           write(6,*) k,st(i),dtwt(i)

c           if(k.eq.3)go to 7000  !  3 stations now
...
c lo 19/11/2020
          k=0
          do i=1,nphase
c            if (.not.dupph(i).and.dtwt(i).gt.0.) k=k+1
            if (.not.dupst(i).and.dtwt(i).gt.0.) k=k+1
...
c jh oct 28 2012        ips(i)=0      ! correct lo 19/11/2020 ???
...
c lo checked that ips is ok  after it is set in rmsv
...
c 10/94; changed dtwt(i) to dtw1(i)
        ...
        if(phase(i)(1:3).ne.'BAZ'.and.ips(i).eq.0.and.ip(i).ne.9
     &  .and.dtw1(i).ge.0.0.and.dtwt(i).gt.0)then
          nzcount=nzcount+1
        endif
c        write(*,*) ' debug lo ',nzcount,dtw1(i),dtwt(i)
...
1000    format(/,3x,'First arrival: ',a9,1x,i4,1x,2i2,2x,2i3,f6.2,/) !nt
...
c           location became large
     ...
     &  depthlim,iflagd,ires,fixor,scorr,appar_veloc)
        ...
        call unfold1(xhh0(2),xhh0(1),xlat,xlon)   ! convert to geographical coordinates

        if(.not.multi_model) then
           write(iulst,*)
           write(iulst,'(a,2f7.2)') ' Start location used',xlat,xlon
        endif
     *  
...
c
c   next lines for final grid search, put in start location from grid search
c
        grid_test=0       ! set variable to control only grid search once            
        goto 1020         ! first time around, skip grid search

 1010   continue          ! from below after first iterated solution

        xh0(1)=x1grid     ! minimum rms location found from grid search. use as new
        xh0(2)=x2grid     ! start location
        xh0(3)=x3grid
        xhh0(3)=x3grid   ! since reset later
...
c
c  fixed starting depth and or epicenter, also starting depth/epicenter from
c  input file
c
      else
...
c  leave depth as it was
      ...
      endif          ! end of setting start location
...
c972       format('         (sec)  (dg mn)   (dg mn)  (km)         ',
972       format('         (sec)  (   dg)   (   dg)  (km)         ',
...
c 10/93 don't include azimuth and difference phases in wsum1
            ...
            if(dtw1(i).lt.0.0.or.ips(i).eq.1.or.phase(i)(1:3).eq.
...
c 10/93 don't center azimuths & differences (modified to exlude ires=2, 4/94)
              ...
              if(dtw1(i).gt.0.0.and.ips(i).eq.0.and.phase(i)(1:3).ne.

     &        'BAZ'.and.ires.ne.2)then
...
c
c  jh change format 12-2018
c

          if(iulst.gt.0.and.(.not.multi_model)) write(iulst,941
     &                      ,iostat=code)it,secs,(la1+ala1/60.0),ins1,

     &    (lo1+alo1/60.0),iew1,dp2,ndata,m,rmssv,xk2,(std(i),i=1,3)

c     &                      ,iostat=code)it,secs,la1,ala1,ins1,lo1,

c     &    alo1,iew1,dp2,ndata,m,rmssv,xk2,(std(i),i=1,3)
...
c941       format(i5,f9.2,i3,f5.2,a1,i4,f5.2,a1,f6.1,i4,i3,f7.2,f8.3,

941       format(i5,f9.2,f7.3,1x,a1,f8.3,1x,a1,f6.1,i4,i3,f7.2,f8.3,
...
c        do i=1,nphase
c          write(55,*) ' debug lo ',i,st(i),phase(i),dtw1(i),dt2(i)
c          write(55,*) ' debug lo ',g(i,1),g(i,2),g(i,3)
c        enddo
        call minv(m,nphase,np,dt2,di,dtw1)
c        write(*,*) ' debug lo continue ',nphase
c        write(55,*)' eigen ',(eig(i),i=1,3)
...
c        write(*,*) ' debug lo ',nphase,ndata2
...
c
c              write(55,*) ' debug lo dth,ss1 ',dth(j),ss1(j)
              ddd=dth(j)/ss1(j)
...
c          write(55,*) ' debug lo adjust ',j,ddd
...
c             dincr=10.c             dpmin=xh1(3)c             rmsdt=rms1c             rmsmin=rms1c             dpmin=xh1(3)c             xh1(3)=xh1(3)-dincrc             if(xh1(3).lt.0.0)xh1(3)=0.0cc             do while (xh1(3).lt.dpmin.and.rmsdt.le.rmsmin.and.xh1(3).cc     &       gt.0.0)c             do while (xh1(3).lt.dpmin.and.xh1(3).c
c     &       gt.0.0)c              rmsdt=rmsv(nphase,st,phase,tp,x0,y0,xh1,ndata,c     &        nmoho,nconrad,iulst,dtdl,dtdh,dddp,tt,phcd,test,c     &        dtwt,dtw1,tpc1,dl,iselv,maxelv,ips,ip,dt,xfar,xnear,c     &        tph,distind,aninc1,trphs,dtav,minflag,data,imatch,c     &        depthlim,iflagd,ires,fixor,scorr)cc              write(iulst,*)rmsmin,dpmin,rmsdt,xh1(3)c              if(rmsdt.le.rmsmin)thenc               rmsmin=rmsdtc               dpmin=xh1(3)c              endifc              xh1(3)=xh1(3)-dincrc              if(xh1(3).lt.0.0)xh1(3)=0.0c             enddoc             rms1=rmsminc             xh1(3)=dpminc c   new section mar 2021 jhc             dincr=1.0             xh1(3)=xh1(3)*3.0             xh1(3)=int(xh1(3))             if(xh1(3).gt.700.) xh1(3)=699.0             if(xh1(3).lt.15.0) xh1(3)=40.0             xh1(3)=xh1(3)+0.222             dincr=xh1(3)/40.0             dpmin=xh1(3)             rmsdt=rms1             rmsmin=rms1             rmsmin=99.0             dpmin=xh1(3)             xh1(3)=xh1(3)-dincr             if(xh1(3).lt.0.5)xh1(3)=0.5c             do while (xh1(3).lt.dpmin.and.rmsdt.le.rmsmin.and.xh1(3).c     &       gt.0.0)             do while (xh1(3).lt.dpmin.and.xh1(3).     &       gt.0.5)              rmsdt=rmsv(nphase,st,phase,tp,x0,y0,xh1,ndata,     &        nmoho,nconrad,iulst,dtdl,dtdh,dddp,tt,phcd,test,     &        dtwt,dtw1,tpc1,dl,iselv,maxelv,ips,ip,dt,xfar,xnear,     &        tph,distind,aninc1,trphs,dtav,minflag,data,imatch,     &        depthlim,iflagd,ires,fixor,scorr)c              write(6,*)rmsmin,dpmin,rmsdt,xh1(3)              if(rmsdt.le.rmsmin)then               rmsmin=rmsdt               dpmin=xh1(3)              endif              xh1(3)=xh1(3)-dincr              if(xh1(3).lt.0.0)xh1(3)=0.0             enddo             rms1=rmsmin             xh1(3)=dpminc       c recalculate everythingc
            go to 41
...
c
c   now do grid search if reset test is set and it is not done before
c   do not do grid search if epicenter is fixed or/else depth fixed
c   depending on what is required (ires parameter)

      krange=0        ! number of grid points for epicentral search
      idepth_range=1  ! number of grid points for depth search

      if((test(118).gt.0.0.or.test(120).gt.0.0)
     *.and.grid_test.eq.0               ! not done before
     *.and.ires.ne.7.and.ires.ne.9)     ! hypocenter fixed 
     *then

c         write(*,*) ' debug lo grid search '
         rms_grid=1000.0
c
c   calculate rms in a grid around epicenter if required, ires=8 is epicenter fixed
c   so skip
c

         if(test(118).gt.0.0.and.ires.ne.8) then
            km_step=test(118)    ! step in km horizonatally
            rad_step=km_step/(111.2*57.3)  ! step in radian

            xstep=rad_step/cos(xh1(2))      ! correct for latitude
            ystep=rad_step

            if(test(119).gt.0.0) then       ! check if set, else use default +/-10 steps
              krange=test(119)
            else
              krange=10
            endif
...
c
c   check if depth grid search, not if depth only fixed
c   fixed: ires=1: depth fixed
c

         if(test(120).gt.0.and.ires.ne.1) 
     *   then
           depth_step=test(120)
           if(test(121).gt.0.0) then
              idepth_range=test(121)              
           else
              idepth_range=10          ! default
           endif
         endif
c
c   grid loop
c      
         ...
         do jj=1,idepth_range
         do ii=-krange,krange
         do kk=-krange,krange
            
            if(krange.gt.0) then
               xxxh(1)=xh1(1)+ii*xstep
               xxxh(2)=xh1(2)+kk*ystep
            else
               xxxh(1)=xh1(1)+ii*xstep
               xxxh(2)=xh1(2)+kk*ystep
            endif

            if(idepth_range.gt.1) then
               xxxh(3)=(jj-1)*depth_step
            else
               xxxh(3)=xh1(3)              
            endif
c
c   calculate rms in grid point
c 
            rmsxxx1=rmsv(nphase,st,phase,tp,x0,y0,xxxh,ndata,nmoho,

     &      nconrad,iulst,dtdl,dtdh,dddp,tt,phcd,test,dtwt,dtw1,

     &      tpc1,dl,iselv,maxelv,ips,ip,dt1,xfar,xnear,tph1,distind,

     &      aninc1,trphs,dtav,minflag,data,imatch,depthlim,iflagd,
     &      ires,fixor,
     &      scorr)


            if(rmsxxx1.lt.rms_grid) then
               rms_grid=rmsxxx1
               x1grid=xxxh(1)
               x2grid=xxxh(2)
               x3grid=xxxh(3)
...
c 6/94 recalculate rms at minimum rms solution - residuals in dt1
     ...
     &scorr)
c
c   check if grid search found a local minimum, if so, that location
c   is used for a new start location for iterations. only do this once,
c   controlled by grid_test.  skip if
c   grid min rms is 0.000 indicating no rms calculated. 
c


      if((test(118).gt.0.0.or.test(120).gt.0.0)   ! one of the grids done
     *.and.grid_test.eq.0                         ! it has not been done before
     *.and.(krange.gt.0.or.idepth_range.gt.1)     ! grid search was done
     *.and.rms_grid.gt.0.0)
     *then

          if(rms_grid.lt.(rms1-0.002)) then       ! do not use too small minimum
             write(iulst,*)
             write(iulst,'(a,1x,2f6.3)') 
     *      'Grid rms smaller than solution rms', 
     *       rms_grid,rms1
             grid_test=1      ! no more grid search
c            
             delx=(x1grid-xh1(1))*57.3*111.2*cos(xh1(2))
             dely=(x2grid-xh1(2))*57.3*111.1
             delz=(x3grid-xh1(3))
             deldist=sqrt(delx*delx+dely*dely+delz*delz)
             write(iulst,'(a,f7.1)') 
     *      'Hypocentral dist. (km) from epicenter to grid point min.',
     *       deldist
             write(iulst,'(a,f7.2)') 'Depth change',delz

             if(deldist.gt.xfar-10.0.or.rms_grid.eq.0.0) then
                write(iulst,'(a)')
     *         'Beyond xfar or grid rms zero, skip new iteration'
             else
               call unfold1(x2grid,x1grid,xlat,xlon)   ! convert to geographical coordinates
               write(iulst,'(a,2f8.3,f8.1)')
     *         'Iterate again with new start location:',
     *          xlat,xlon,x3grid
               write(iulst,*)
               goto 1010  ! start iteration again
             endif
          else
             write(iulst,'(a)')
     *      'Grid search done, no local minimum found'
          endif
      endif
...
c       center residuals if ips(i)<>0. Changed 4/94 for fixed origin case
        ...
        if(ips(i).eq.0.and.phase(i)(1:3).ne.'BAZ'.and.ires.ne.9
...
c          write(66,*) ' debug lo ',st(i),phase(i),ips(i),dtwt(i)
...
c 10/94: changed to exclude azimuths
     ...
     &     'S   '.or.phase(i)(1:1).eq.'L'.and.phase(i)(1:3).ne.'BAZ'
...
c                  write(*,*)data(1)(1:78)
                ...
                write(iulst,'('' time for '',a9,'' phase '',a8,  
...
c         for this test
     ...
     &    .and.phase(1)(1:1).ne.'s'.and.phase(i)(1:3).ne.'BAZ')then
...
c               set its weight to 0 
     ...
     &              '' station '',a9,/,'' observed:'', f10.2,   

     &              '' calculated: '',f10.2,'' delta: '',f10.1)'

     &              ,iostat=code)

     &              st(j2),dabs(tp(j2)-tp(i)),tpc1(j2),dltmin2  !nt
...
c                   write(*,*)data(1)(1:78)
                    ...
                    write(iulst,'(a9,1x,a8,'' weighted out:''     

     &               ,'' inconsistent time'')')st(i),phase(i)   !nt
...
c                  set its weight to 0 
     ...
     &              st(j3),dabs(tp(j3)-tp(j1)),tpc1(j3),dltmin3 !nt
...
c cannot distinguish bad arrival: weight both to zero
     ...
     &            st(j1),dabs(tp(j1)-tp(i)),tpc1(j1),dltmin1 !nt
...
c                 write(*,*)data(1)(1:78)
     ...
     &               ,'' inconsistent time'')')st(j1),phase(j1) !nt

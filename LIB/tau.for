...
c   changes 
c
c  sep 98 by jh :   --------- sesian version 7.0 check, no change --------------
c
c  oct 28 98 bmt : linux changed, iargc vs iiargc, abort vs aabort
c                  because are reserved function
c  jul  27   jh  : change message
c  jul 21 03 jh  : diff to dif
c  feb 5 2013 jh : fix dimension
c  may 14 2013 jh: more dimension
c  nov  7 2013 lo: changed dimensions in trtm
c  2014 03 14 pv : cleanup warnings:
c                  replaced IF (Expression) Label1, Label2, Label3 statements, 
c                  the Arithmetic IF is considered harmful.
c  2019 02 19 jh : patch up an array bound error, not fixed, see 2-2019 jh
c  2020 03 03 jh : increased dimension in r4sort, needed for large data set
...
c changed BRL: added char(0)
      ia=ib(1:nb)//char(0)
      call assign(lu,mode,ia)
...
c
 ...
 23   open(lu,file=ia,status='new',form='unformatted')
      ...
      subroutine bkin(lu,nrec,length,buf)
c
c $$$$$ calls no other routines $$$$$
c
c   Bkin reads a block of length double precision words into array buf(len)
c   from record nrec of the direct access unformatted file connected to
c   logical unit lu.
c
      save
      double precision buf(length),tmp
c
      if(nrec.le.0) go to 1
      read(lu,rec=nrec)buf
c      write(10,'(6e12.4)')(buf(i),i=1,length)
      tmp=buf(1)
      return
...
c
 22   write(*,1000)
1000  format(' Enter desired branch control list at the prompts:',$)
      no=0
 21   fnam=char(0)
      call query(fnam,fnd)
...
c   the crustal branch and core phase branch identifiers.
 ...
 3    continue
 4    continue
c    print *,'j phcd phtmp =',j,' ',phcd(j),' ',phtmp
c
c   Make sure generic names are unique within a segment.
      if(kseg.lt.1) go to 7
...
c   Print the result.
 20   j2=j2-1
     ...
     1 'been selected for calculation-'
      fnd=.true.
...
c
c      write(10,*)'depcor:  nph nph0 nin',nph,nph0,nin
...
c   Find where the source slowness falls in the ray parameter array.
 ...
 3    continue
      ...
      call aabort('Source slowness too large.')
 4    k2=i
  50  continue
c 50   write(10,*)'k2 umin',k2,sngl(umin)
c
...
c   down to the turning point of the shallowest down-going ray.
 ...
 35   xus2(nph)=xus2(nph)+tx
  36  continue
c 36   write(10,*)'is ks tauus2 xus2',is,ks,sngl(tauus2(nph)),
c     1 sngl(xus2(nph))
...
c      write(10,*)'k m',k,m
      go to 24
  22  continue
c 22   write(10,*)'k m',k,m
      if(dabs(pt(k-1)-umin).le.dtol) k=k-1
      ki=ki+1
      kk(ki)=k
      pk(ki)=pt(k)
      pt(k)=umin
...
c     2 sngl(tauus2(2)),sngl(tau(1,k))
 ...
 25   jndx(j,2)=min0(jidx(j),k)
      ...
      go to 20
  37  continue
c 37   write(10,*)'j jndx jidx',j,jndx(j,1),jndx(j,2),jidx(j),' ',
c     1 phcd(j)
      do 30 l=1,2
...
c
 14   j=1
      ...
      nph=iabs(nafl(i,1))
c      write(10,*)'i iidx nph msrc nafl =',i,iidx(i),nph,msrc(nph),
c     &nafl(i,1)
...
c      write(10,*)'spfit:  j int =',j,int 
      ...
      j=j+1
c      write(10,*)'j jidx indx jndx =',j,jidx(j),indx(i,2),jndx(j,2)
...
c 200  format(/10x,i5/(1x,3i5,f12.6))
      usrc(1)=us(1)/pn
      usrc(2)=us(2)/pn
      return
      end
...
c
c  here is a bug, ij can be 0 so a brutal fix, 2-2019 jh
c
      kkk=ij
      if(ij.lt.1) kkk=1
      x=x0(kkk)
cold      x=x0(ij)
      ...
      in=index(phnm(n),'ab')
      if(in.le.0) go to 8
      if(ps.le.xbrn(jb,3)) phnm(n)(in:in+1)='bc'
      go to 8
 2    do 4 jj=1,2
      go to (5,6),jj
...
c jul 2003      phnm(n)=phcd(jb)(1:ln)//'diff'
...
c
      save
      include 'ttlim.inc'
      double precision tau(4,i2),x1,xn,a(2,100),ap(3),
     &b(100),alr,g1,gn,coef(5,i2)
c
      if(i2-i1.GT.0) goto 2
      if(i2-i1.LT.0) goto 13
 1    tau(2,i1)=x1
 13   return
 2    n=0
      do 3 i=i1,i2
...
c changed BRL
      if((phcd(jp)(1:2).eq.'Pg'.or.phcd(jp)(1:2).eq.'Pb'.or.
     &phcd(jp)(1:2).eq.'Pn'.or.phcd(jp)(1:1).eq.'P').and.
     &xbrn(jp,1).le.0d0) go to 3
...
c changed BRL
      if((phcd(js)(1:2).eq.'Sg'.or.phcd(js)(1:2).eq.'Sb'.or.
     &phcd(js)(1:2).eq.'Sn'.or.phcd(js)(1:1).eq.'S').and.
     &xbrn(js,1).le.0d0) go to 1
...
c changed BRL
 1    if(phnm(1:1).ne.'P'.and.phnm(1:1).ne.'p') go to 5
...
c
c changed BRL
 5    if(phnm(1:1).ne.'S'.and.phnm(1:1).ne.'s') go to 13
...
c     print *,'iupcor xcor tcor',iupcor,xcor,tcor
      ...
      subroutine pdecu(i1,i2,x0,x1,xmin,int,length)
      save
      include 'ttlim.inc'
...
c      length=i2
 ...
 10   continue
 19   length=k
c      write(10,300)(i,pt(i),tau(1,i),i=i1,length)
c 300  format(/(1x,i5,0pf12.6,1pd15.4))
      return
      end
...
c
c last character of string MUST be a blank (BRL)
...
c
      save
      dimension rkey(n),iptr(n),il(15),iu(15)
c
c   cjh   changed il andf iu form 10 to 15, needed for large data sets
c
...
c 301  format(/1x,4i3,2l3,2f12.8/(1x,i5,0p2f12.8,1p5d10.2))
      call fitspl(1,nn,tau(1,i1),xbrn(jb,1),xbrn(jb,2),tcoef(1,1,k))
      int0(k)=int
      go to 11
 10   continue
c      write(10,*)'xbrn ',jb,xbrn(jb,1),xbrn(jb,2)
       call fitspl(i1,i2,tau,xbrn(jb,1),xbrn(jb,2),coef)
c       write(10,*)'i1,i2,tau ',i1,i2,tau(1,i1),tau(2,i1)
 11   pmn=pt(i1)
      dmn=xbrn(jb,1)
      dmx=dmn
      mxcnt=0
...
c 300  format(/(1x,i5,4f12.6))
 ...
 4    continue
c      if(prnt(1)) write(10,100,iostat=ios)disc,j,pt(j),
c      1 (tau(k,j),k=1,4),(cn*xlim(k,j),k=1,2)
c 100  format(1x,a,i5,f10.6,1p4e10.2,0p2f7.2)
...
c     if(prnt(1)) write(10,100,iostat=ios)'   ',j,pt(j)
      ...
      i=mod(i,nbrn)+1
c changed BRL
      if(phcd(i)(1:1).eq.phcd(jb)(1:1).and.phcd(i)(2:2).ne.'P'
     &.and.(pe.ge.px(i,1).and.pe.le.px(i,2))) go to 9
 ...
 15   if(mxcnt.gt.mncnt.or.mncnt.gt.mxcnt+1)then
      fnam='Bad interpolation on '//phcd(jb)//char(0)
         call warn(fnam)
         write(*,*)mxcnt,mncnt
      endif
      return
      ...
      subroutine tabin(inunit,modnam)
      save
      include 'ttlim.inc'
      character*80 modnam
      character*80 fnam
c     logical log
      ...
      double precision pm,zm,us,pt,tau,xlim,xbrn,dbrn,zs,pk,pu,pux,tauu,
     1 xu,px,xt,taut,coef,tauc,xc,tcoef,tp,ccc(5)
c
      common/umdc/pm(jsrc,2),zm(jsrc,2),ndex(jsrc,2),mt(2)
      common/tabc/us(2),pt(jout),tau(4,jout),xlim(2,jout),xbrn(jbrn,3),
     1 dbrn(jbrn,2),xn,pn,tn,dn,hn,jndx(jbrn,2),idel(jbrn,3),mbr1,mbr2
...
c++
       nb=index(modnam,' ')-1
       fnam=modnam(1:nb)//'.HED'//char(0)
       call assign(nin,-1,fnam)

c split up this read to isolate a weird problem in MS Fortran 5.1
c (see below)
      read(nin) nasgr,nl,len2,xn,pn,tn,mt,nseg,nbrn,ku,km,fcs
      do i=1,jseg
        read(nin)(nafl(i,j),j=1,3)
        read(nin)(indx(i,j),j=1,2)
        read(nin)(kndx(i,j),j=1,2)
      enddo
      do i=1,nsr0
        read(nin)(pm(i,j),j=1,2)
        read(nin)(zm(i,j),j=1,2)
      end do
      do i=1,nsr0
        read(nin)(ndex(i,j),j=1,2)
      end do
      do i=1,nsl1
        read(nin)(pu(i,j),j=1,2)
      end do
      do i=1,jbrn
        read(nin)(pux(i,j),j=1,2)
      end do
      read(nin) phcd
      do i=1,jbrn
        read(nin)(px(i,j),j=1,2)
        read(nin)(xt(i,j),j=1,2)
        read(nin)(jndx(i,j),j=1,2)
      end do
      read(nin) pt,taut
      do i=1,jout

c this patch gets around a bug in MS Fortran 5.1, running under
c Windows, which causes an unformatted read of an array > 64Kb to
c set some of the values to zero 
       read(nin)(ccc(j),j=1,5)
       do j=1,5
         coef(j,i)=ccc(j)
       end do
      ...
      call retrns(nin)
c
      fnam=modnam(1:nb)//'.TBL'
      call dasign(nin,-1,fnam,nasgr)
c      call dasign(nin,-1,modnam(1:nb),nasgr)
c
      do 11 nph=1,2
...
c 200  format(/(1x,i3,i7,2f12.6,i7,2f12.6))
c      write(10,201)(i,(pu(i,j),j=1,2),i=1,ku(2)+1)
c 201  format(/(1x,i3,2f12.6))
...
c   Construct the contributions -dG;i-1(p)/dp | p;i-2, p;i-1, and p;i.
 ...
 4    continue
      ...
      include 'ttlim.inc'
      character*8 phnm(max)
      character*8 ctmp(max)
c      character*8 ctmp(100)
c      dimension tt(max),dtdd(max),dtdh(max),dddp(max),tmp(100,4),
c     1 iptr(100)
      dimension tt(max),dtdd(max),dtdh(max),dddp(max),tmp(max,4),
     1 iptr(max)
      double precision us,pt,tau,xlim,xbrn,dbrn
...
c   If we are in a quoted string skip the conversion.
      if(nfl.lt.0) go to 1
c   Do the conversion.
 2    if(ichar(ia(i:i)).ge.65.and.ichar(ia(i:i)).le.90)
     1 ia(i:i)=char(ichar(ia(i:i))+32)
 1    continue
      return
      end
...
c
     ...
     1 zm(js,nph))-1d0)/(pm(i,nph)-pm(js,nph))+1d0,1d-30))
      ...
      subroutine dasign(lu,mode,ia,length)
c
c $$$$$ calls no other routine $$$$$
c
...
c
 ...
 11   open(lu,file=ia,status='old',form='formatted',
     1 access='direct',recl=length)
      return
 12   open(lu,file=ia,status='new',form='formatted',
     1 access='direct',recl=length)
      return
 13   open(lu,file=ia,status='new',form='formatted',
     1 access='direct',recl=length)
      return
 14   open(lu,file=ia,status='old',form='formatted',
     1 access='direct',recl=length)
      return

c BRL 92: this line changed from nn=(len+3)/4 - caused direct record
c         overflow
 2    nn=length

      go to (21,22,23,24),ns
 21   open(lu,file=ia,status='unknown',form='unformatted',access=
     &'direct',recl=nn)
      return
 22   open(lu,file=ia,status='unknown',form='unformatted',access=
     &'direct',recl=nn)
      return
 23   open(lu,file=ia,status='unknown',form='unformatted',access=
     1 'direct',recl=nn)
      return
 24   open(lu,file=ia,status='unknown',form='unformatted',access=
     &'direct',recl=nn)
      return
      end
      subroutine ggetarg(n,ia)
c
...
c
      ...
      character*(*) ia
      do i=1,80
       if(ia(i:i).eq.char(0))go to 200
      end do
200   ib=i-1
      if(ib.ge.1)write(*,100)ia(1:ib)

c $ is no line feed on both SUN & PC
 100  format(1x,a,$)
      return
      end

      subroutine vexit(ierr)
      if(ierr.eq.1) call aabort('Terminated with errors.')
      end

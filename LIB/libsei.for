...
c 2019 01 16 jh    :   add new_nordic_format
c 2020 03 19 lo    :   read content from files given as @filename in SEISAN.DEF
c 2020-07-13 pv    :   added verbosity parameter in SEISAN.DEF
c 2020-10-05 jh    :   addeD routine find_waveform_files
c 2020-11-03 lo    :   check that @filename exists
c 2020-11-04 jh    :   NEW_NORDIC_FORMAT to NORDIC_FORMAT
c 2021-01-17 jh    :   add alertemail_sum
c 2021-01-28 jh    :   add mail_system,seisan_agency,email_sum_title,
c                      gmt_locality_file, epimap_locality_file,epimap_plot_locality,
c                      change routine SEI CMD SET so more than one argument is 
c                      possible when using SEI CMD SET (used for epimap)
c 2021-05-06 jh    :   make sure map_station is not a null char
c 2021-05-18 lo    :   fix to when @...def file not found
c 2021-12-27 jh    :   add alertemail_netdet, n_alertemail_netdet
c 2022-07-06 jh    :   add map_lat_div and map_lon_div for eev epimap
c 2023-03-12 jh    :   add alertemail_sum_netdet,n_alertemail_sum_netdet
c                      alertemail_format
c
CEND****************************************************************************
C
...
c      CHARACTER   CHR_TEXT *(TEXT_C) ! & text.    ! old jh, only one variable
c                                      so only one argument was possible
      CHARACTER*80   CHR_TEXT(10) ! & text.        ! new jh
...
C
     ...
     &         ' ',                  ! No prompt.
     &         CHR_TEXT(1),          ! Filename.   ! now index 1
...
C
      ...
      CHR_TEXT(1) = CHR_PROG(:SEICLEN(CHR_PROG)) // ! Output file name.
     &           '.inp'                          ! 
...
c   as for file name
      ...
      include 'seidim.inc'    ! for libsei
      include 'waveform.inc'  ! for libsei
...
c
c         call getarchinfo(arc_archive,j,arc_type
c filling wav variables as needed by which_archive
c         wav_stat(1)=stat
c         wav_comp(1)=comp
c         wav_network(1)=net
c         wav_location(1)=loc
c         comp3=comp(1:2)//comp(4:4)
         call which_archive(archive,
     &     stat,comp,net,loc,
     &     year,month,day,hour,min,sec)

         j=seiclen(archive)
         call getarchinfo(archive,j,arc_type
     *   ,stat,5,comp,3,
     *   net,2,loc,2,
...
c
     ...
     &         sei code                      ! Error processor.
      ...
      integer i,k,j,l
c-- unit for file
      integer def_unit,def_unit2
      character*120 deffile
c-- directory separator char
      character*1 dchar
      character*5  waveform_base             ! one wav base name
      character*180 data                     ! one line of data
      character*180 line(5000)               ! first file(s) read to line
...
c
     ...
     &  '-adobe-courier-bold-r-normal--12-120-75-75-m-70-iso8859-1'
      ...
      email_system=' '
      email_sum_title=' '
      email_summary_map=0.0
      
      seisan_agency=' '
      n_alertemail=0
      n_alertemail_netdet=0
      n_alertemail_sum=0
      n_alertemail_sum_netdet=0
      alertemail_format=0   ! 0 nordic, 2 is nordic 2
      epimap_locality_file=' '
      epimap_plot_locality=.false.
      map_stations=' '
      map_lat_div=1.0
      map_lon_div=1.0
      
      
      new_nordic_format=.false.
      new_nordic_format_only=.false.
...
c                                                                               
 ...
 10   continue
c
c lo here we can read whole file and then have loop over lines where we
c    set data to array
c    makes it easy to read extra files @.....
c
     ...
     &               b_eof )                   ! End of file?.
      if( .not.b_eof ) then                    ! Not end of file.
        ...
        line(nl)=data
c
c read lines from file if data(1) is '@'
c
        if (data(1:1).eq.'@') then
          deffile=data(2:seiclen(data))
c          write(*,*) ' debug lo reading ',deffile(1:seiclen(deffile))
    
            call sei get file( open$+ignore$,    ! Find and open without messages.
     &                   def_unit2,         ! On file unit.
     &                   code,             ! Condition (n/a).
     &                   'DAT ',            ! Alternative directory to search.
     &                   deffile)    ! For this file.
c          inquire(file=deffile,exist=exists)
c          if (.not.exists) then
c            write(*,*) ' stop, file not found ',
c     &        deffile(1:seiclen(deffile))
c            stop 
c          endif
          if(code.ne.e_ok$) then
            write(*,'(a)') ' file not found in SEISAN.DEF : @'//deffile 
            stop
          endif
creturn

20    continue
          read(def_unit2,'(a)',iostat=code) data    ! Read from file.
          call sei code( fort$,                    ! Process fortran i/o condition.
     &               code,                     ! Condition.
     &               def_unit2,                 ! On unit.
     &               b_eof )                   ! End of file?.
          ...
          call sei close( close$, def_unit2, code ) ! Close (Default stop on error).
        endif
        goto 10
      endif
...
c
c now parse the content of line()
c

c      write(*,*) ' debug lo parsing nlines ',nl
      do l=1,nl
         data=line(l)
c         write(*,*) ' lo ',l,data
...
c            wav_dirs(n_wav_dirs)(1:40)=data(41:80), lot nov 07
         ...
         elseif (data(1:15).eq.'MAP_LAT_LON_DIV'.and.
     *   data(41:60).ne.' '.and.data(51:60).ne.' ') then
            read(data(41:60),'(2f10.1)') map_lat_div,map_lon_div
         elseif (data(1:17).eq.'MAP_LON_BORDER') then
            read(data(41:50),'(f10.1)') map_lon
         elseif (data(1:17).eq.'EPIMAP_LAT_BORDER') then
            read(data(41:50),'(f10.1)') map_lat
         elseif (data(1:20).eq.'EPIMAP_LOCALITY_FILE') then
            read(data(41:120),'(a)') epimap_locality_file
         elseif (data(1:20).eq.'EPIMAP_PLOT_LOCALITY') then
            read(data(41:50),'(f10.1)') x 
            if(x.eq.1.0) epimap_plot_locality=.true.
...
c
c  make sure not a null char, jh may 6 2021
c
            if(ichar(map_stations(1:1)).eq.0) map_stations(1:1)=' '
     ...
     *      data(41:50).ne.'          ') then
         ...
         elseif(data(1:13).eq.'SEISAN AGENCY'.and.
     *      data(41:50).ne.'          ') then
            seisan_agency=data(41:43)
     ...
     *      data(41:50).ne.'          ') then
         ...
         elseif(data(1:18).eq.'ALERT_EMAIL_NETDET'.and.
     *      data(41:50).ne.'          ') then
            n_alertemail_netdet=n_alertemail_netdet+1
            alertemail_netdet(n_alertemail_netdet)=data(41:)
         elseif(data(1:12).eq.'EMAIL SYSTEM'.and.
     *      data(41:50).ne.'          ') then
            email_system=data(41:120)
         elseif(data(1:13).eq.'EMAIL SUMMARY'.and.
     *      data(41:50).ne.'          ') then
            n_alertemail_sum=n_alertemail_sum+1
            alertemail_sum(n_alertemail_sum)=data(41:)
         elseif(data(1:20).eq.'EMAIL_SUMMARY_NETDET'.and.
     *      data(41:50).ne.'          ') then
            n_alertemail_sum_netdet=n_alertemail_sum_netdet+1
            alertemail_sum_netdet(n_alertemail_sum_netdet)=data(41:)
         elseif(data(1:19).eq.'EMAIL_SUMMARY_TITLE'.and.
     *      data(41:50).ne.'          ') then
            email_sum_title=data(41:120)
         elseif(data(1:17).eq.'EMAIL_SUMMARY_MAP'.and.
     *      data(41:50).ne.'          ') then
            read(data(41:50),*) email_summary_map
         elseif(data(1:18).eq.'ALERT_EMAIL_FORMAT'.and.
     *      data(41:50).ne.'          ') then
            read(data(41:50),*) x
            alertemail_format=x
            if(alertemail_format.ne.0.and.alertemail_format.ne.2) 
     *      alertemail_format=0
         elseif(data(1:17).eq.'MAILX_COMMAND_MAP'.and.
     *      data(41:50).ne.'          ') then
            mailx_command_map=data(41:seiclen(data))
     ...
     *      data(41:50).ne.'          ') then
         ...
         elseif(data(1:17).eq.'GMT_LOCALITY_FILE'.and.
     *      data(41:50).ne.'          ') then
            gmt_locality_file=' '
            read(data(41:120),'(a)') gmt_locality_file
         elseif(data(1:13).eq.'HIGH_ACCURACY'.and.
     *      data(41:50).ne.'          ') then
            read(data(41:50),'(f10.1)') parameter
            if(parameter.gt.0.0) high_accuracy=.true.
         elseif(data(1:13).eq.'NORDIC_FORMAT'.and.
     *      data(41:50).ne.'          ') then
            read(data(41:50),'(f10.1)') parameter
            if(parameter.eq.1.0.or.parameter.eq.2.0)
     *      new_nordic_format=.true.
            if(parameter.eq.2.0) new_nordic_format_only=.true.
...
c     &         magnitude_order(n_magnitude_order)
     ...
     *      data(41:45).ne.' ') then
c           write(*,*) ' debug lo ',data
            if (n_cont_base_sel.le.0) then
              n_cont_base=n_cont_base+1
c              write(*,*) ' debug lo contbase ',n_cont_base
...
c            if(parameter.gt.0.0) cont_int_s=parameter
         ...
         elseif(data(1:9).eq.'VERBOSITY') then
            read(data(41:50),'(f10.0)') parameter
            verbosity=int(parameter)
            if (verbosity.ge.1) write(*,*) ' (debug) ',
     *        'verbosity=',verbosity
         elseif(data(1:14).eq.'SEISAN_LOGGING') then
            read(data(41:50),'(f10.0)') parameter
            seisan_logging=int(parameter)
            if (verbosity.ge.1) write(*,*) ' (debug) ',
     *        'loggin is on - seisan_logging=',seisan_logging
     ...
     *     .and.data(51:60).ne.'          ') then
         ...
         elseif(data(1:9).eq.'ARC_CHAN ') then
            iarc=iarc+1
...
c
         ...
         elseif(data(1:9).eq.'ARC_CHAN2') then
c            write(*,*) ' debug lo read ARC_CHAN2 line ',data
            iarc=iarc+1
            read(data(41:119),'(a5,a3,2a2,1x,f3.1,1x,i4,2i2,1x,i4,2i2,
     *                          1x,a9)')
     *      arc_stat(iarc),
     *      arc_comp(iarc),arc_net(iarc),arc_loc(iarc),
     *      parameter,  
     ...
     *      arc_label(iarc)
c            write(*,*) ' debug lo label ',arc_label(iarc)
...
c
            ...
            call TIMSEC (YEAR1,mon1,DAY1,hour1,1,0.0,arc_start(iarc))
            call TIMSEC (YEAR2,mon2,DAY2,hour2,1,0.0,arc_stop(iarc))
c            write(*,*) arc_start(iarc),arc_stop(iarc)
c
c check for virtual networks, up to 6
c
           do k=0,5
             vnet=' '
             read(data(86+k*6:90+k*6),'(a)') vnet
c
c find index
             if (vnet(1:1).eq.'_') then
               j=0
               do i=1,arc_vnet
               ...
               if (j.eq.0) then
                 ...
                 j=arc_vnet
c                 write(*,*) ' new vnet ',vnet
               endif
               arc_vnet_nchan(j)=arc_vnet_nchan(j)+1
               read(data(41:52),'(a5,a3,2a2)')
...
c               write(*,*) ' vnet stat chan ',vnet,arc_vnet_stat(j,
c     *         arc_vnet_nchan(j)),
c     *         arc_vnet_comp(j,arc_vnet_nchan(j))
             endif
           enddo
c
...
c
           do k=1,iarc-1
             if (
     &         arc_stat(iarc).eq.arc_stat(k).and.
     &         arc_comp(iarc).eq.arc_comp(k).and.
     &         arc_net(iarc).eq.arc_net(k).and.
     &         arc_loc(iarc).eq.arc_loc(k))
     &       then
               write(*,*) ' WARNING: duplicate ARC_CHAN2 '//
     &           arc_stat(iarc),' ',arc_comp(iarc),' ',
     &           arc_net(iarc),' ',arc_loc(iarc),' '
             endif
           enddo

         elseif(data(1:12).eq.'ARC_ARCHIVE '.and.
     *      data(41:50).ne.'          ') then
            read(data(41:120),'(a80)')arc_archive 
c new lo 20 March 2020
         elseif(data(1:12).eq.'ARC_ARCHIVE2'.and.
     *      data(51:60).ne.'          ') then
            arc_archive_n=arc_archive_n+1   ! counter for archives
            read(data(41:49),'(a9)') arc_archive_label(arc_archive_n)
            read(data(51:120),'(a80)') arc_archive_folder(arc_archive_n)
...
c find index
     ...
     *     arc_vnet_loc(j,arc_vnet_nchan(j))
         ...
         arc_nchan=iarc
      enddo    ! end of loop over all lines
c
c find archive folder for given labels from ARC_CHAN2 lines
c
      k=0
      do i=1,arc_nchan
        if (arc_label(i).ne.' ') then
          do j=1,arc_archive_n
            if (arc_label(i).eq.arc_archive_label(j)) then
              arc_folder(i)=arc_archive_folder(j)
c       write(*,*) ' debug lo ',arc_folder(i)
            endif
          enddo
        endif
      enddo
c
c
c     For SEISAN to work with new nordic format only, uncomment
c       the two following lines and recompile SEISAN:
c     new_nordic_format=.true.
c     new_nordic_format_only=.true.
c
c
      return
      end
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine find_waveform_files(nwav,file_in,file_out)
c
c   look for location of waveform files, used with se but could 
c   potentially be used elsewhere
c 
c  jh oct 2020
c
      implicit none
      include 'seidim.inc'
      include 'seisan.inc'
      integer nwav        ! number of files
      character*80 file_in(500)   ! file names
      character*200 file_out(500) ! file name with location
      character*200 text
      integer i,k,seiclen

      k=0   ! count files found
      ...
      do i=1,nwav
         file_out(i)=' '
         call  get_full_wav_name(file_in(i),file_out(i))
         if(file_out(i).eq.' ') then
            write(6,'(a,a)') 
     *      ' File does not exist: ',
     *      file_in(i)(:seiclen(file_in(i)))
          else          
            text=file_out(i)
            write(6,'(a,a)') 
     *      ' Full path name    :  ', 

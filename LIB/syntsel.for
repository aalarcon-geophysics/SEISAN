...
c     feb 20  2011 jh : to prevent errro mess. from reading delay, put in a small delay
c     jan 4   2012 jh : add argument to auto_tr
c     apr 02  2019 jh : new nordic format, fix bugs
...
c
c  make codes for real and synthetic channels, synthetics will be every 
...
cold         if (i.eq.1.or.stat(i).ne.stat(i-2))  j=j+1
         if(i.eq.1) j=j+1
         if(i.gt.2)then
           if(stat(i).ne.stat(i-2))  j=j+1
         endif
         jstat(i+1)=j
         k=k+1
      enddo
...
cold        if (ichan.gt.1.and.stat(ichan).eq.stat(ichan-2))   then 
        if (ichan.gt.2) then
           if(stat(ichan).eq.stat(ichan-2))   then
c
...
c
        ! fix new
           k=11
           if(new_nordic_format) k=17
           do i=2,ndata
              if(data(i)(2:6).eq.stat(ichan).
     &                   and.data(i)(k:k).eq.'P') then
                 read(data(i)(64:68),'(f5.1)') p_residual
                goto 300
              endif
           enddo
 350       write(6,*)'No P-reading, residual set ot 0.0 '
           p_residual=0.0
 300       continue
c
...
c
           call wav_sheads(chan(kchan),'synt ',text,mmmhead,chead)
c
c   read channel
c

...
c    nov 6             : epi location, color synt, delete A for automatic
...
c                        correction for ml for any filter, make it impossible
...
c    2018 09 21    jh. increase header variable dimension in traceplot to be
c                      able to plot long file names
c    2019 02       jh: new format
c    2019 06 09    jh: fix old comp codes plotting
c    2019 06 16    jh: BAZ problems in old version
c    2020 02 02    jh: automatic amplitude in multi trace mode, some comments
c    2020 02 07    jh: fix problem with missing '_' in IV phases
c    2020 04 26    jh: allow plot of phases with no component
c    2020 09 21    jh: change index for ph_auto, was wrong
c    2020 10 08    jh: amplitude was sometimes written on a phase line,
c                      com, ntlo in spectral output
c    2020 11 25    jh: amplify traces (magnification) also with mouse click, 
c                      fix plot of selected Y-phase, A for y-phases removed
c    2020 12 30    jh: bug with IV amplitude, also amplify
c                      with mouse if cursor below frame
c    2021 01 08    jh: bug manual amplitude reading in multitrace
c    2021 02  5    Jh: synthetics were accumulating
c    2021 02 24    jh: a very short channel together with a long in
c                      multi trace mode gave a problem with skip, fix amp selection
c                      for ground motion and no predefined phase
c    2021 03 01    jh: bug when using ground in multitrace with only one channel
c                      in multi trace disp_vel was remebered from privious plot, 
c                      so amplitudes woukd be names nm even if in count.
c                      Taper was done for spectrum and response removel. removed
c                      since also done in amp_spec and rem_resp routines. amplituded
c                      are now larger if picked near the beginning of the window.
c                      Foe WA extra filter a gain correction was done, however this
c                      has no or little effect since max amp then ussully is above
c                      low filter. If real max amplitude is below low filter, 
c                      the filter will still cuase the max ampltude to be picked
c                      above the filter so a too low amplitude is obtaind and the
c                      filter correction cnano correct for this.
c    2021 03 16    jh  update array data after updating variabels in rea, 
c                      nordic2 format
c    2021 04 06    jh  if a permanent filter was set, it was not cleared when
c                      doing ground motion so filter was added to the ground
c                      motion calculation. now cleared and put back.
c                      For exta wa filter, mss and mbb filters, they were
c                      done in both time and frequecy domian, now diabled in
c                      frequency domain so amplitude might increrase a  bit 
c                      and stability go down. however, tests showed
c                      insignificant difference.
c                      if using g and a filter, it is still used in both 
c                      and frequency domain, for some data it blows up
c                      without, to be investigated.
c   2021 04 14      jh commant out option u, ALCC
c   2021-06-08      pv fix bug with fhour and hour*3690
c   2021 09 16      jh: enable right mouse click to select end or beginning in zoom
c   2022 01 20      jh: if location is '--' in s-file ti does not have to match
c   2022 04 19      jh: plot A for automatic also for new format, ok in old
c   2022-05-02      pv: minor bug fix, station selection and right mouse buttom.
c   2022-07-10      jh: format for number of screens
c   2022-10-01      jh: for area selection (R), enter will cancel
c   2023-08-22      jh: write corrected amp for mb and ms on screen
c   2023-10-29      jh. if location code in s-file is '--' or blank
c                       it does no have to match.
c   2023-12-06      jh: if same syntheric phase was picked 2 times, the first 
c                       picked was not deleted
c    
c    
c
...
c           picc:        single trace mode               
c                                                                               
c    Some of the secondary routines are:                                                
c                                                                               
c           tecpic:     interactive picking and cursor, one trace               
c           plotw:      plot one trace including axis and frame, used for both
c                       single trace and multi trace mode                 
c           trans_filt: transfer and filter one trace before it is              
c                       used by plotw, alos includes instrument correction                                           
c           chasel:     selects channels and options using traceplot     
c           get_filter: get filter form keyboard    
c           mulplt_help: display all options and commands  
c           plot_cont:  plots continuous seismogram    
c           plot_cont_frame: plots the frame for ----- 
c           plot_cont_hardcopy:  send hard copy to printer in plot_cont
c           convert:    converts from pics to s-file lines, forth and back
c           pick_menue_box: displays the menu boxes
c           auto_amp_mulplt: pick amplitudes automatically and plot the amplitudes
c           mulplt_cont_picking: seting markers in cont mode for 
c           extract windows and for phase readings
c           get_dist: get epicentral and hypocentral distance for a given station
c
c                                                                               
...
c    (pframe etc).
c
c    Some paramter definitions, see also mulplt.inc:
c
c    trace-delay: in multi trace mode, the time from the left hand frame
c                 and the start of the trace
c    chan_delay:  delay of current channel with respect to main header
c    start:       number of samples from start of trace to start of
c                 selected window
c    stop:        number of samples from start of trace to end of 
c                 selected window 
c                                                                   
c                                                                               
c--------------------------------------------------------------                 
...
c
c   This routine has many more options, see seisan manual.
c                                                                               
...
c
c   clear filter if not set permanenet or default from mulplt
c
      if(.not.filter_perm.and.filt_single.eq.0) filt=0
      
      current_seq_chan=0    ! must be defined to get question about D, V, or A
c                                                                               
...
c                                                                               
      if(ptitle.eq.1) then                                                      
         call xchars(head,seiclen(head),xpos,ypos+height+20)                                
      endif                                                                     
c                                                                               
...
c 2021-06-08 pv bug in single trace plot on hour label
c        fhour=wav_hour(wav_current_chan(1))
         fhour=wav_hour(wav_current_chan(1))+fmin/60.0
      endif
c
...
c
        if(do_mb.or.do_wa.or.do_wahf.or.do_ms.or.
...
c
          first=old_zoom  ! make sure pics are made relative to zoom window,
...
c-- common block 
       ...
       include 'seiplot.inc'
       include 'seisan.inc'
       include 'rea.inc'      ! for new format
c                                                                               
c   local variables                                                             
c                                                                               
...
c-- local selected stop time                      
      real      local_stop
      integer k_index_y            ! index of Y phases picked      
c
...
c   old format has polarity and weight as part of phase new, new has
c   it in addtion            
c
           k=seiclen(phase(i))
           if(new_nordic_format) then
              if(phase(i)(1:1).ne.'Y') then
                text=' '
                if(ph_auto(i)(1:4).eq.'auto') text(1:1)='A'  !jh apr 22
                call tchars(phase(i)(1:k)//' '//ph_polarity(i)//
     *          ' '//ph_weight_in(i)
     *          //' '//text(1:1),k+6,ix-15,ypos+height-25.0)
              else
                call xset_color(color_syn)   ! synthetic has a different color
                call tchars(phase(i)(1:k)//' '//ph_polarity(i)//' '//
     *          ph_weight_in(i),k+4,ix-15,ypos+5.0)    
...
c  in this and follwing similar selctions, do not allow this
...
c-- W
       if(char(ich).eq.'H'.and.ypos.ne.150.0.and.xpos.lt.10.0) then
          write(text,203)
 203      format('Sel. window for Wood And. HF')
c-- response flag set
          call xchars(text,28,680+xpos,ypos-55-ntit*12)
                  ntit=ntit+1
          remove_response = 1
          do_wahf=.true.
       endif
...
c
       ...
       if(ich.eq.2.or.
     *  (ich.eq.6.and.iy.lt.449)) then
         ymag=ymag*2.
         choice='REPL' 
         goto 99
       endif
       if(ich.eq.4.or.
     * (ich.eq.32.and.iy.lt.449)) then
         ymag=ymag/2.
         choice='REPL' 
         goto 99
       endif
...
c----------------------------------------------------------------
c
c       if((char(ich).eq.'h'.or.char(ich).eq.'H').and.iy.gt.400.0) then ! lo
       if(char(ich).eq.'h'.and.iy.gt.400.0) then
          do_3comp=.true. 
c
c   indicate on plot
c
...
c
          if(k.gt.0) then
             ...
             ph_operator(k)(1:3)=operator(1:3)
             ph_agency(k)=pick_agency  
             ph_azimuth(k)=-999.0 ! azimuth   
             ph_angle_of_incidence(k) = -999.0  
             ph_residual(k)=-999.0
             ph_mag_residual(k)=-999.0
             ph_baz_residual=-999.0
             ph_distance(k)=-999.0
             ph_weight_out(k)=' '
                    
c                                                                               
c   check for  polarity reading indicated by y - cursor position
c   do not read if phase name occupy posiiton, in new nordic this is not a requirment
c 
             ph_polarity(k)=' '
             if(new_nordic_format) then
                if(iy.gt.ypos+height*0.85) ph_polarity(k)='C'
                if(iy.lt.ypos+height*0.15) ph_polarity(k)='D'                          
             else                  
...
c                                                                               
c   plot phase with  polarity                                         
c                                                                               
             call xmovabs(ix,ypos+40)                                                 
             call xdrwabs(ix,ypos+height-40)  
             if(new_nordic_format) then  
                j=seiclen(phase(k))                                      
                call tchars(phase(k)(1:j)//' '//ph_polarity(k),j+2,
     *          ix-15,ypos+height-25.0)
             else    
...
c   be a problem,
c   currently in iaspei amplitudes and long phase names read with ISP funtions. 
c   something to fix.   
c
c   for new format all 9 chars can be compared
c
c   remember, phase name includes i or e 
c
             old_pic=.false.
c
c   check if same phase as before, then overwrite
c
             k_index_y=k     ! index of picked Y-phase

             do k=1,nphas
                if(new_nordic_format) then
                   if(phase(k).eq.phase(k_index_y).and.k.ne.k_index_y)
     *                then   
                      old_pic=.true.    
                      old_index=k
                    endif
                   ...
                   if((phase(k)(1:5).eq.phase(k_index_y)(1:5).and.
     *             k_index_y.ne.k.and.
     ...
     *             (phase(k)(2:3).eq.'IA'.or.phase(k)(2:3).eq.'IV'))) 
     *             then    
                   old_pic=.true.    
                   old_index=k
c
...
c-- screen x-pos of old pick  
             picix=(pictim(old_index)-(first-1)/rate-chan_delay)*   
     *       xscale+xpos
c-- indicate deletion of pick    
...
c   alt_weight. not used in new format
c
                   if(new_nordic_format) then      ! new format has own variable
                      ph_weight_in(i)=i_weight(j)
                      goto 118
                   else
...
c
c   restore old response settings this channel
c
             do_wa=do_wa_old
             do_wahf=do_wahf_old
             do_mb=do_mb_old
...
c  
      ...
      if(char(ich).eq.'A'.and.amp.eq.0.0.and.per.eq.0.0) then

          if(pmax.eq.2) then   ! multi trace mode
c
c   for multi trace mode, only the last channel plotted is in memory.
c   to do auto amp for another channel, that channel must be read in again
c   and correpsonding channel parameters first,last, xscale, yscale,rate
c   and trace_delay copied from memory where they were saved when
c   respective channel was plotted. The channel must also be treated with the
c   same filter and response parameters before call to trans_filt. it
c   assuem that all channels have the same parameters

c
c   this is set to avoid asking for type of response in trans_filt
c   when channel is read in a second time when using A
c
             current_seq_chan=2
c
c  reset filter and response
c
             if(filt_old.ne.0) then   ! maybe no filter values to restore
                filt=filt_old
...
c
             ...
             ml_low=ml_low_old
             ml_high=ml_high_old                                                       
             mlhf_low=mlhf_low_old
             mlhf_high=mlhf_high_old                                                       
c
c   find channel, assume top of plot frame is at y=max_ypos, only multi trace mode
c
             ichan_pic=(max_ypos-iy)/height+1      ! sequential number plotted 
             current_chan=channelname(ichan_pic)   ! channel number in file

             first=save_first(current_chan)
             last=save_last(current_chan)
c
c   read channel
c
...
c
c   filter and correct for response
c
             call trans_filt(1)

             xscale=xscale_save(current_chan)
             yscale=yscale_save(current_chan)
c
c   ix is the point on the screen where amp is picked. in multi trace mode
c   it is counted from start of frame even if there is no sample in the 
c   beginning so it starts with point at trace_dealy.
c


             ix=trace_delay_save(current_chan)*xscale


             call auto_amp_mulplt(ix,amp,per)

             first=1
             remove_response=0   ! reset 
          else
c
c   single trace mode, plot always start with first sample or later
c   so ix to add is 0
c
...
c
c   plot just below trace
c
                call xchars(text,25,20.0,max_ypos-ichan_pic*height)
            endif
            amp_flag=.true.
         else
...
109    continue
c
c   check if Ml, Mb, MB, MS or Ms filter active to set 
c   amplitude phase. but not if reponse was not available
c

       if (amp_flag) then   ! there was an amplitude
          if(response_info_chan(current_chan)) then   ! it has response
             if (do_wa) then  ! check if pre defined
               on_amp(2)=1
               disp_vel=1   ! is reset to 0 at end of trace_plot
               goto 2222    ! so set again
             elseif (do_mb) then
               on_amp(3)=1
               disp_vel=1
               goto 2222
...
c
c  needed in case of option g
c
           remove_response=remove_response_old
           if(remove_response.eq.1) disp_vel=disp_vel_old

c
c   get amplitude phase from manual selection since not predefined
c
...
c          endif
 ...
 2222      continue
c
c check that only one amplitude selected
c
          flag=.false.
          do j=1,n_amp_text
...
c  seems AMP is selected by default ???? comment out now jan 2020

c         if (phs(new_phase).eq.' '.and.last_ix.ne.0) then
c           new_phase=99
c            phs(new_phase)=' AMP'
c          endif

          goto 5    ! below. save, phase has now been determined. 
       endif
...
c-- check if mouse click                          
       if(ich.eq.mouse.or.ich.eq.6) then				
c-- not on upper plot              
...
c
c   if right click, start at beginning
c
         if(ich.eq.6) ix=2
c                                                                               
...
c
c  left mouse to end of window
c  
         if(ich.eq.6) ix2=1023                                    
...
c
c   first find if this is a BAZ as indicated by a non zero veli and azim
c   if phase is I or E, only use BAZ, if any other P or S, use first 4 chars
c   of phase name. this is only done this way with new format.
c
c   phs is the predefined phase, and new_index is given the number in the list
c   of predefined phases. since BAZ is not there, this new phase is stored at
c   the end of the phase list in max_phase and then made to be the actual
c   phase. Thsiis also the case with amplitude phases.
c
c

       if(new_nordic_format) then
          if(veli.gt.0.0.and.azim.ne.0.0) then
             if(phs(new_phase)(2:2).eq.'P'.or.
     *          phs(new_phase)(2:2).eq.'S') then
                phs(max_phase)=' '//'BAZ'//'-'//phs(new_phase)(2:5)
             else
                phs(max_phase)=' BAZ'//'     '
             endif
             new_phase=max_phase
...
c
       do k=1,nphas
         if(new_nordic_format) then
            if(phase(k).eq.phs(new_phase))then   
               old_pic=.true.    
               old_index=k
            endif
...
c   phase do not blank out weight
c
               if(phase(k)(6:6).ne.'9'.and.phase(k)(2:3).ne.'IA'.and.
     *         phase(k)(2:3).ne.'IV') 
     *         phase(k)(6:6)=' '  ! keep weight 9
               if(phase(k)(7:7).eq.'A') phase(k)(7:7)=' '  ! not automatic now
            endif
...
c
c   save new reading in same array element as old if there so
c   phase is not given since the same as the old
c
       if(old_pic) then
          new_index=old_index                ! save in same
          ...
          nphas=nphas+1
          new_index=nphas
          phase(new_index)=phs(new_phase) ! this is a new phase
       endif    
...
c
c   do not associate next reading with BAZ
c
       azim=0.0
       veli=0.0      
c
...
c 
       ...
       pictim(new_index)=(first-1)/rate+(ix-xpos)/xscale+chan_delay    
c
c------------------------------------------------------------------
c   check for  polarity reading indicated by y - cursor position
c--------------------------------------------------------------------
...
c
     ...
     *and.phase(new_index)(2:3).ne.'IV') then
        if(new_nordic_format) then
           ph_polarity(new_index)=' '
           if(iy.gt.ypos+height*0.85) ph_polarity(new_index)='C '
           if(iy.lt.ypos+height*0.15) ph_polarity(new_index)='D '
       else
           phase(new_index)(8:9)='  '
...
c
        if(new_nordic_format) then
           if(phase(new_index)(2:3).eq.'AM') then 
              ph_polarity(new_index)=' '
           endif
        else
          if(phase(new_index)(2:3).eq.'AM') then 
...
c   for new format also network, location, operator, agency
c   distance and azimuth

      data_end(new_index)(5:14)='          '  
c
c   for new nordic format, also blank out calculated parameters
c   and others that are not known 
c
      if(new_nordic_format) then
        ph_azimuth(new_index)=-999.0    ! azimuth from event to station
        ph_angle_of_incidence(new_index) = -999.0  
        ph_residual(new_index)=-999.0
        ph_distance(new_index)=-999.0
        ph_mag_residual(new_index)=-999.0
        ph_baz_residual(new_index)=-999.0
c
c   keep input weight 9 if overwriting same pick
c

        if(ph_weight_in(new_index).ne.'9') ph_weight_in(new_index)=' '
        ph_weight_out(new_index)=' '
        ph_auto(new_index)=' '

 
c
c  for new nordic format, put in attributes for new pick
c
        ph_network(new_index)=wav_network(current_chan)
        ph_location(new_index)=wav_location(current_chan)
        ph_operator(new_index)(1:3)=operator(1:3)
        ph_agency(new_index)=pick_agency  
      endif                                    
c                                                                               
c  if any amplitude or period or back azimuth, indicate on plot                                 
c                                                                            
       if(amp.ne.0.0.or.per.ne.0.0.or.azim.ne.0.0.or.
     *    azimuth(new_index).ne.0.0.or.velocity(new_index).ne.0.0) then                        
          call xmovabs(ix,ypos+40)                                              
          call xdrwabs(ix+15,ypos-15)                                           
          call xmovabs(ix,ypos+40)                                              
          call xdrwabs(ix-15,ypos-15)                                           
...
c                                                                               
       ...
       call xdrwabs(ix,ypos+height-40)                                          
c                                                                               
c   phase and polarity                                                        
c 
       if(new_nordic_format) then  
          k=seiclen(phase(new_index))                                                                            
          call tchars(phase(new_index)(1:k)//' '//
     *    ph_polarity(new_index),k+2,ix-15,ypos+height-25)
       else
...
c
       if(amp.ne.0.0) then
c
c   if response not available for that channel, amplitude can only be counts
c
          text=' '
          write(text(1:12),'(f12.1)') amp
          write(text(24:31),'(f8.2)') per
          text(33:36)='secs'

          text(14:19)='counts'
          if(.not.response_info_chan(current_chan).or.disp_vel.eq.0)then
             text(14:19)='counts'
          else
...
c
          if(pmax.eq.1) then
             call xmessage(text,1,36,10.0,60.0)
          else
             call xmessage(text,1,36,xpos+10.0,ypos+height/2.0)
          endif
c                                                                               
c-- save last picked amplitude, warn user if not in nm or nm/s if mB or MS
c

          if(disp_vel.eq.0.0.or..not.response_info_chan(current_chan)) 
     *    then
      	      text(1:67)=
...
c   require an answer
c
             if(answer(1:1).ne.'y'.and.answer(1:1).ne.'Y'.and.
     *       answer(1:1).ne.'n'.and.answer(1:1).ne.'N') goto 7766
...
c
c   if mb or Ms, correct for gain. For explanation, 
c   see Havskov and Ottemøller 2010
c
...
c               write(6,*) 'amp',amplitude(new_index)
                amplitude(new_index)=amplitude(new_index)/gain
cxx
c                                                                               
c   write corrected max value                                                             
c  
...
c
                ...
                write(text,'(a,f12.1,a)')
     *          'Read amp corrected:',amplitude(new_index),' nm' 
                if(pmax.eq.1) then
                   call xmessage(text,1,80,320.0,60.0)
                else
                   call xmessage(text,1,80,xpos+320.0,ypos+height/2.0)
                endif                                                                            
 
             endif
c
c  if wa and extra filter, correct for that filter, not used
c  see commant 1-3-2021
c
c           write(6,*) pole_low_save,pole_high_save, 
c    *      f_low,f_high
c            if(do_wa.and.(
c     *      pole_low_save.gt.0.or.pole_high_save.gt.0)) then
c               gain=1.0
c               if(f_low.gt.0.0.and.pole_low_save.gt.0) then
c                  call bworth(1.0/period(new_index),
c     *            f_low,-pole_low_save,hs) 
c                  gain=gain*cabs(hs)
c               endif
c               if(f_high.gt.0.and.pole_high_save.gt.0) then
c                  call bworth(1.0/period(new_index),
c     *            f_high,pole_high_save,hs) 
c                  gain=gain*cabs(hs)
c               endif
c
c  reset ml filter
...
c                                                                               
  99   continue                                                                 
c                                                                               
c   save picks before returning, shift down to remove deleted picks                                                 
c                
       kphas=0     
       ...
       do 20 i=1,nphas  
          ...
          if(pictim(i).ne.-100000.0) then ! if not deleted indicated by -100000.0                                                                                       
             kphas=kphas+1                                                      
             phase(kphas)=phase(i)                                                
             pictim(kphas)=pictim(i)                                           
             amplitude(kphas)=amplitude(i)                                        
             period(kphas)=period(i)     
             azimuth(kphas)=azimuth(i)  ! back azimuth
...
c
             if(new_nordic_format) then
                ph_azimuth(kphas)=ph_azimuth(i) ! azimuth   
                ph_angle_of_incidence(kphas) = ph_angle_of_incidence(i)  
                ph_residual(kphas)=ph_residual(i)
                ph_mag_residual(kphas)=ph_mag_residual(i)
                ph_baz_residual(kphas)=ph_baz_residual(i)
                ph_distance(kphas)=ph_distance(i)
                ph_network(kphas)=ph_network(i)
                ph_location(kphas)=ph_location(i)
                ph_operator(kphas)=ph_operator(i)
                ph_agency(kphas) =ph_agency(i)  
                ph_polarity(kphas)=ph_polarity(i)  
                ph_weight_in(kphas)=ph_weight_in(i)
                ph_weight_out(kphas)=ph_weight_out(i)  
                ph_auto(kphas)=ph_auto(i) 
             endif                       
          endif                                                                 
 20    continue    
...
c
 999   continue
       amp=0.0   ! do not carry to next trace if not used
       per=0.0
       azim=0.0
       veli=0.0
...
c
c  there should be enough points, if not use skip 1
c
      if(last-3*skip.le.0) skip=1   ! jh feb 24 2021
c
      if(skip.gt.50) skip=50                                         
c-- assume plot x axis go from         
      xstep=(1023-xpos)*skip/float(npoint) 
c      write(*,*) ' debug lo rate ',rate,xscale,xstep
      dc=0.0
c                                                                               
c
...
c 2021-06-08 pv fix 3690 guess bug
c        if(delsec.ge.3600) ix=(ihour-fhour)*3690*xscale+xpos
         if(delsec.ge.3600) ix=(ihour-fhour)*3600*xscale+xpos
...
c-- filter type, hp or lo or bp used in recfil routine
      ...
      real perm_low,perm_high          ! permanet filter
      integer perm_low_pole,perm_high_pole,perm_filt !---|
c
...
c
c   next 2 apparently never used
c                                                             
      if(icomp.eq.2) then                                                                               
         do i=1,numb_samp                                                          
            y(i)=signal2(i)                                                         
...
c
c   set permanent filter, it might be overwritten by mss, mbb or wa filters
c
      if(filter_perm.and.filt.gt.0) then
         perm_low=flow(filt)
         perm_high=fhigh(filt)
         perm_low_pole=flow_pole(filt)
         perm_high_pole=fhigh_pole(filt)
         perm_filt=filt
      endif
...
c
c setup filter for high frequency ML
c
            if((mlhf_low.gt.0.or.mlhf_high.gt.0).and.do_wahf) then

c   check, poles not be the same
c
c                if (ml_low_pole.ne.ml_high_pole) then
c                   if(ml_low_pole.lt.ml_high_pole) then
c                      ml_high_pole=ml_low_pole
c                   else
c                      ml_low_pole=ml_high_pole
c                   endif
c                endif
                filt=9
                flow(9)=mlhf_low
                flow_pole(9)=mlhf_low_pole
                fhigh(9)=mlhf_high
                fhigh_pole(9)=mlhf_high_pole
             endif
...
c
        if(filt.ne.0) call applytaper(y,numb_samp,10.)   ! apply taper, width 10% of half the samples
c
c  save dc
c 
        dc_chan=dc
...
c   filter
c
c   if mb, ms any fixed filter should not be used since it 
c   will lower the amplitude, only paz is used. 
c   skip filter section
c
c   if ml, only the extra ml filter is allowed, else skip
c   filter section.
c
c   for mbb and mss, the hardweired mbb and mss filters
c   will override the current filter so no problem,
c   must pass filter section.
c
c   this change april 5, 2021, before that fixed filters
c   were used in reponse
c
c--------------------------
c
c 
      if((filt.ne.0.and..not.do_mb.and..not.do_ms.and..not.do_wa).or.
     *   (filt.ne.0.and.do_wa.and.
     *   (ml_low.gt.0.0.or.ml_high.gt.0.0))) then
 
c      write(6,*) 'transfilt',filt,
...
c     *      flow(filt),fhigh(filt),recfil_pole,npasses
     ...
     *         filt_name,flow(filt),fhigh(filt),1.0/rate,npasses)
         ...
         filt_name='BP      '  ! default again not sure why
c                                                                               
...
c
         dc_chan=dc
c
c   restore permanent filters
c
         if(filter_perm.and.perm_filt.gt.0) then
            flow(perm_filt)=perm_low
            fhigh(perm_filt)=perm_high
            flow_pole(perm_filt)=perm_low_pole
            fhigh_pole(perm_filt)=perm_high_pole
            filt=perm_filt
         endif
c
...
c
         ...
         if((.not.do_mb).and.(.not.do_wa).and.(.not.do_wahf).and.
     *       (.not.do_ms).and.
...
c
          ...
          call read_resp
c
c   save if response is available for a given channel. this is needed
c   if amplitudes are read in multi trace mode since all channels
c   use the same transformation, like wood anderson, but not all traces
c   might have the response information.
c
          response_info_chan(current_chan)=.true.
          if(wav_resp_status(1:1).eq.'9') then
              response_info_chan(current_chan)=.false.
...
c
c   set up filter response paz for Ms 
c
          if(do_ms) then
             remove_response=1
              disp_vel=1
...
c
c  extra filtering for ml  and fixed filters for mss and mbb is done in 
c  time domain    
c
c  before april 2021, filtring was also done in frquency domain
c  for ml, mss and mbb so signals were filtered twice. this has 
c  been disbled.
c  addtional filtering is now only done with option g so
c  filters are used twice. this should really have been disabled
c  so no filters at all in frequency domian but some g-options
c  seem to blow up without, to be investigated
c
c           write(6,*) 'filt in rem_resp',filt
           if(filt.gt.0.and.(.not.do_mb).
     *        and.(.not.do_ms).and..not.do_wa.and..not.do_mbb.
     *        and..not.do_mss) then
              f_low=flow(filt)
              f_high=fhigh(filt)
...
c  if a bp filter, but can be different for a lp and a hp. number of poles can be 
c  set interctively
c  
c
...
c
           ...
           if(do_wahf) write(text,'(a)')      'Wood And HF  '
...
c
        if(total_n_screen.gt.1) then
           text=' '
           write(text(1:10),'(a,i3,a,i3)')'W',n_screen,' of',
     *     total_n_screen           
c           do i=1,7
...
c
c   save in case amplitude is automatically picked
c
          trace_delay_save(ich)=trace_delay                   
c                                                                               
...
c
c   save for each channel, might be needed for automatic amplitude
c
          save_first(ich)=first
          save_last(ich)=last
c
...
c
          ...
          call plotw
c
c   save scaling factors for automatic amplitude
c
          xscale_save(ich)=xscale
          yscale_save(ich)=yscale
...
c  
     ...
     +       wav_comp(ich),wav_network(ich),wav_location(ich),-1)
c                                                                               
c   plot pics                                                                   
c                                                                               
             call tecpic                                                        
...
c 
       npasses=0
c
c   clear so next plot so next plot will no assume type of reponse
c   if next plot needs disp_vel different from 0 it will be reset
c
        disp_vel=0  
...
c
      ...
      ichan1=0  ! indicate no channel selection
c
...
c
     ...
     *and.iy.lt.max_ypos) valid_key=.true.
      ...
      if(char(ich).eq.'A') valid_key=.true.
c
...
c
      if(valid_key) then
c
c   if automatic amplitude, restore flags
c

         if(char(ich).eq.'a'.or.char(ich).eq.'A') then
            do_wa=save_wa
            do_wahf=save_wahf
            do_mb=save_mb
            do_ms=save_ms
...
c                                                                               
         ...
         ichan_pic=(max_ypos-iy)/height+1     ! sequential number     
c
...
c-----------------------------------------------------------------  
c increase or decrease plot magnification, use arrows
c or left and right mouse click if cursor outside frame
c-----------------------------------------------------------------

       if(ich.eq.2.or.
     * (ich.eq.6.and.((iy.gt.max_ypos.and.iy.lt.max_ypos+30.0).or.
     *  iy.lt.30.0))) then
         ymag=ymag*2.
         choice='REPL' 
         goto 99
       ...
       if(ich.eq.4.or.
     * (ich.eq.32.and.((iy.gt.max_ypos.and.iy.lt.max_ypos+30.0).or.
     *  iy.lt.30.0))) then
         ymag=ymag/2.
         choice='REPL' 
         goto 99
...
c
       if(char(ich).eq.'R') then
          prompt=
     *  ' Enter radius (deg) for area selection, enter to cancel'
     *    //char(0)
          call oneline(prompt,55,text,30,20.0,500.0)
c 
          if(text(1:5).ne.'     ') then
...
c   if right mouse indicate to use start of window providing
c   previous click was not channel select (indicated by ichan1=0).
c   if previous click was channel select, and current click is
c   right mouse, the whole section is skipped so many channels
c   are selected                             
c--------------------------------------------------------------------
c
c
...
c      if(ich.eq.mouse.or.(ich.eq.6.and.ichan1.eq.0)) then
       if(ich.eq.mouse.or.(ich.eq.6.and.ichan1.eq.0.AND.ix.gt.xpos))then
c         write(*,*) ' xxx mouse '
...
c--------------------------------------------------------------
c                                                                               
         if(iy.lt.max_ypos.and.ix.gt.xpos.and.iy.gt.30.0) then
c
c   if rigth click, start from beginning of plot
c
            if(ich.eq.6) ix1=xpos+1
c
c   plot first line
c
...
c
 3000       continue   ! get here if interval was too small
            call xscursr(ich,ix2,iy)
c
c   if not within frame, do it again 
c
           if(iy.lt.30.0.or.iy.gt.max_ypos.or.ix.lt.xpos) goto 3000 
c
c   if mouse right click, go to end of plot
c
           if(ich.eq.6) then
             ix2=1023
           endif 
...
c
 ...
 3001     continue          ! enter here if wrong filter limits
     ...
     *  ' Low and high cut for filter, enter for no filter'//char(0)
          call oneline(prompt,47,text,30,20.0,500.0)
c 
           if(text(2:5).eq.'    ') then                                        
              filt=0                                                           
           else                                                                
...
***************************************************************************     
      subroutine convert
     *(reference_time,station,compon_in,network,location,iswitch)                                                   
c                                                                               
c     Routine to convert between vectors with old/new picks                     
c     for plotting on the trace to/from S file. this is for station
c     station, component compon_in, network network and and location location                                
c                                                                               
c     Written by C. Lindholm, Oct. -89  
c                                        
c                                                                               
c     If iswitch =-1    : info in data records converted into vecors, data array 
c                         unchanged
c     If iswitch = 0    : info in data records are converted into vectors,
c                         data records are stripped of the extracted data       
c     If iswitch = 1    : Info in vectors are converted into data records     
c
c     NOTE: In new format, check for location and network can be disabled
c           if location is set to '-1'. This can be used if exsting S-file has
c           no location and network but the waveform data has. If input data
c           is from old format but new_nordic_format is set, there is no check
c           on location and network.  
c                                                                               
c       
      implicit none
      save
...
c-- real value of coda                                        
      real xcoda
c-- azimuth from station to event
      real azimu
c-- first p
      real first_p
c-- index of first p
...
c-- Counters                                             
      ...
      integer   code          ! local condition for write
c-- See  'indata'                     
      integer nnrecord,nr	
...
c                                                                               
cccccc   blank all local variables for this channel                                
c                                                                               
         nphas = 0                                                              
         ...
         do i = 1,max_phase                                                    
           amplitude(i) = 0.
           alt_weight(i)=' '    ! weight  not in phase
           period(i)    = 0.                                                    
           azimuth(i)   = 0.    ! back azimuth observed                                               
           velocity(i)  = 0.    ! apparent velocity observed                                                
           phase(i)     = '         '                                          
           data_end(i)  = '                       '
           coda=0.0
           ph_azimuth(i)=-999.0 ! azimuth   
           ph_angle_of_incidence(i) = -999.0  
           ph_residual(i)=-999.0
           ph_mag_residual(i)=-999.0
           ph_baz_residual(i)=-999.0
           ph_distance(i)=-999.0
           ph_network(i)=' '
           ph_location(i)=' '
           ph_operator(i)=' '
...
c
         ...
         if(new_nordic_format) then
c
c   loop for finding data for one channel
c

            do i=1,rea_nphase

c
c  find all readings for this station, to be used with identification
c  of type of spectrum, assume all channels start at the same time for
c  one station
c
              if(rea_stat(i).eq.station.and.rea_phase(i)(1:5)
     *        .ne.'SPEC'.and.rea_phase(i)(1:6).ne.'DELETE') then 
                jjj = jjj + 1    
                 call timsec(pyear,pmonth,pday,rea_hour(i),
     *           rea_min(i),rea_sec(i),pfsec)                                                        
                spec_tim(jjj) = pfsec - ffsec  ! ! ffsec is waveform start time
                spec_phs(jjj) = ' '//rea_phase(i) ! first char in old is I or E 
c
c   find hypocentral distance for spectrum
c
                 if(hyp_depth(1).gt.-10.and.rea_dist(i).gt.-10.0) then
               
                    sdistance=sqrt(hyp_depth(1)*hyp_depth(1)+
     *              rea_dist(i)*rea_dist(i))
                    edistance=rea_dist(i)
                    edepth=hyp_depth(1)
                 endif
              endif
c
c   get all observations from rea structure for this 
c   channel to internal variables, some are not plotted
c   but just read to be carried over to output. they must be
c   in the loop here since they could be deleted if phase is deleted
c

c   no component, location and network for synthetic phases

              if((rea_stat(i).eq.station.and.rea_onset(i).eq.'Y').or. 
c
c   new input format, network and location must match unless data is from 
c   old format converted, see below
c

     *        ((rea_stat(i).eq.station.and.
     *        rea_com(i).eq.compon(1:2)//compon(4:4).and. 
     *        rea_location(i).eq.location.and.
     *        rea_network(i).eq.network).or.    
c
c  new input format, network is ok but location code is '--',
c  or '  ' or '00'
c  and might not match wav file, so not taken into consideration
c

     *        (rea_stat(i).eq.station.and.
     *        rea_com(i).eq.compon(1:2)//compon(4:4).and. 
     *        (rea_location(i).eq.'--'.or.rea_location(i).eq.'00'.
     *        or.rea_location(i).eq.'  ').and.
     *        rea_network(i).eq.network).or.    
     ...
     *        (rea_stat(i).eq.station.and.
     *        rea_com(i).eq.compon(1:2)//compon(4:4).and. 
     *        rea_location(i).eq.'--'.and.
     *        rea_network(i).eq.network).or.    
c
c
c   new input format, network and location check has been disabled
c   if location and network are blank, could be because data has
c   been converted from old format data
c

     *        (rea_stat(i).eq.station.and.
     *        rea_com(i).eq.compon(1:2)//compon(4:4).and. 
     *        rea_location(i).eq.'  '.and.rea_network(i).eq.' ').or. 
 
c
c   new input format, network and location check has been disabled
c   if location and network are blank, could be because data has
c   been converted from old format data so only two chars
c   in component is checked if second is blank
c

     *        (rea_stat(i).eq.station.and.
     *        rea_com(i)(1:1).eq.compon(1:1).and.
     *        rea_com(i)(3:3).eq.compon(4:4).and.
     *        rea_com(i)(2:2).eq.' '.and. 
     *        rea_location(i).eq.'  '.and.rea_network(i).eq.' ').or. 
c
c   no component
c     
     *        (rea_stat(i).eq.station.and.rea_com(i).eq.' ').or.            
       
c
c   old input format,  there are only 2 chars component, no requirement
c   for network and location to match
c
     *        (rea_stat(i).eq.station.and.
     *        .not.rea_new_in.and.     
     *        rea_com(i)(1:1).eq.compon(1:1).and.
     *        rea_com(i)(3:3).eq.compon(4:4))).and.
c
c   do not plot a phase with name DELETE
c   no spectral phases here, treated later
c
     *        rea_phase(i)(1:6).ne.'DELETE'.and.   
     *        rea_phase(i)(1:4).ne.'SPEC')        
     *        then 

                 j=j+1
                 pictim(j) = pfsec - ffsec  ! ffsec is waveform start time
c  coda
                 if(rea_coda(i).gt.-999.0.and.rea_phase(i)(1:3).
     *           eq.'END')  coda=rea_coda(i)   ! assume only one coda reading pr channel

c  onset and polarity

                 phase(j) =  rea_onset(i)//rea_phase(i) ! onset part of phase internally  
                 ph_polarity(j)=rea_polarity(i) 

                 ph_auto(j)=rea_auto(i)
                                         
                 if(rea_amp(i).gt.-999.0) amplitude(j) = rea_amp(i)                                                   
                 if(rea_per(i).gt.-999.0) period(j) = rea_per(i)                                                      
                 if(rea_baz_obs(i).gt.-999.0) 
     *           azimuth(j) = rea_baz_obs(i)                                                      
                 if(rea_vel(i).gt.-999.0) velocity(j) = rea_vel(i)
                 if(rea_az(i).gt.-999.0) ph_azimuth(j)=rea_az(i) ! azimuth
                 if(rea_ain(i).gt.-999.0)  
     *            ph_angle_of_incidence(j)=rea_ain(i) 
                 ph_weight_in(j)=rea_weight_in(i)
                 ph_weight_out(j)=rea_weight_out(i)
c  residual
                 if(rea_res(i).gt.-999.0) ph_residual(j)=rea_res(i)
                 if(rea_mag_res(i).gt.-999.) ph_mag_residual(j)=       
     *           rea_mag_res(i)
                 if(rea_baz_res(i).gt.-999.0) ph_baz_residual(j)=
     *           rea_baz_res(i)

                 if(rea_dist(i).gt.-999.0) ph_distance(j)=rea_dist(i)
                 ph_network(j)=network
                 ph_location(j)=location
                 if(rea_operator(i).ne.' ')  
     *              ph_operator(j)=rea_operator(i)(1:3)
                 if(rea_agency(i).ne.' ') ph_agency(j)=rea_agency(i)   


                 if(iswitch.eq.0) rea_phase(i)='DELETE  '   ! remove in input since transferred to local variable
                                                            
             endif
           enddo

           nphas=j   ! phases found for this channel, not counting SPEC phases
           spec_nphas=jjj  ! number of phases for this station for spec
...
c                                                                               
cccccc   Read the data in s-file to find all readings for this station and component                                  
c                      
...
c
c  as of apr 2020, blank component is accepted
c
           if(outcomp.eq.comp.or.pphase(1:1).eq.'Y'.or.comp.eq.' ') 
     *     goto 778
...
c        if((stacode.eq.station) .and. (outcomp(2:2).eq.comp(2:2)))then ! why this ? jh 050216                  
         if((stacode.eq.station) .and. (outcomp(1:2).eq.comp(1:2).
     *   or.comp.eq.' '))then                   
           if(coda.ne.0)then                                                    
             j = j + 1                                                          
...
c-- end iswitch = 0                                   
c
      endif    ! endif for old format
      endif    
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc                                                                                
cccccc   Iswitch = 1  ;  Write into data records   ccccccccccccc                 
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
c
         ...
         do i=1,rea_nphase
           ...
           if(rea_onset(i).eq.'Y') then
              rea_phase(i)(1:6)='DELETE'
              k=k+1
           endif
         enddo
         rea_nphase=rea_nphase-k

         do i=1, nphas   ! all phases for one station, component, network and location
c                                                                               
...
c                                              
     ...
     +    phase(i)(1:1).eq.'E').and.phase(i)(1:1).ne.'Y')then   
c      +    phase(i)(1:1).eq.'E'))then                   
             pfsec = ffsec + pictim(i)          ! time after wav start to real time                                   
...
c
     ...
     *       hour=hour+24
             ...
             rea_sec(rea_nphase)=secc
             rea_phase(rea_nphase)=phase(i)(2:9)      
             if(rea_phase(rea_nphase)(7:7).eq.'D'.or. ! new fix, should not be possible
     *       rea_phase(rea_nphase)(7:7).eq.'C') 
     *       rea_phase(rea_nphase)(7:7)=' ' 
             rea_onset(rea_nphase)=phase(i)(1:1)
             rea_polarity(rea_nphase)=ph_polarity(i) 
c
c   amplitude and period
c
             if(amplitude(i).gt.0.0) then
               rea_amp(rea_nphase)=amplitude(i)
               rea_per(rea_nphase)=period(i)
             else
              rea_amp(rea_nphase)=-999.0
              rea_per(rea_nphase)=-999.0
             endif 
c
c   back azimuth
c

             if(azimuth(i).gt.0.0) then 
                 rea_baz_obs(rea_nphase)=azimuth(i)  
             else
                 rea_baz_obs(rea_nphase)=-999.0
             endif
             if(velocity(i).gt.0.0) then
                rea_vel(rea_nphase)=velocity(i)
             else
                rea_vel(rea_nphase)=-999.0
             endif                
c
c   weight
c

             rea_weight_in(rea_nphase)= ph_weight_in(i)                  
             rea_weight_out(rea_nphase)=ph_weight_out(i)  ! assume nothing has changed
             
             rea_stat(rea_nphase)=station
             rea_com(rea_nphase)=compon(1:2)//compon(4:4) 
             rea_az(rea_nphase)=ph_azimuth(i)

             rea_res(rea_nphase)=ph_residual(i)
             rea_mag_res(rea_nphase)=ph_mag_residual(i)
             rea_baz_res(rea_nphase)=ph_baz_residual(i)

             rea_dist(rea_nphase)=ph_distance(i)
             rea_ain(rea_nphase)=ph_angle_of_incidence(i)

             if(xcoda.gt.0.0.and.rea_phase(rea_nphase)(1:3).eq.'END') 
     *       rea_coda(rea_nphase)=xcoda

             rea_network(rea_nphase)=network
             rea_location(rea_nphase)= location 
 
             rea_auto(rea_nphase)=ph_auto(i)            
             if(ph_operator(i).ne.' ')
     *          rea_operator(rea_nphase)(1:3)=ph_operator(i) 
             if(ph_agency(i).ne.' ')
     *          rea_agency(rea_nphase)=ph_agency(i) 
           endif
        enddo 
...
c                                                                               
ccccc    Now write all new readings into end of data array, nphas is number
c        of phases for this station and component                               
c                                                                               
...
c
c   check size of amplitude and period                                               
c        
          if(period(i).ge.100.0) period(i)=99.9
...
c
c   put in end of record with distance  from previous run, comment
c   if not wanted
c
...
c
      data(nrecord) = '   '
c
      endif     ! end of old format writing phases				
c
c-------------------------------------------------------------
...
c---------------
      ...
      if(new_nordic_format.and.omega0.ne.0.0) then
c
c   check if spectrum there before for this channel, then delete,
c   it is assumed that all spectral phases are first in list. 
c   no check of network and location
c
         do i=1,rea_nphase            
            if(rea_stat(i).eq.station.and.
     *      rea_comp(i).eq.compon.and.          ! here 4 letter component
     *      spec_phase.eq.rea_spec_phase(i)) then
               rea_phase(i)(1:6)='DELETE  '             
            endif
         enddo
c
c   add one phase for new spectrum
c
         rea_nphase=rea_nphase+1
         rea_nspec=rea_nspec+1           ! fix new, needed?
c
...
c
         ...
         call sectim(pfsec,year,doy,month,day,hour,min,secc)     
c
c   transfer values to rea structure
c
         rea_year(rea_nphase)=year
         rea_month(rea_nphase)=month
         rea_day(rea_nphase)=day
         ...
         rea_sec(rea_nphase)=secc
         rea_stat(rea_nphase)=station
         rea_comp(rea_nphase)=compon    ! 4 letter component
         rea_com(rea_nphase)=compon(1:2)//compon(4:4)
         rea_network(rea_nphase)=network
         rea_location(rea_nphase)=location

         rea_phase(rea_nphase)(1:4)='SPEC'
         rea_onset(rea_nphase)=' '    
         rea_auto(rea_nphase)=' '       
         rea_spec_phase(rea_nphase)=spec_phase

         rea_moment(rea_nphase)=moment  
         rea_spec_mw(rea_nphase)=mw
         rea_omega0(rea_nphase)=omega0
         rea_cornerf(rea_nphase)=cornerf
         rea_radius(rea_nphase)=radius
         rea_sdrop(rea_nphase)=sdrop
         rea_slope(rea_nphase)=sslope
         rea_swin(rea_nphase)=swindow
         rea_vs(rea_nphase)=spec_velocity
         rea_vp(rea_nphase)=spec_velocity
         rea_kappa(rea_nphase)=kappa
         rea_q0(rea_nphase)=q0
         rea_qalpha(rea_nphase)=qalpha
         rea_q_below_1hz(rea_nphase)=q_below_1hz
         rea_density(rea_nphase)=density
         rea_geo_dist(rea_nphase)=geo_distance
c
...
c
      ...
      if(new_nordic_format) call rea_event_out(0,.true.,data,k) 
...
c---------------
      ...
      if(omega0.ne.0.0.and..not.new_nordic_format) then
c
...
c
      if(opmode.ne.0.and.data(1)(2:10).eq.'         ') then    ! fex new
...
c
c  output: ix:     pixel (position on plot) where amplitude is picked
c                  in multi trace mode ix in input is added to the  position
c          amp:    amplitude
c          per:    period
c
      ...
      integer ixstart                  ! to add to ix in multi trace mode
                                       ! yfirst and ylast
      integer i,n                      ! counters
      integer maxcros                  ! maximum number of zero crossings
c
c  set auto amp parameter
...
c
c     if ifirst is zero, no amplitude picked
c
      if(ifirst.eq.0) then
         amp=0.0
         return
...
c
       ix=(ifirst/rate)*xscale+xpos-1      ! calculated from first sample used
...
c
       ix=(ilast/rate)*xscale+xpos-1+ixstart
...
c
        ...
        ix=i       ! return position on plot of first value

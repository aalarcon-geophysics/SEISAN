...
c    feb 04 2019 jh: sd of slope and window not calcualted and written
c                    in avrage spectra, ml and mc could be written to data5
c                    even when no data causing array bound error
c    feb 14 2019 jh: 5 char station not output on screen when calcualting mw
c    mar 29 2020 jh: increase number fo messages from 50 to 2000
c    jul 21 2020 jh: 9 char stations, look for !nt mnb does no work with ntlo
c    oct 19 2020 jh: limit mn to -9.9 to 9.9
c    oct 21 2020 lo: rename MN to Mw
c    nov 11 2020 jh: correct writing of magnitudes to data5
c    dec 6  2020 jh: several E and H lines
c    mar 24 2021 jh: MB residual was not written, do not calculate magnitude
c                    if weight is 4
c    apr 12 2021 lo: extended ML with short distance terms, Luckett 2017
c    sep 21 2021 jh: added agency to hypocent call
c    jul 12 2022 jh: magnitudes not correctly written if written on additional
c                    lines, particualrly if fixed epicenter. Routine 
c                    insert_magnitude rewritten and used everywhere. 
c                    check for 0's in header and extra mag lines
c    aug 20 2023 jh: no more mc mag correction, field used for s-delay
c    nov 10 2023 jh: if magnitude infor on same line as a phase like coda
c                    and ml amplitude, then the magnitude will be calculated
c                    if weight is 4 on the arrival phase. only if the 
c                    phase is on a separate line like IAML or END will
c                    4 on that line prevent magnitude calcuation. 
c
C#################################################################
...
c      real test(*)
      ...
      character*9 station(nstats)   ! station list   !nt
      character*9 stat              ! one station    !nt
...
c
     ...
     *    ,err=1234)
     *    stat(1:5),icoda,amp,PERIOD,DIST  !nt   
          stat(6:9)=data5(nrec)(81:84)     !nt
          read(data(nrec)(19:28),'(i2,i2,f6.3)')
     &    hour,min,sec
c
c   check for weight, if 4 skip if not on a phase line
c
        if((data(nrec)(9:9).eq.'4'.or.data(nrec)(15:15).eq.'4').and.
     *  (data(nrec)(11:13).eq.'END'.or.data(nrec)(11:12).eq.'IA'.or. 
     *   data(nrec)(11:12).eq.'AM'.or.data(nrec)(11:12).eq.'IV'))
     *  goto 1 
...
c
     ...
     *    test(106).gt.0.0) then
             do i=nhead+1,nrecord-1
                if(stat(1:5).eq.data(i)(2:6).and.stat(6:9).
     *          eq.data5(i)(81:84).and.data(i)(71:75).ne.' ') 
     *          then  !nt
...
c
          ...
          enddo
c
c   station withe nt and lo not found, now look for station
c   without
c
          do i=1,nstats         ! nt
            if(station(i)(1:5).eq.stat(1:5)) then
               kstat=i
               goto 1245
            endif
          enddo         
...
c
     ...
     *             +hdist*test(9)
                ...
                if(mag_cor(1,kstat).ne.99.0) then
                   nmc=nmc+1
                   cmag(nmc)=cmag1
c                  cmag(nmc)=cmag1+mag_cor(1,kstat) ! jh aug 2023
...
c
                  if(nmc.gt.0) 
     *            write(data5(nrec)(62:68),'(a2,f5.2)') 'MC',cmag(nmc)
c
...
c
     ...
     *       dist.gt.0.and.dist.le.test(57).and.
     *       data(nrec)(15:16).ne.'HF') then
                magnitude=am*alog10(amp)+bm*alog10(hdist)+cm*hdist+dm
c
c add near source part
c
                if (em.ne.0..and.fm.ne.0.) then
                  magnitude=magnitude+em*(exp(-fm*hdist))
c                  write(*,*) ' Info: ML using short distance term '
                endif
...
c
                ...
                if(nml.gt.0) 
     *          write(data5(nrec)(62:68),'(a2,f5.2)') 'ML',xmag(nml)
...
c compute group velocity vg
             vg=hdist/(psec-hsec)
             if (data(nrec)(11:16).eq.'IAMLHF') then
               hf=1.
             else
               hf=0.
             endif
             magnitude=compute_mbn(amp,period,lat,lon,hdist,
     &         vg,stat(1:5),mbns,mbne,hf)  !nt
             if(magnitude.gt.9.9) magnitude=-9.9     ! jh oct 2020
             if(magnitude.lt.-9.9) magnitude=-9.9    ! jh
             if (magnitude.gt.-9.) then
               nmn=nmn+1
               nmag(nmn)=magnitude
               av_nmag=av_nmag+magnitude
               write(data5(nrec)(62:68),'(a2,f5.2)') 'MN',nmag(nmn)
             endif
             if(nline.ge.0.and.output.and.magnitude.gt.-9.) then
                         write
     *                   (6,'(1x,a7,2x,a,2x,f8.1,3x,a,2x,f12.1,
     *                   a,f5.1,3x,a,f5.2)')
     *                   data(nrec)(2:8),' hdist:',hdist,
     *                   'amp:',amp,' T: ',period,'Mw = ',magnitude
                   nline=nline+1
             endif
             if(nline.gt.maxline.and.output) then
                  nline=0
...
c
     ...
     *          1.66*alog10(dist/111.2)+3.3
                if(mag_cor(4,kstat).ne.99.0) then
                   ...
                   write(data5(nrec)(62:68),'(a2,f5.2)') 'Ms',smag(nms)
                endif
c
...
c         
     ...
     *          1.66*alog10(dist/111.2)+3.3
                if(mag_cor(4,kstat).ne.99.0) then
                   ...
                   write(data5(nrec)(62:68),'(a2,f5.2)') 
     *             'MS',ssmag(nmss)
                endif
c
c   write distance, amplitude and magnitude 
...
c           
                if(test(108).ne.1.0.and.test(108).ne.0.0) then
                 ...
                 if(mag_cor(3,kstat).ne.99.0) then
                    ...
                    write(data5(nrec)(62:68),'(a2,f5.2)') 'mb',bmag(nmb)
                 endif
c
...
c         write(*,*) ' debug av_nmag ',av_nmag
         call insert_magnitude(data,av_nmag,'w',agency,nhead,nrecord)
c         write(data(1)(56:63),'(f4.1,a1,a3)')av_nmag,'w',agency
c
c add residual into data5
c     
         do k=nhead+1,nrecord
           if (data5(k)(62:63).eq.'Mw') then
             read(data5(k)(64:68),'(f5.2)') magnitude
...
c
c-------------------------------------------------------------------
c   check if mc magnitude is available, write in first availablr position
...
c     
         ...
         enddo
         call insert_magnitude(data,av_smag,'s',agency,nhead,nrecord)
...
c     
         ...
         enddo
         call insert_magnitude(data,av_ssmag,'S',
     *   agency,nhead,nrecord)
       endif
...
c     
          ...
          enddo
          call insert_magnitude(data,av_bmag,'b',agency,nhead,nrecord)
...
c     
          do k=nhead+1,nrecord
             if (data5(k)(62:63).eq.'MB'.or.data5(k)(62:63).eq.'mB')
     *       then
               read(data5(k)(64:68),'(f5.2)') magnitude
          ...
          enddo
          call insert_magnitude(data,av_bbmag,'B',agency,nhead,nrecord)
       endif
...
c   and old H lines
c   now, dec 2020, do not clear E and H since there can be many and
c   rea should deal with it. jh
c
      implicit none
      include 'seidim.inc'
      include 'rea.inc'
...
c--- spectral values, see mulplt.inc
     ...
     *     sslope(max_data),q_below1hz,
     *     radius(max_data),swindow(max_data),
     ...
     *     q0,qalpha
      ...
      real avcornerf
      real sdmw,sdmoment,sdsdrop,sdradius,sdomega0,
     *     sdcornerf,sdswindow,sdsslope
...
c
      ...
      integer i,k
      character*9 stat(nstats)      ! station list   !nt
...
c     *     station(1:4),compon,moment(nspec1),sdrop(nspec1),
     ...
     *     radius(nspec1),sslope(nspec1),swindow(nspec1),mw(nspec1)
...
c
          do i=1,nstats
            if(stat(i)(1:5).eq.station) then  !nt
               kstat=i
               goto 1245
            endif
          enddo
             ...
             if(nline.ge.0.and.output) then
                if(mag_cor(5,kstat).ne.99.0) then
                   if(mag_cor(5,kstat).eq.0.0) then
                      write
     *                (6,'(1x,a5,2a1,2x,a,2x,f8.1,
     *                3x,a,2x,f12.1,3x,a,f5.1)')
     *                station,data1(k)(11:11),
     *                data1(k)(14:14),' gdist:',geo_distance,
     *                'mom:',moment(nspec1),'mw = ',mw(nspec1)
                   else        ! magnitude correction
                      write
     *                (6,'(1x,a5,2a1,2x,a,2x,f8.1,3x,a,2x,f12.1,
     *                3x,a,f5.1,a,f5.2)')station,
     *                 data1(k)(11:11),
     *                 data1(k)(14:14),' gdist:',geo_distance,
     *                'mom:',moment(nspec1),'mw = ',mw(nspec1),' + ',
     *                mag_cor(5,kstat)
...
c
           if(spec_phase.eq.'P'.or.spec_phase.eq.'S') then
              ...
              avsslope=avsslope+sslope(nspec1)
              avswindow=avswindow+swindow(nspec1)
              if(mag_cor(5,kstat).ne.99.0) then
                nmw=nmw+1
                avmw=avmw+mw(nspec1)
...
c
c   clear old values in memory so not written out again
c
       rea_av_moment=0.0
       rea_av_omega0=0.0
       rea_sd_moment=0.0
...
c
      ...
      if(avsslope.gt.999.0)then
         avsslope=999.9
      else
        call calcsd(sslope,avsslope,sdsslope,nspec)
      endif


      if(avswindow.gt.999.9) then
         avswindow=999.9
      else
        call calcsd(swindow,avswindow,sdswindow,nspec)
      endif       
...
c
     ...
     *write(data1(k)(49:54),'(f6.4)') sdradius
      ...
      write(data1(k)(58:62),'(f5.1)')sdsslope
      write(data1(k)(66:70),'(f5.1)')sdswindow
...
c
      ...
      logical mag_write  ! indicater if magnitude written
      integer nhead,nrecord,k,i

      mag_write=.false.

      if(data(1)(7:7).eq.' ') data(1)(7:7)='0'
      if(data(1)(12:12).eq.' ') data(1)(12:12)='0'
      if(data(1)(13:13).eq.' ') data(1)(13:13)='0'
      if(data(1)(14:14).eq.' ') data(1)(14:14)='0'
      if(data(1)(15:15).eq.' ') data(1)(15:15)='0'
      if(data(1)(17:17).eq.' ') data(1)(17:17)='0'
...
c
c   put in a new extra line with magnitudes or use an existing extra line 
c

         do i=2,nhead             ! check if other headers can be used

           if(data(i)(80:80).eq.'1'. 
     *     and.data(1)(46:48).eq.data(i)(46:48).    ! same agency as first
     *     and.data(1)(1:23).eq.data(i)(1:23).      ! same date and time
     *     and.data(i)(24:43).eq.' ') then          ! no location to ind. ex. mag
c
c  check if room on extra line
c

              if(data(i)(56:63).ne.' '.and.
     *           data(i)(64:71).ne.' '.and.
     *           data(i)(72:79).ne.' ')then
                 write(6,'(a,f4.1,a1,a3)') 'Not room in s-file for: ',
     *           mag,type,agency
                 mag_write=.true.  ! no new line
              else
                 if(data(i)(60:60).eq.' ')then 
                    mag_write=.true.
                    write(data(i)(56:63),203) mag,type,agency
                 elseif(data(i)(68:68).eq.' ')then
                    mag_write=.true. 
                    write(data(i)(64:71),203) mag,type,agency
                 elseif(data(i)(76:76).eq.' ')then 
                    mag_write=.true.
                    write(data(i)(72:79),203) mag,type,agency
                 endif
              endif
           endif
         enddo
...
c
          ...
          if(.not.mag_write) then        
              ...
              if(data(2)(7:7).eq.' ') data(2)(7:7)='0'
              if(data(2)(12:12).eq.' ') data(2)(12:12)='0'
              if(data(2)(13:13).eq.' ') data(2)(13:13)='0'
              if(data(2)(14:14).eq.' ') data(2)(14:14)='0'
              if(data(2)(15:15).eq.' ') data(2)(15:15)='0'
              if(data(2)(17:17).eq.' ') data(2)(17:17)='0'
c
c
c
c
c
c
c   use first position since first time extra line is written
c

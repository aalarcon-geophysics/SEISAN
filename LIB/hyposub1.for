...
c   jun 13 2018  jh: for some unknown reason, the weight on s-p is set to  -1
c                    going from one to another when locating in a sequence
c                    there is a check for that
c                    and if a -1 is found, it is reset to 0, change found at 
c                    'jh june 13 2018'
c   dec 04 2018  jh: put in reset test 118 and 119 for gridsearch for a new start location
c                    test 120 and 121 for depth grid search, add routine unfold1 to convert to
c                    decimal deg instead of deg min 
c   apr 22 2019  jh: array bounds error, se jh apr 2019
c   mar 29 2020  jh: increase number of messages from 50 to 2000
c                    owerflow when writing out number of stations, nstation
c   apr 14 2020  jh: do not write out phases that are not seismic phases
c                    like amplitudes
c                    and unidentified phases in order to make print.out simpler.
c                    so if a baz phase, the full phase name is written like BAZ-Pn
c                    while if BAZ in a phase, it is still BAZ. so no more is 2 
c                    lines written if BAZ.
c   jul 20 2020  jh: 9 char stations, look for !nt
c   sep 15 2020  jh: blank phases were printed, now disabled, see 'blank phases'
c                    causes baz on an E line to be printed 2 times, cases where
c                    blank phases should be printed?
c   oct 23 2020  jh: AZ to BAZ
c   nov 18 2020  jh: declare all variables
c   nov 24 2020  lo: init dtwt at start
c   nov 25 2020  lo: checked my changes
c   dec  6 2020  jh: multiple H and E lines, fix so H lines always written if there
c   dec 14 2020  jh: rmssum is now a simple average instead of rms of rms
c   jan 22 2021  jh: add I and E phases to output
c   feb 10 2021  jh: error reading E line, wrong field checked for location method
c                    it was not I and E that should have been added, but blank phases
c                    calculate average of average station residuals
c   mar 25 2021  lo: change in rms calcualtion for baz
c   apr 12 2021  lo: added TEST(122-123)
c   sep 21 2021  jh: add agency
c   dec 30 2022  jh: add geo location, test 124
c   mar 13 2023  jh: depth can now be written to 2 or 3 decimals
c                    depending on size of depth
c   aug 11 2023  jh: a fix to be able to use s-delays, put in first magnitude residual
c
...
c      logical bgs_seisan
     ...
     &fixdep,wtflag,phsflag,fd,fo,magtype
      ...
      character*9 s(nstats) !nt
      character*5 stat   ! not a station, used to read test variables 
      character*4 thrmin
      character*5 depth
      integer*4 idd                                         
      ...
      logical ph_ok,phase_ok   ! true if a seismic phase
      character*3 agency       ! location agency

c change BRL 12/29/93: fmg(narriv) changed to fmg(nstats)              
c 4/94: added sres2, pres2
c 9/94: added di
      real av_sres,av_pres  ! avrage p and s residuals
      integer npres,nsres   ! number of residuals
      character*9 old_stat  !nt
      real old_dist
...
c
c   declarations after implicit none
c

       integer ifunct
       integer iustat          ! unit
       integer iuphs,iulst,iusum
       integer init
       integer maxline           ! lines in output on screen
       integer nhead,nrecord
       real covar,oterr
       real ala1,ala2,alat,alatm,alonm,alon,alo1,alo2
       real az1,az2,azgap,azmax,azmax1,azz,azzm,axxm,azapp
       integer iazflag,iazz,iz,nazimuth,nzcount
       real cvxy,cvxz,cvyz,ddx,ddy,ddz
       real dedegm
       real degtorad
       real dekmm,dly,dmin
       real dor,erh,erx,ery,erz
       real fma
       real fmag,fmg,fmgc,fmp
       integer icd,ichk
       integer iday,idy,idy1,ihr,ihn,imm,
     *         imnth,imnth1,imonth,iyear,iyr,iyr1,jday
       integer ielv               ! station elevation
       integer ievent
       integer ifirst
       integer isindex
       integer igap
       ...
       real parg
       real pi
       real pknt,pknt2,pmean,pres,pres2,pw2res,res,scrst,sknt,sknt2
       integer iphs,ires
       integer isf,isindix
       integer ittt
       integer ix,iy
       integer kntp,knts,ks,kss,kst,kt
       integer la,la1,la2,lo,lo1,lo2,lor
       integer lar
       integer linecount
       integer maxelv
       integer nconrad,nmoho
       integer ndata                    ! arrivals used for laoction?
       integer ndd
       integer nphase,nphase1
       integer nstat,nstation,nstation1
       integer nvalid
       real oldrms
       real rearth
       real rms,rmss1,sarg,smaen,sres,sres2,sw2res,smean
       real testj
       real timdiff,tpp,ttt,ttt2
       real vapp
       real wtmax1,wtt
       real xfar,xfar_old,xhd2,xhdiff,xhm,xhmsave,xk
       real xlatr,xlonr,xmag,xme,xmm,xnear,xnear_old,xnn,xres,xresn
       real xsave,xxlat,xxlon
       real yres,yresn,ysave
       real zres,zresn,zsave,ztr_old
       integer nh        ! number of H  lines
     
       integer i,i1,ii,iddd,idiff,idmin,im,ixmx,iemx,io,is,nn
       integer j,j1,j2,k,k1,kmin,l,m,j2im
...
c
      idummy=maxline
c
c reset nmessage, lo correct??? jh: yes, else there is array overflow
c when many events are located
c
      nmessage=0
      ...
      rearth=6371.
c
c   with new nordic format, only delete items filled out by hyp 
c   the rest comes from input file to be carried on like component, network ..
c
      do i=1,max_data
        data5(i)(1:74)=' '
      enddo
...
c
c call function to determine if BGS seisan outputs are to be used
c bjb 2001/02/14
c
...
1729    format(1x,/,' Maximum elev. station: ',a9,1x,2(I4,f5.2,a1),
     &  '   elevation = ',i5,' m') !nt
...
c
     ...
     &    oldrms,appar_veloc)
c lo force S-P indicator to 0 at start
c         write(55,'(a)') data(1)
          do i=1,ks
            ips(i)=0.
          enddo
...
c
c   check for station code and ntlo
c
          do 15 j=1,nstat
c
c
c          if(phase(i)(1:4).eq.' '.or.phase(i)(1:1).eq.'I') goto 133
15        if(s(j).eq.st(i)) go to 16
c
c   check for only station code  !nt
c 
          do  j=1,nstat             
             if(s(j)(1:5).eq.st(i)(1:5)) go to 16
          enddo

          if(iulst.gt.0)write(iulst,105)st(i)
          write(*,105)st(i)
105       format(1x,'Station ',a9,' is not on station list: ignored') !nt
...
c change 10/93: max elev removed from here: in init section
          ...
          tp(iphs)=tp(i)
          fmp(iphs)=fmp(i)
          appar_veloc(iphs)=appar_veloc(i)   ! lo 13/11/2020
          if(phase(iphs)(1:1).eq.'P')dl(iphs)=dly(j)
cdly          if(phase(iphs)(1:1).eq.'S')dl(iphs)=dly(j)*pos
          if(phase(iphs)(1:1).eq.'S')dl(iphs)=dlys(j)
          dtwt(iphs)=0.0 ! lo 24/11/2020 - seems that this gave problem in batch location,
c           setting dtwt to 0 at start, these are later set by rmsv (?)
c change 10/93: initialize dtw1
          dtw1(iphs)=0.0

c          write(*,*) ' debug lo setting weights  '
c    calculate the station weights, dtwt(i)  (never changed)
          if(ip(iphs).ge.0.and.ip(iphs).le.4.and.phase(iphs)(1:3).ne.
     &    'BAZ')then

            dtwt(iphs)=(4.-float(ip(iphs)))/4.   
c            write(iulst,'(a8,f6.1)')phase(iphs),dtwt(iphs)
          elseif(phase(iphs)(1:3).eq.'BAZ'.and.ip(iphs).ge.0)then
            if (ip(iphs).gt.4)ip(iphs)=4 ! lo 17/11/2020, dont use baz with weight 9
c change 10/93: calculate azimuth weights here
            dtwt(iphs)=(4.-float(ip(iphs)))/4./(degtorad*test(52))
c      write(55,*) ' debug lo set dtwt ',phase(iphs),ip(iphs),dtwt(iphs)
          elseif(ip(iphs).ne.9)then
            write(iulst,'(a9,1x,a8,i3, '' invalid weight: set to zero'' 
     &       )')st(iphs),phase(iphs),ip(iphs)       !nt
            dtwt(iphs)=0.0
          endif
...
c 5/94 find first arrival station
        if(fixepi.eq.' '.or.ucase(fixepi).eq.'N')then
         ...
         ifirst=0
c
c  next if was put in to avoid ifirst to be zereo when there was no phases
c  without array bounds check, it was not seen, jh apr 2019
c
         if(nphase.gt.0) then      
            do i=1,nphase        
...
c   of input variables since some variables times are replaced
c   by e.g. S-P times in hypoloc1, hard to figure out exactly which
...
c
c         write(55,*) ' debug lo nstartdepth ',n_start_depths
         do i=1,n_start_depths
            if(n_start_depths.gt.1) then
c                if(diff_phase) then   ! reread all from 2. time
...
c      do ii=1,nphase
c          write(55,*) ' debug lo bef hl ',ii,st(ii),phase(ii),
c     &      dtwt(ii),dtw1(ii)
c      enddo
...
c
             ...
             ! this is the normal location in most cases
c
c   for some unknown reason, the weight on s-p is set to  -1 going from one 
c   to another when locating in a sequence so here there is a check for that
c   and if a -1 is found, it is reset to 0
c   jh june 13 2018

c lo commented out 24/11/2020 problem still there, can we fix?
c         do ii=1,nphase
cc              write(6,*)phase(ii),dtwt(ii),ips(ii)
c               if(dtwt(ii).eq.-1.0) dtwt(ii)=0.0
c         enddo
...
c      do i=1,nphase
c         write(55,*) ' debug lo after hl ',i,st(i),phase(i),
c     &      dtwt(i),dtw1(i)
c      enddo
...
c accumulate mean square rms value
        if(nphase.gt.3.and.rms.le.test(43))then
          ievent=ievent+1
c
c          rmssum=rmssum+rms*rms
           rmssum=rmssum+rms    ! jh dec 16 2020
        endif
...
c it is used to renormalize all the final weights
        ...
        do kst=1,nphase
         if(dtwt1(kst).gt.wtmax1.and.ip(kst).
     *    eq.0.and.phase(kst)(1:3).ne.
     &   'BAZ'.and.phase(kst)(1:3).ne.'END')wtmax1=dtwt1(kst) ! lo

c change 4/94: dtw1(kst) now .ge.0 - was previously .gt.0 allowing
c bisquare weighting to remove phases
         if(dtw1(kst).ge.0.0.and.phase(kst)(1:3).ne.'BAZ')then
          res=res+dt(kst)**2
          ndd=ndd+1
         endif
        end do
...
c        do i=1,nphase
c          write(55,*) ' debug lo before sor ',i,st(i),phase(i),
c     &      dtw1(i)
c        enddo
...
c       
          if(phase(k)(1:3).eq.'BAZ') nazimuth=nazimuth+1    ! fix new                                        

c 10/93: added '*' to showed changed weights and phases
          phsflag=' '
          wtflag=' '
          if(dtw1(k).ne.dtwt(k).and.ip(k).ne.9)wtflag='*'
          if(trphs(k)(1:4).ne.phase(k).and.phase(k)(1:3).ne.'BAZ'.and.
     &        phase(k)(1:3).ne.'END'.and.   ! lo
     &        ips(k).ne.1.and.phase(k)(1:4).ne.'    ')phsflag='*'

c accumulate residual sums
c 4/94: added pres2 & sres2 to calculate rms deviation
          if(phase(k)(1:3).ne.'BAZ'.and.ips(k).ne.1.and.ip(k).le.4)then
             wtt=(4.-float(ip(k)))/4.
...
c
          if(phase(k)(1:3).eq.'BAZ') then    ! fix new
...
c           write(6,*)'az0 = ',az0(k),'azb= ',baz0(k),'tp(k)= ',tp(k)
     ...
     &         f7.2,f5.2,a1,i2,1x,a4)')st(k)(1:5),int(dlt(k)+.49999),
     *         azz,
c    &         phase(k),tp(k),azb,ttt2,degtorad*dtwt1(k),wtflag
     &         phase(k),tp(k),baz0(k),ttt2,degtorad*dtwt1(k),wtflag
     &         ,int(di(k)+.4999),st(k)(6:9) !nt
            endif    


            write(data5(is)(37:60),'(f7.2,1x,f7.2,1x,f5.2,1x,i2)')
...
c            
     ...
     *      write(data(is)(71:75),'(f5.1)')(dlt(k) -0.01) 
            if(dlt(k).ge.100)write(data(is)(71:75),'(i5)')
     *      int(dlt(k)+0.5)         
            iazz=int(azz+.49999)
...
c
c    end of BAZ residual , delta and az write out
c 
...
c
c
c   only print real phases here, below blank phases are allowed
c
            ph_ok=phase_ok(phase(k))

            if(ph_ok.
     *      and.ips(k).eq.0.and.dtw1(k).ge.0.0.and.(ip(k).ne.9.
...
c 2/2006 lot, 2 decimals for travel times
     ...
     &            2a8,2i2,f5.1,2f7.2,f7.2,f5.2,a1,i2,1x,a4)')
     *            st(k)(1:5),
     ...
     *            st(k)(6:9)  !nt
                  ...
                  write(iulst,'(1x,a5,i5,f6.1,f5.1,1x,i1,1x,
     &            2a8,2i2,f5.1,2f7.1,f7.2,f5.2,a1,i2,1x,a4)')
     *            st(k)(1:5),
     &            int(dlt(k)+.49999),azz,aninc(k),ip(k),phase(k),
     &            trphs(k),ihn,imm,xhm,tpp,tpc(k),dt(k),
     &            dtwt1(k)/wtmax1,wtflag,int(di(k)+.4999),
...
c
c  2-02 jh  write ainc, angle of incidence to data array
c
...
c store additional info
              write(data5(is)(1:35),
     *        '(a8,1x,f8.3,1x,f8.3,1x,f5.2,1x,i2)')
...
c
c as of april, 2020, all these phases are not printed out
c
            elseif(dtw1(k).lt.0.0.or.ip(k).eq.9.and.nzcount.eq.0)then
            if(ph_ok.or.phase(k).eq.' ') then ! blank phases printed  jh 
c                 if(ph_ok) then ! blank phases not printed   jh
                 if(phase(k).eq.' ') then     ! allow blank phases
                    if(iulst.gt.0.and.(.not.multi_model))
     *              write(iulst,'(1x,a5,i5,f6.1,6x,i1,1x,
     &              a8,8x,2i2,f5.1,30x,a)')st(k)(1:5),
     &              int(dlt(k)+.49999),azz,ip(k),phase(k),ihn,imm,
     &              xhm,st(k)(6:9) !nt
                 else
                   if(iulst.gt.0.and.(.not.multi_model))
     *             write(iulst,'(1x,a5,i5,f6.1,6x,i1,1x,
     &             a8,8x,2i2,f5.1,f7.1,23x,a)')st(k)(1:5),
     &             int(dlt(k)+.49999),azz,ip(k),phase(k),ihn,imm,
     &             xhm,tpp,st(k)(6:9) !nt
                 endif
              endif
...
c
            elseif(ips(k).eq.1.and.dtw1(k).ge.0)then
              ...
              if(iulst.gt.0.and.(.not.multi_model))
     *        write(iulst,'(1x,a5,i5,f6.1,f5.1,1x,i1,1x,
     &        2a8,9x,2f7.1,f7.2,f5.2,a1,i2.1x,a4)')st(k)(1:5),
     *        int(dlt(k)+.49999),
     &        azz,aninc(k),ip(k),phase(k),trphs(k),tpp,tpc(k),dt(k),
     &        dtwt1(k)/wtmax1,wtflag,int(di(k)+.4999),st(k)(6:9)  !nt
              write(data5(is)(1:35),
     *        '(a8,1x,f8.3,1x,f8.3,1x,f5.2,1x,i2)')
     &        trphs(k),tpp,tpc(k),dtwt1(k)/wtmax1,int(di(k)+.4999)
c       write(55,*) ' data5 ',st(k),k,data5(is)   ! debug lo
            endif   ! end of non BAZ phases
c        write(55,*) ' debug lo ',k,st(k),phase(k),dtwt(k),dtw1(k)
c      if (ips(k).eq.1) 
c     &   write(55,*) ' debug lo ips ',k,st(k),dtwt(k),dtw1(k)
c
cccccccccccccccccccccccccccccccccccccccccccccccccccc
c  end of writing to print.out in station loop
cccccccccccccccccccccccccccccccccccccccccccccccccccc
c         
...
c
c   write depth with optimal digits, jh mar 2023
c
        if(zsave.lt.100.0) 
     &   write(data(1)(24:43),'(f7.3,f8.3,f5.1)')xxlat,
     &  xxlon,zsave   
        if(zsave.ge.100.0) 
     &   write(data(1)(24:43),'(f7.3,f8.3,f5.0)')xxlat,
     &  xxlon,zsave   
...
c
c  ndata is number of phases used in final solution
c  this looks funny, ndata1 never used, should probably be nstation
c  so changed to that
c
cjh        ndata1=ndata
cjh        if(ndata1.gt.999)ndata1=999
        nstation1=nstation
        if(nstation1.gt.999)nstation1=999
c        write(*,*) ' debug lo writing ',nstation

        write(data(1)(49:51),'(i3)')nstation1
        rmss1=rms
...
c 10/94 look for error records for that hypocenter, overwrite if same aga and method
c

        k=0                                   
        do i=2,nhead
          if(data(i)(80:80).eq.'E'.and.
     *    data(i)(10:10).eq.' '.and.              ! location method is hypocenter
     *    (data(i)(12:14).eq.agency)) k=i
        enddo 
...
cxx        data(k)(12:14)=hyp_agency(1)(1:3)
        data(k)(12:14)=agency    
...
c
c   check if a high accuracy line present for main hypocenter
c
        l=0
        do i=2,nhead           ! find if a line already there
           if(data(i)(80:80).eq.'H'.and.
     *     data(i)(6:6).eq.' '.   ! hypocenter  has blank as location method
     *     and.(data(i)(61:63).eq.hyp_agency(1).or.nh.eq.1)) l=i
       enddo

c
c   check if high accuracy output or if a H line is alredy there, then it is also
c   updated
c

        if(high_accuracy.or.l.gt.0) then
...
c          if(ievent.gt.0)rmssum=sqrt(rmssum/float(ievent))
          if(ievent.gt.0)rmssum=rmssum/float(ievent)    ! jh dec 16 2020
...
c
c   make summary of average resduals of all stations
c         
          write(iulst,980)
980       format(/,' Average station residuals (weighted):'//,
     &    ' station',9x,'p residual rms dev  no p',5x,'s residual rms',
     &    ' dev  no s') !nt
981       format(1x,a9,9x,2f8.2,i6,7x,2f8.2,i6) !nt
     ...
     &        pres2(i),kntp(i)
982           format(1x,a9,38x,2f8.2,i6) !nt
              if(isf.eq.1.and.ipf.eq.0)write(iulst,982)s(i),sres(i),
     &        sres2(i),knts(i)
            endif
c
c  average residuals of all
c
            if(kntp(i).gt.0.and.abs(pres(i)).gt.0.0) then
              npres=npres+kntp(i)
              av_pres=av_pres+pres(i)*kntp(i)
            endif
            if(knts(i).gt.0.and.abs(sres(i)).gt.0.0) then
              nsres=nsres+knts(i)
              av_sres=av_sres+sres(i)*knts(i)
            endif
          end do
          if(npres.gt.0) av_pres=av_pres/npres
          if(nsres.gt.0) av_sres=av_sres/nsres
          write(iulst,'(a,13x,f6.2,23x,f6.2)')
     *    ' Average',av_pres,av_sres         
c         if(ievent.gt.0)rmssum=sqrt(rmssum/float(ievent))
...
c   new declarations
c
      real alat,alon,ala,alo       ! lat lon
      integer la,lo
      real c1,c2
      real deg
      real equrad
...
c
      ...
      real cdelt,ct0,ct1,cz0
      real delat,delon,dlon,slat,slon
      real delta
      real az1
      real radius
      real sdelt,st0,st1
...
c
      ...
      real slat,slon,dekm,dedeg,az0,eqlat,eqlon
      real cdelt,cdlon,ct0,ct1
      real sdelt,sdlon,st0,st1
...
c
      ...
      real alat,alon,ala,ala1,alo,alo1,blat
      integer lo,la
...
c
      ...
      subroutine unfold1(alat,alon,xlat,xlon)
c     
c
c-------- given geocentric lat and lon compute geographic coordinates in decimal
c
c     same as unfold, only output is decimal, jh 2018
c
c
c input and output definition just reverse of entry fold
c
...
c     alat     geocentric lat (radians)
c     alon                lon (radians)
c     xlat,xlon  geographic lat lon in deg
c     
c
c gctogg - convert from geocentric to geographic latitude
      if (halfpi-abs(alat) .ge. 0.02) goto 403
...
c     test(118) decides if a local minimum should be searched for after
c               iterations ends. If an RMS with lower rms than the iterated
c               solution is found, a new iterations will be done with that
c               solution as a starting epicenter. if zero, grid search 
c               is not done, if positive nymber grid search is done with the number
c               used as grid size.
c               
c     test(118)=0.0
      test(118)=0.0
      
c     test(119) is the number of gridpoints in one quadrant so the grid will be
c               +/-test(119) on each side
c     test(119)=10.0
      test(119)=10.0

c     test(120) decides if depth grid search for a local minimum like test 118
c               test 118 and 120 can be combined. the value is the size of the
c               depth step (km). default 0.0, no grid search.
      test(120)=0.0

c     test(121) is the number of steps in depth grid search. default is 10
      test(121)=10.0

c 122 and 123 used for short distance ml, disabled if 0.
      test(122)=0.
      test(123)=0.
c
c  geo location
...
c
c   not used, not put in implicit none
c   
...
c      init=-1 means EOF
      ...
      save
      character*8 phase(*)
      character*9 st(*),statid  ! nt changed but routine not used   
...
c  new declarations
c
      real alat,alon,dly,fmg
      integer la,lo,nstat,nstats

      integer i,j,k,iunit1,iunit2,init,ielv
...
c   statid:       station identifier (9 letters max.), now include nt and lo
...
2     continue
     ...
     *   ins(i),lo(i),
     ...
     &,   (scrst(i,j),j=1,9),statid(i)(6:9)  !nt                                !scrst added
         statid(i)(5:5)=' '
      else    ! 5 char station
         read(text,1103,err=8844)statid(i)(1:5),la(i),alat(i)
     *   ,ins(i),lo(i),
     &   alon(i),iew(i),ielv(i),dly(i),(mag_cor(k,i),k=1,5) !mag_corr added
     &,   (scrst(i,j),j=1,9),statid(i)(6:9)                     !nt        !scrst added
      endif
 ...
 8844 continue
      ...
      stop
 8845 continue
103   format(2x,a4,i2,f5.3,a1,i3,f5.3,a1,i4,f6.2,5f5.2,9f6.2,a4)  ! 4 char station  !nt
1103  format(1x,a5,i2,f5.3,a1,i3,f5.3,a1,i4,f6.2,5f5.2,9f6.2,a4)  ! 5 char station  !nt
c
...
1203  format(1x,a9,i2,f6.3,a1,i3,f6.3,a1,i5,f6.2,5f5.2,9f6.2)  ! 5 char station !nt
...
c  new delcarations
c
      real alat1,alon1,aza,alat2,alon2,azb, 
     *  dista,distb,alat,alon
      integer ierr
      real alatin,alonin,az,azi,baz
      real c1,c2,c3,c4,c5,f,g,h,e
      real ra,rb,rc
      real degtrad,delta,dist
      integer npode                               
...
c
      ...
      real cdel,cdlon,clat1,clat2,dtor,rdlon,rlat1,rlat2,rtod,sdlon
      real slat1,slat2,xazi,xbaz,yazi,ybaz
...
c 
      implicit none
      real x(3),rtod,radius,alat,alon  
      real r13sq,r13,r123,z                                                         
...
c   
      implicit none                                                       
      real x(3) 
      real alat1,alon1,delta,azi, alat2,alon2,dlon,z                                      
c                                                          
...
c 
      ...
      real rtod,alat,alon,alatr,alonr,sinlat,coslat,sinlon,coslon  
      real a,b,c                                                 
      parameter (rtod = 57.2957795)                       

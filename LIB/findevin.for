...
c                    bug to be fixed
c   2019 08 08 jh  : wrong documentation on default data base
c   2020 04 30 lo  : changes for mulplt cashe
c   2020 04 11 jh  : back did no work
c
c   input: base_name : Blank : default data base
c          ,,        : local data base       
...
c                         directory name must be terminated by a /, only an indicater
c                         and not related to a directory separator. this
c                         option only used in SE                          
c                     
...
c
c   case of absolute path, only used in SE
c
             if(base_name(i1:i1).eq.'/') then
                standard=.false.
                sdir(1:i1)=base_name(1:i1)   ! replace standard sdir
                i2=i1-11
             else
...
c-- cannot backspace from beginning of list         
	if(old_no.ne.1) then   
           event_no=event_no-1
c else
c   evfile=sfiles(event_no)
        else
c jump to last event, lo 30/4/2020
             event_no=total_no_events
        endif
        evfile=sfiles(event_no)  ! added to make lo change work 11/10/20 jh 
        if(slength.gt.0) then
             if(base_dir.eq.'     ') then    ! index file, fix year and month
                   sdir(slength-7:slength-4)=syear(event_no)
                   sdir(slength-2:slength-1)=smonth(event_no)

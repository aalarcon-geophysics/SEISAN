...
c   2020 03 20   lo: added which archive to support multiple folders
c   2020 04 06   lo: call to waveform_cache
c
c
...
c
      ...
      integer seiclen
      character*80 archive               ! used when calling archive routines
      ...
      integer tyear,tmonth,tday,thour,tmin,tdoy
      real tsec
...
c have to search for channel archive here
c         write(*,*) ' debug lo in mulplt ',arc_folder(ichan)
         comp3=wav_comp(ichan)(1:2)//wav_comp(ichan)(4:4)
c         write(*,*) ' debug lo a ',arc_folder(561)
         call which_archive(archive,
     *     wav_stat(ichan),comp3,
     *     wav_network(ichan),wav_location(ichan),
...
c         call  getarchdata(arc_archive,j,arc_type,
         call  getarchdata(archive,j,arc_type,
     *   wav_stat(ichan),5,
     *   b_comp,3,
     ...
     *   signal_int)
c send udp message to waveform_cache to read next segment from same channel
         udptext=' '
         portcache=5100
         call timsec(wav_year(ichan),wav_month(ichan),wav_day(ichan),
     &      wav_hour(ichan),wav_min(ichan),wav_sec(ichan),msec)
c         write(*,*) ' debug lo kk=',kk
         msec=msec+float(kk)
         call sectim(msec,tyear,tdoy,tmonth,tday,thour,tmin,tsec)
         call systime_msec(msec)
         write(udptext,
     &     '(a10,1x,f14.2,a80,i1,a5,a3,a2,a2,i4,4i2,f5.2,i5)')
     &     'READARC   ',msec,archive,arc_type,wav_stat(ichan),b_comp,
     &     wav_network(ichan),wav_location(ichan),
     &     tyear,tmonth,tday,thour,tmin,tsec,kk
...
c have to search for channel archive here
         comp3=wav_comp(ichan)(1:2)//wav_comp(ichan)(4:4)
c         write(*,*) ' debug lo b ',arc_folder(561)
         call which_archive(archive,
     *     wav_stat(ichan),comp3,
     *     wav_network(ichan),wav_location(ichan),
     *     wav_year(ichan),wav_month(ichan),wav_day(ichan),
...
c         call  getarchdata(arc_archive,j,arc_type,
     ...
     *   signal_int)
         ...
         enddo
c send udp message to waveform_cache to read next segment from same channel
c commented out as probablt better to read next sfile, as implemented in
c eev_sub.for
c         udptext=' '
c         portcache=5100
c         call timsec(wav_year(ichan),wav_month(ichan),wav_day(ichan),
c     &      wav_hour(ichan),wav_min(ichan),wav_sec(ichan),msec)
c         msec=msec+float(interval)
c         call sectim(msec,tyear,tdoy,tmonth,tday,thour,tmin,tsec)
c         write(udptext,'(a10,1x,a80,i1,a5,a3,a2,a2,i4,4i2,f5.2,i5)')
c     &     'READARC   ',archive,arc_type,wav_stat(ichan),comp3,
c     &     wav_network(ichan),wav_location(ichan),
c     &     tyear,tmonth,tday,thour,tmin,tsec,interval
...
c         write(*,*) ' debug lo yes ',arc_time
         if(arc_time.lt.arc_start(i).or.arc_time.gt.arc_stop(i)) then
c           write(*,*) ' debug lo outside channel times'
           goto 200
         endif
c         write(*,*) ' debug lo no '
...
c         j=seiclen(arc_archive)
c have to search for channel archive here
c         write(*,*) 'debug lo crashing here? '
c         write(*,*) ' debug lo c ',arc_folder(561)
         call which_archive(archive,
     *     arc_stat(i),arc_comp(i),arc_net(i),arc_loc(i),
     *     year,month,day,hour,min,sec)
c       write(*,*) ' debug lo ',archive
         j=seiclen(archive)
...
c         j=seiclen(arc_archive)
c have to search for channel archive here
c         write(*,*) ' debug lo d ',arc_folder(561)
         call which_archive(archive,
     *     stat,comp,net,loc,
     *     year,month,day,hour,min,sec)
         j=seiclen(archive)
...
c
      ...
      subroutine which_archive(archive,
     &     stat,comp,net,loc,
     &     year,month,day,hour,min,sec)
c
c find archive folder for given wave channel, return general
c arc_archive if not found
c
c Lars Ottemoller, March 2020
c
      ...
      include 'seisan.inc'
      include 'waveform.inc'
      character*(*) archive          ! name of folder with archive to be returned
      character*5 stat
      character*3 comp
      character*2 net,loc
      double precision msec ! time of data and archive in seconds
...
c      write(*,*) arc_nchan
      do i=1,arc_nchan
c         write(*,*) ' debug lo which ',i,arc_folder(i)
c      if (stat.eq.'BAS23'.and.arc_stat(i).eq.'BAS23') then
c        write(*,*) ' debug lo in which ',i,stat,comp,net,loc,
c     &      ',',arc_stat(i),arc_comp(i),arc_net(i),arc_loc(i),
c     &      ' ',arc_folder(i)
c      endif
c check for scnl
        if (
     *    stat.eq.arc_stat(i).and.
     *    comp.eq.arc_comp(i).and.
     *    net.eq.arc_net(i).and.
     *    loc.eq.arc_loc(i)) then
c check time
c           write(*,*) ' found '
c          write(*,*) msec,arc_start(i),arc_stop(i)
          if (msec.ge.arc_start(i).and.
     *       (arc_stop(i).ge.msec.or.arc_stop(i).eq.0.)) then
            if (archive.eq.' ') then
c              write(*,*) ' in time interval ',arc_folder(i)
              archive=arc_folder(i)
            else 
              write(*,*) ' WARNING: archive channel defined more '//
     *          ' than once ',stat,comp,net,loc
            endif
c          else 
c            write(*,*) ' debug lo time ',msec,arc_start(i),arc_stop(i)
          endif
        endif
      enddo
c if not set, use arc_archive
      if (archive.eq.' ') archive=arc_archive
c      if (stat.eq.'BAS23'.and.arc_stat(i).eq.'BAS23') then
c        write(*,*) ' debug lo which_archive ',archive
c      endif
      return
      end

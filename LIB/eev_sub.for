...
c  may    08  jh  : change array to pfit
...
c  2019 02 25 jh: new format
c  2019 12 17 jh: set reading to end of s-file with rea_read_to_sfile_end
c  2019 12 19 jh: make input for 3. magnitude on header line
c  2020 01 16 jh: keep id line using command DUP
c  2020 06 18 jh: put in command us to make id line if missing and
c                 synchonice id and file name with origin time
c  2020 09 02 jh: fix M message, fix so U does not work as up
c  2020 09 15 jh: do not allow command us from index file
c  2020 09 22 jh: hash to hash_seisan, hash is a bash command, mt_val different 
c                 format in command fm, wrong in mt inversion
c  2020 11 03 jh: always send mail sfile in nordic format
c  2020-11-23 pv: iloc plugin added
c  2021-01-22 jh: add making a summary report and send on a separate email
c                 using emailsum.option for map in emailsum,
c                 find geogrphical loaction from a place name 
c                 file place_names.eev, fix autophase to new format
c                 put in a system command after sending email
c  2021-02-16 jh: option g
c  2021-03-19 jh: when fix command was used and new depth given, the high 
c                 accuracy line was not changed, fixed by using the rea
c                 routines.
c  2021-03-19 jh: comment out pmac
c  2021-04-30 jh: add command id, fix quarry so info can be put into s-file
c  2021-09-16 jh: fix new format for command NEW
c  2021-11-25 jh: mt_val(5,i)=myz and mt_val(6,i)=mxy were switched in Dreger MT
c                 write out in sfile, fix signs so mopad plots correctly MT with
c                 carthesian coordinate system, problem making MT paramters due to
c                 new spec lines
c  2022-01-06 jh: fix dd command so agency can be entered to avoid two.
c                 identical lines (removed again, maybe no needed)
c                 add command eh to edit hyp.out file
c  2022-01-19 jh :R command can also blank out event id, add more info and
c                 also allow to use '_'.  Add command dm to delete many s-files
c  2022-02.17 jh : allow several comments at the same time with command IC
c  2022-04-28 jh : command test will run program test_prog with argument
c                  sfile name. name test for program cannot be used
c                  since there seems to be a test command in linux
c                  when starting a program with a system call.
c  2022-06-02 jh : do not ask for data base if usign REG from eev,
c                  assume it is the current data base
c  2022-07-06 jh : add map_lat_div, map_lon_div
c  2022-09-26 jh : a bug with s command. after call there was a
c                  return instead of goto 10 so evname was lost
c                  so when plotting, wrong event was plotted
c  2022-12-30 jh : add subroutine geo_location
c  2023 02 02 jh : change error message text for w-command
c  2023 02 14 jh : add command PMR to plot magnitude residuals
c  2023 03 16 jh : email can also send in nordic2 format, see SEISAN.DEF
c                 
...
c-- operator code
      ...
      character*1 ns,ew
      character*2 nsw              ! direction NS, NW etc
      character*8 mag_type         ! magnitude phase like IAML
      character*1 mag              ! magnitude type like L
...
c-- event ids--------------               
      ...
      real dist,azim,bazim,amp           ! distance and azimuth  and b azimuth and amplitude
...
c-- number of arc entries in s-file
      integer karc
c-- counter                                                    
      integer		i,j,k,kk,ilen,l,m,n,n1,n2,kkk(10)
...
c      fps_program=' '
       agency=' '
       rea_read_to_sfile_end=-1   ! read to end of single files
...
c send message to put waveforms into cache, lo
c probably too much ???
c      if (firsttime) then
c       write(*,*) ' debug lo findevin '
c         udptext=' '
c          write(udptext,'(a10,1x,a80)')
c     *     'READSFILE ',evfile
c          call send_udp_msg(portcache,udptext)
c        firsttime=.false.
c      endif
...
c  
      ...
      if(keys(1:3).eq.'geo') keys(1:3)='GEO'
      if(keys(1:4).eq.'ge  ') keys='GEMSC'
      if(keys(1:4).eq.'gu  ') keys='GUSGS'
      if(keys(1:4).eq.'gi  ') keys='GISC'
      if(keys(1:4).eq.'gp  ') keys='GPDE'
      if(keys(1:1).eq.'g') keys(1:1)='G'
     ...
     *  'Give spectral and wa windows, a zero means not processing'
      ...
      if(keys(1:3).eq.'id ') keys(1:7)='EVENTID'
      if(keys(1:2).eq.'dm')  keys(1:2)='DM'
      if(keys(1:6).eq.'email ') keys(1:6)='EMAIL '
      if(keys(1:8).eq.'emailsum') keys(1:8)='EMAILSUM'
      ...
      if(keys(1:3).eq.'pmr') keys(1:3)='PMR'
      ...
      if(keys(1:4).eq.'test') keys(1:4)='TEST'
      ...
      if(keys(1:2).eq.'qp') keys(1:2)='QP'

      if(keys(1:2).eq.'ap') keys(1:2)='AP'
      if(keys(1:9).eq.'autophase') keys(1:2)='AP'
      if(keys(1:3).eq.'il ') keys(1:3)='IL '
      if(keys(1:5).eq.'iloc ') keys(1:5)='ILOC '
...
c      if(keys(1:4).eq.'pmac') keys(1:4)='PMAC'
      ...
      if(keys(1:8).eq.'inputmag') keys(1:8)='INPUTMAG'
      if(keys(1:2).eq.'im') keys(1:8)='INPUTMAG' 
...
c   flip fix flag
c
         if(hyp_depth_flag(1).eq.' '.or.hyp_depth_flag(1).eq.'S') then      
cc         if(data1(1)(44:44).eq.' '.or.data1(1)(44:44).eq.'S') then
cc            data1(1)(44:44)='F'
              hyp_depth_flag(1)='F'
            write(6,*)
     *      ' Give depth to fix to, enter to keep depth in file'
            ...
            if(text.ne.' ') then
               read(text,*) depth(1)
cc               write(data1(1)(39:43),'(f5.1)') depth(1)
                 hyp_depth(1)=depth(1)
...
c                            
         ...
         call sei code(fort$,code,write01,b_eof)
cc         do i=1,nrecord1
cc          write(write01,'(a)',iostat=code) data1(i)
cc          call sei code(fort$,code,write01,b_eof)
cc        enddo
...
c
         write(6,'(a,$)') 
     *   ' Input model indicator, one character, enter to remove: '
         read(5,'(a)')text
...
c
         write(6,'(a,$)') 
     *   ' Input event ID like P, one character, enter to remove: '
         read(5,'(a)')text
         data1(1)(23:23)=text(1:1)
c  
c-- write in file
c                            
         rewind (write01,iostat=code)
...
c
 ...
 3849    continue
c        write(6,*)'The two header lines must have different agencies'
c        write(6,*)'Change(1) or (2), e.g. 2 XXX, XXX can be blank'
c        read(5,'(i1,1x,a3)') i,text(1:3)
c        if(i.ne.1.and.i.ne.2) goto 3849
...
c       text(4:6)=data1(1)(46:48)
c       if(i.eq.1) then
c          data1(1)(46:48)=text(1:3)
c       endif

        write(write01,'(a)') data1(1)
c       data1(1)(46:48)=text(4:6)
...
c
 1630   continue
         write(6,*)
         write(6,'(a)')
     *  ' This command will overwrite 3. magnitude on header line'               
         write(6,'(a)')
     *  ' Input magnitude like 4.5LISC, 7 chars, blank will delete'

         read(5,'(a)')text(1:7)
c
c  some check of string
c
         read(text(1:3),'(f3.1)',err=1631) x
         if(x.gt.9.9) then
            write(6,*)'Magnitude too large'
            goto 1630
         endif
         if (text(4:4) .ne. 'L' .and. text(4:4) .ne. 'B' .and.
     1       text(4:4) .ne. 'S' .and. text(4:4) .ne. 'C' .and.
     2       text(4:4) .ne. 'W' .and. text(4:4) .ne. 's' .and.
     2       text(4:4) .ne. 'N' .and.
     3       text(4:4) .ne. 'b' .and. text(4:4).ne.' ') then
             write(6,*)'Wrong magntude type'
             goto 1630
         endif
         goto 1632
...
c        if(text(1:7).eq.' ') then
c          call sei close (close$,write01,code)
c          write(6,'(a)') ' No change'
c          keys(1:4)='SAME'
c          goto 10
c        endif
...
c
c------------------------------------------------------------------------
c run a test program
c------------------------------------------------------------------------
c
      if(keys(1:4).eq.'TEST') then
         write(6,*)'Run program test_prog with argument ',
     *   evfile(1:fstart+20)
         text='test_prog '//evfile(1:fstart+20)
         write(6,*) text
...
c
 ...
 9679    continue
     ...
     *     text(1:34),ref_station,baz,appvel
           ...
           if(answer.eq.'y') then
              do i=1,rea_nphase
                 if(rea_stat(i).eq.ref_station.
     *           and.rea_phase(i)(1:1).eq.'P') then
...
c
c   check if phase not there from before, then overwrite
c
           k=rea_nphase+1      ! normally a new phase
           do l=1,rea_nphase
              if(rea_stat(l).eq.ref_station.and.
     *        rea_phase(l)(1:5).eq.'BAZ-P') then
                 k=l
              endif
           enddo

           call rea_copy_phase(i,k,1)
           if(k.eq.rea_nphase+1) rea_nphase=rea_nphase+1
           rea_phase(k)(1:5)='BAZ-P'
           rea_onset(k)=' '
           rea_baz_obs(k)=baz
           rea_vel(k)=appvel
           call rea_event_out(0,.true.,data1,k)
           call write_s_file_e(evfile)
 9680    continue
         close(80)
         keys(1:4)='SAME'
...
c------------------------------------------------------------------------
c      if(keys(1:4).eq.'PMAC') then
c        if(.not.pc) then
c          write(6,*)' Program only works on PC'
c        else
c          text='promac ' // evfile(1:fstart+20)
c          call systemc(text,fstart+27)
c        endif
c        keys(1:4)='SAME'
c        goto 10
c      endif
...
c
c-----------------------------------------------------------------------
c delete many events
c-----------------------------------------------------------------------
c
      if(keys(1:2).eq.'DM') then

         current_event_number=event_no 
         write(6,*)
     * ' Delete events, how many starting from current ?'
         write(6,*)' There is no backup to DELET data base'
         read(5,*) l
c
c  delete
c
...
c
            if(i.gt.1.and.event_no.eq.1) then
               write(6,*)' Reached end of list, stop delete at # ',i
               goto 3346   
            endif
            open(111,file=evfile,status='unknown')
...
c        
c   go to event before start event snce that was deleted
c
         keys(1:1)='#'
         write(keys(2:5),'(i4)') current_event_number-1 
         call findevin
     *   (base_name,start_time,end_time,keys,from_eev,event_no,
     *   evfile,fstart,new_month,status)
         keys(1:4)='SAME'
...
c         write(*,*) ' debug lo ',id,evfile
         if (id.gt.0) then    ! lo 10 oct 2019
...
c   assume same base
c
         reg_base=base_name(1:5)     ! jh jun 2 22
         write(6,'(a,a)') 'Current data base: ',reg_base
...
c               write(6,*) 'Give operator code, max 3 characters'
c               read(5,'(a)') operator
c               goto 4723
c            endif
c
...
c quick phase plot
c------------------------------------------------------------------------
      if(keys(1:2).eq.'QP') then
        call systemc('plotphase '//evfile(1:seiclen(evfile))//' all',
     &     13+seiclen(evfile)+4)
         keys(1:4)='SAME'
         goto 10
      endif
...
c         write(*,*) data1(1)(24:38)
         ...
         if(nquarry.eq.0) then
             call sei close (close$,read01,code)  
             goto 10
         endif        
         do k=1,nquarry
           if (k.eq.1) write(*,*) '     dist     lat      lon '
           write(6,'(a,i3,a)') '#',k,quarry(k)(1:seiclen(quarry(k)))
         enddo
         write(6,*) 'Number to include in s-file, enter for none'
         read(5,'(a)') text
         if(text.ne.' ') then
            read(text,*) i
            write(read01,'(a80)') data1(1)
            text=' '
            text(1:22)=' Quarry: dist,lat,lon '
            text(22:22+seiclen(quarry(i))-1)=
     *      quarry(i)(1:seiclen(quarry(i)))
            text(80:80)='3'
            write(read01,'(a80)') text(1:80)
            do i=2,nrecord1
              write(read01,'(a80)') data1(i)
            enddo
         endif
         call sei close (close$,read01,code)
         goto 10
...
c
     ...
     *        'nor2nor2.out'
     ...
     &            mailx(1:seiclen(mailx))//" "//
     &            alertemail(k)(1:seiclen(alertemail(k)))//"<"//
c     &            evfile(1:seiclen(evfile)),
     *            'nor2nor2.out',
c     &       seiclen(mailx)+seiclen(alertemail(k))+seiclen(evfile)+2)
     *       seiclen(mailx)+seiclen(alertemail(k))+12+2)
           enddo
c
c   system command if needed
c
           if(email_system.ne.' ') then
              write(6,'(a)') email_system
              call systemc(email_system, seiclen(email_system))
           endif
...
c
c-----------------------------------------------------------------
c    email of summary s-file, also option for a plot
c-----------------------------------------------------------------
c
       if(keys(1:8).eq.'EMAILSUM') then
         write(*,*) ' Sending summary email to the following address: '
         do k=1,n_alertemail_sum
           write(*,'(2x,i2,1x,a)') k,alertemail_sum(k) 
         enddo
         write(*,*) ' Really send summary email (y/n) ?'
         read(5,'(a1)') answer
         if (answer.eq.'y') then
...
c
           ...
           if(email_sum_title.ne.' ') then
              write(121,'(a)') 
     *        email_sum_title(1:seiclen(email_sum_title))
           else
              write(121,'(a)')'Eartquake report'
           endif
           write(121,'(a)')
 ...
 3354         continue
     ...
     *        rea_locality(k:seiclen(rea_locality))
           ...
           do i=1,rea_ncomment
             if(rea_comment(i)(2:9).eq.'FELTINFO') then
               write(121,'(a)') rea_comment(i)(2:79)
               goto 3355
             endif
           enddo
 3355      continue
           ...
           if(email_summary_map.eq.1.0) then
c
c   plot map with epimap
c           
              text='map '//evfile(1:seiclen(evfile))//' l n m'
              write(6,'(a)') text(1:seiclen(text))
              call systemc(text,seiclen(text))
c
c   convert to jpg, output is map.jpg
c
              text='gmt psconvert epimap.eps -Tj -Fmap'
              write(6,*) text
              call systemc(text,seiclen(text))
           endif

           if(email_summary_map.eq.2.0) then
c
c   plot map with mapg
c           
              text='mapg '//evfile(1:seiclen(evfile))//' L m 0.5'
              write(6,'(a)') text(1:seiclen(text))
              call systemc(text,seiclen(text))
c
c   convert to jpg
c
              text='gmt psconvert mapg.ps -Tj -Fmap'
              write(6,*) text
              call systemc(text,seiclen(text))
           endif
 ...
 3356      continue  
           ...
           if(text(1:1).ne.'y') goto 10

           if(n_alertemail_sum.eq.0) then
              write(6,*)'No email defined'
              goto 10
           endif

           do k=1,n_alertemail_sum
              if(email_summary_map.eq.1.or.
     *           email_summary_map.eq.2.0) then   ! with picture
                 long_text=mailx_command_map
     *           (1:seiclen(mailx_command_map))//
     *           ' '//alertemail_sum(k)(1:seiclen(alertemail_sum(k)))
              else
                long_text=
     &          mailx(1:seiclen(mailx))//" "//
     &          alertemail_sum(k)(1:seiclen(alertemail_sum(k)))//"<"//
...
c
c-----------------------------------------------------------------
c   Find geograpical location corresponding to epicenter and 
c   calcualte distance
c-----------------------------------------------------------------
c
       if(keys(1:3).eq.'GEO') then
     ...
     &                      code)                  ! Returned condition.
c
c   read all parameters for one event from file unit write01
c
         call rea_event_in(write01,.true.,data1,code)
c
c  check if location
c
         if(data1(1)(25:37).eq.' ') then
            write(6,*)'No epicenter, cannot find geographical location'
            call sei close (close$,write01,code)
            goto 10
         endif
c
c   find nearest locality, open locality file
c
...
c
c  read file to end and find nearest locality
c
         ns=' '
         x=30000.0
         do i=1,10000
            read(read03,'(a)',end=4787) text
            read(text(1:24),*) elat,elon
            call azibazi(elat,elon,hyp_lat(1),hyp_lon(1),
     *      dist,azim,bazim)

            if(dist*111.2.lt.x) then
               x=dist*111.2
               text1(1:56)=text(25:80)
               if(azim.ge.337.5.or .azim.lt. 22.5) nsw='N '
               if(azim.ge. 22.5.and.azim.lt. 67.5) nsw='NE'  
               if(azim.ge. 67.5.and.azim.lt.112.5) nsw='E '
               if(azim.ge.112.5.and.azim.lt.157.5) nsw='SE'       
               if(azim.ge.157.5.and.azim.lt.202.5) nsw='S '
               if(azim.ge.202.5.and.azim.lt.247.5) nsw='SW'  
               if(azim.ge.247.5.and.azim.lt.292.5) nsw='W '
               if(azim.ge.292.5.and.azim.lt.337.5) nsw='NW'           
            endif
         enddo   
 ...
 4787    continue
         ...
         write(6,'(a)')'Put this text in S-file locality: '
         write(6,*)
         write(6,'(i5,a,a,a,a)') int(x),' km ',nsw,' of ',
     *   text1(1:seiclen(text1))
         write(6,*)
         write(6,'(a)') 'Enter=y, else n'
         read(5,'(a)') text
         if(text(1:1).eq.' '.or.text(1:1).eq.'y') then

            write(rea_locality,'(i5,a,a,a,a)') int(x),' km ',nsw,' of ',
     *      text1(1:seiclen(text1))
            rewind(write01)
            call rea_event_out(write01,.true.,data1,code) 
         endif
 ...
 2733    continue
         if (n_eev_comments.gt.0) then
           ...
           do k=1,n_eev_comments
             write(*,'(i2,1x,a)')
     *       k,eev_comments(k)(1:seiclen(eev_comments(k)))
           enddo
           write(*,'(a)') '------------------------ '
           write(*,'(a)') 'Enter numbers, enter to abort or finish '
           read(5,'(a)') text

           if(text.ne.' ') then
c
c   find numbers entered
c
              do i=1,10
                 read(text,*,err=2734,end=2734) (kkk(k),k=1,i)
              enddo
 2734         continue
              m=i-1
              do i=1,m
                 if(kkk(i).gt.n_eev_comments) then
                    write(6,*)'Comment not defined: ',kkk(i)
                    goto 2733
                 endif
              enddo
           ...
           if (m.gt.0.and.m.le.n_eev_comments) then
...
c             enddo
     ...
     &  eev_comments(kkk(i))(1:seiclen(eev_comments(kkk(i))))
...
c
     ...
     *           // 'event location in degrees (e.g. 10.) '
     ...
     &                       code)                  ! Returned condition.
            ...
            x1=map_lat_c-map_lat
            if(x1.le.-90.0) x1=-88.0
            x2=map_lat_c+ map_lat
            if(x2.ge.90.0) x2=88.0
            write(write01,'(f6.1,1x,f6.1)') x1,x2
            x1=map_lon_c-map_lon
            if(x1.le.-180.0) x1=-180.0
            x2=map_lon_c+map_lon
            if(x2.gt.180.0) x2=180.0
            write(write01,'(f7.1,1x,f7.1)') x1,x2
            write(write01,*)
            write(write01,'(2f7.2)') map_lat_div,map_lat_div
            write(write01,'(2f7.2)') map_lon_div,map_lon_div
            write(write01,*)
...
c
c-----------------------------------------------------------------------        
c  update event id and s-file name                                                         
c-----------------------------------------------------------------------        
c
      if(keys(1:6).eq.'SYNC') then
...
c
         if(index_file) then
            write(6,*)
     *      ' You cannot update ID and S-file name from an index file'
            keys(1:4)='SAME'                                                   
            goto 10
         endif   
...
c
            call get_operator(operator)    
            call systime(p_time,proc_time)   
            write(6,*)'Old S-file name is: ',evfile                 

            call rewrite_sfile
     *      (write01,evfile,base_name(1:5),proc_time,operator,
     *       data1,'US ')
            write(6,*)'New S-file name is: ',evfile
...
c
           write(write01,'(a80)',iostat=code) 
     *          (data1(i),i=1,id)
           write(write01,'(a80)') text   ! old id line
           write(write01,'(a80)',iostat=code) 
     *          (data1(i),i=id+1,nrecord1)
...
c
         if(new_nordic_format) then
            data1(3)( 1:40)=' STAT COM NTLO IPHASE   W HHMM SS.SSS   '
            data1(3)(41:80)='PAR1  PAR2 AGA OPE  AIN  RES W  DIS CAZ7'
         else
...
c
c   if carthesian coordinate system, change 2 values
c
              if(mt_coor(1).eq.'C') then
                mt_val(5,1)=-mt_val(5,1)
                mt_val(6,1)=-mt_val(6,1)
              endif
...
c
              if(mt_nmt.gt.0) then
                  ...
                  write(text(15:22),'(g8.2)') mt_val(2,1)
                  text(23:23)=','
                  write(text(24:31),'(g8.2)') mt_val(3,1)
                  text(32:32)=','
                  write(text(33:40),'(g8.2)') mt_val(1,1)
                  text(41:41)=','
                  write(text(42:49),'(g8.2)') mt_val(5,1)
                  text(50:50)=','
                  write(text(51:58),'(g8.2)') mt_val(4,1)
                  text(59:59)=','
                  write(text(60:67),'(g8.2)') mt_val(6,1)
c
c   remove blanks
c
                  j=15
...
c
c   read s-file  if 'FO'     ! seems not needed jh feb 2019
c
c         if(keys(1:2).eq.'FO') then
c
c   open s-file
c
c            chr_file = evfile(1:fstart+20)
...
c   read all parameters for one event from file unit 1
c
c              call rea_event_in(write01,.true.,data1,code)
c              call sei close (close$,write01,code)
c           endif
...
c
      ...
      if(keys(1:3).eq.'PMR') then
         ...
         WRITE(6,*)'Give mag type: c, L, b, B, W, s or S'
         read(5,'(a)') mag(1:1)
         if(mag.eq.'W') mag_type='SPEC'   
         if(mag.eq.'c') mag_type='END'   
         if(mag.eq.'L') mag_type='IAML'  
         if(mag.eq.'b') mag_type='IAmb'
         if(mag.eq.'B') mag_type='IVmB_BB' 
         if(mag.eq.'s') mag_type='IAMs_20' 
         if(mag.eq.'S') mag_type='IVMs_BB' 
c
c  fish out distance and mag etc
c
         open(137,file='hyp.out',status='unknown')
         call rea_event_in(137,.true.,data1,code)
         close(137)
         open(137,file='hypmag.out',status='unknown')
         write(137,*)
         rewind 137     ! make suere no old data

         do i=1,rea_nphase
            if(rea_phase(i).eq.mag_type.and.rea_mag_res(i).gt.-900) 
     *      then 
            if(rea_phase(i).eq.'SPEC') rea_dist(i)=rea_geo_dist(i)
               write(137,'(a5,1x,a3,1x,a8,2x,4f12.1)') 
     *         rea_stat(i),rea_com(i),rea_phase(i), 
     *         rea_amp(i),rea_per(i),rea_mag_res(i),rea_dist(i)
            endif
         enddo
         close(137)             
c
         call systemc('plotmr',6)
         keys(1:4)='SAME'
         goto 10
      endif
c
c------------------------------------------------------------------------       
c   change event type and or id                                                          
c------------------------------------------------------------------------       
c
 ...
 724     continue
     ...
     *   ' Second character for event ID (e.g. E or P)'
         write(6,*)
     *   ' If second character is _ or b, event ID will be blank'
         write(6,*)' Return for no change ?'
         read(5,'(a2)') new_type
     ...
     *   'R'.and.new_type(1:1).ne.'D') then
         ...
         if(new_type(2:2).eq.'_') new_type(2:2)='B'
c
...
c                                                                               
c   check if type is different for same distance id                                                 
c                                                                               
     ...
     *       (data1(i),i=1,nrecord1)
             ...
             call sei close (close$,write01,code)
              write(6,'(a,a)')
     *        ' Updated file       ', newfile(1:fstart+20)
           else
             write(6,*)' New type same as old, nothing changed'
           endif
...
c
 2155    continue             ! from just above
               ...
               if(new_type(2:2).ne.' ') then
                 if(new_type(2:2).eq.'B'.or.new_type(2:2).eq.'_') then
                   data1(1)(23:23)=' '
                 else
                   data1(1)(23:23)=new_type(2:2)
                 endif
...
c
c   read and write file in case input is old format and new required
c
...
c    &                      b_old,                 ! Already exists? (n/a).
c    &                      code)                  ! Returned condition
c             call indata(write01,nstat1,nphase1,nhead1,
c    *        nrecord1,evid1,exp1,data1,id)
c             rewind(write01)
c             write(write01,'(a80)')(data1(i),i=1,nrecord1)
c             call sei close (close$,write01,code)
c          endif
...
c
         if(id.eq.0) then
            write(6,*)' No valid ID line, fix with command us'
...
c
c------------------------------------------------------------------------       
c  editit hyp.out file                                                                       
c-----------------------------------------------------------------------        
c
      if(keys(1:2).eq.'EH') then
c
c   get editor type
c
         call get_editor(eev_edit)
c        write(6,*) eev_edit
         ...
         txt = eev_edit(1:sei clen(eev_edit)) // ' hyp.out '
...
c  if the channel is not there or it has been deselected, put in zeros, there 
...
c
     ...
     *           ' Full path name    :  ', text(:seiclen(text))
         ...
         if(k.eq.0) 
     *   write(6,'(a,a)')
     * ' No waveform file name or arc-reference in S-file or',
     * ' arc reference  has invalid start time'
         keys(1:4)='SAME'
         goto 10
      endif                                                                     
...
c
c-----------------------------------------------------------------------        
c  get event  from another database                                                           
c-----------------------------------------------------------------------        
      if(keys(1:1).eq.'G') then
         if(keys(2:6).eq.' ') then
            write(6,*)'You must give a data base name like gISC'
            keys(1:4)='SAME'
            goto 10
         endif
         text='eev_get_event '//keys(2:6)//' '//evfile                               
...
c
c-----------------------------------------------------------------------        
c  iloc location program                                                        
c-----------------------------------------------------------------------        
      if(keys(1:5).eq.'ILOC ') then                                                
        if(.not.pc) then
          call systemc('iloc_seisan   '//evfile(1:fstart+20)//' '//
     *    operator,fstart+20+14+5)             ! locate
c
          INQUIRE(file='iloc-out.isf', SIZE=idummy)
          if(idummy.GT.0) then
            write(6,*)''
            write(6,*)'ILOC Solution:'
            open(42,file='iloc-out.isf',status='old')
  40        read(42,'(a)',end=41) txt
            do while (txt(119:122).NE.'ILOC') 
              read(42,'(a)',end=41) txt
            enddo
            write(6,*)'   Date       Time        Err   RMS Lati',
     +      'tude Longitude  Smaj  Smin  Az Depth'
            write(6,*)txt(1:77)
  41        continue
            close(42)
            write(6,*)''
            write(6,*)' Add ILOC location to Sfile ?  [y/n]'
            read(5,'(a)') answer
            if(answer.eq.'y') then
            call systemc(
     +      'cp    iloc_seisan.out '//evfile(1:fstart+20),fstart+20+22)
c add logging
c        Log the iLog location:
              if ( seisan_logging.GE.1 ) then
                call get_operator(operator)
                call create_log_file(evfile,operator)
                call message_to_log_file(evfile,operator,9)
              endif                                                                     
            endif                                                                     
          endif                                                                     
        else
          write(6,*)' ILOC location not available on PC'
        endif
        keys(1:4)='SAME'
        goto 10
      endif                                                                     
...
c
c  remove 7 line and phases so old autophase can run with new format
c
         if(new_nordic_format) then
           open(110,file=evfile,status='old')
           do i=1,10000
              read(110,'(a)',end=6121)data1(i)
              k=i
              if(data1(i)(80:80).eq.'7') goto 6121
           enddo
...
c
         ...
         if(text(1:2).ne.'OK') then
            write(6,*)'Problem with waveform file'
            goto 10
         endif 
c
c  channelinfo from seed.out, for nordic2 format
c
         if(new_nordic_format) then
            wav_filename(1)='seed.out'
c
...
c
c   run autophase, gives old format
c
         txt=' '
     ...
     *   ' -wavefile seed.out'
         ...
         call systemc(txt,seiclen(txt))
c
c   convert format to nordic2 if nordic2 out 
c
         if(new_nordic_format) then
           call get_operator(operator)
           if(agency.eq.' ') agency=seisan_agency
           rea_new_out=.true.          
           txt='nor2nor2 -format 2 -ope '//operator//
     *     ' -aga '//agency//' -file '//evfile(1:seiclen(evfile))
           write(6,'(a)') txt
           call systemc(txt,seiclen(txt))
c
c   copy converted file to data base
c

           open(110,file='nor2nor2.out',status='old',err=6123)
           call rea_event_in(110,.true.,data1,code)
           close(110)
c
c   add info from wav file
c
           do i=1,rea_nphase
             do k=1,wav_nchan
                if(rea_stat(i).eq.wav_stat(k).and.
     *          rea_com(i)(1:1).eq.wav_comp(k)(1:1).and.
     *          rea_com(i)(3:3).eq.wav_comp(k)(4:4)) then
                    rea_com(i)(2:2)=wav_comp(k)(2:2)
                    rea_location(i)=wav_location(k)
                    rea_network(i)=wav_network(k)
                endif
              enddo
           enddo
           open(110,file=evfile,status='old',err=6123)
           call rea_event_out(110,.true.,data1,code)
           close(110)
 6123      continue
         endif
...
c
 ...
 6353    continue
         close(113)
         if(i.gt.3) then
             write(*,*) ' Plot autoratio results (y/n=enter) ? '
             read(5,'(a)') choice
             if (choice.eq.'y') then
                call systemc('plotratio '//evfile(1:seiclen(evfile)),
     *          10+seiclen(evfile))
             endif
         endif
         keys(1:4)='SAME'
...
c
c--------------------------------------------------------------------------
c plotspec, plot results from automag
c--------------------------------------------------------------------------
c
...
c
c send message to waveform_cache to read waveforms for next sfile
c only if not on last event
c
c        write(*,*) ' debug lo ',event_no
        oldno=event_no
        oldfile=evfile
        tkeys=' '
        tkeys(1:4)='NEXT'
        call findevin
     *   (base_name,start_time,end_time,tkeys,from_eev,event_no,
     *   evfile,fstart,new_month,status)
        if (status.eq.0) then
          call systime_msec(msec)
          text=' '
          if (seiclen(evfile).eq.19) then
            k = getcwd(text)
          endif
          udptext=' '
          write(udptext,'(a10,1x,f14.2,a,a1,a)')
     *     'READSFILE ',msec,text(1:seiclen(text)),dchar,evfile
          call send_udp_msg(portcache,udptext)
        endif
c        if (event_no.ne.1) then
          tkeys(1:4)='BACK'
c        else
c          event_no=oldno
c          evfile=oldfile
...
c
c lo 29/04/2010 fixed in get_seisan_message, should not be needed anymore
...
c
      ...
      do i=1,ndata
cxx
c         write(6,*) 'nstat, start ', nstat,data(i)(1:1)
     ...
     *   then
            ...
            do k=1,nstat
              ...
              if(stat(k).eq.data(i)(2:6).and.data(i)(80:80).eq.' ') 
     *        goto 80
            enddo
c
...
cx            nstat=nstat+1
            ...
            if(data(i)(1:1).eq.'s'.and.data(i)(73:79).ne.' ') then
               ...
               if(new_nordic_format) then
                 comp(nstat)=data(i)(7:8)//' '//data(i)(9:9)
               else
                 comp(nstat)=' '
               endif
            else
...
c
               if(change_depth) then
                  do l=1,ndata
                     if(stat(istat)(1:5).eq.data(l)(2:6).
     *               and.data(l)(80:80).eq.' ') then
 ...
 163         continue
         ...
         enddo
         do i=1,ndata
            if(stat(istat)(1:5).eq.data(i)(2:6).
     *      and.data(i)(80:80).eq.' ') then
               read(data(i)(71:75),'(f5.0)') distance
cxx
         write(write05,206,iostat=code) 
cold     *         stat(istat),distance
     *         stat(istat),comp(istat),distance
         call sei code(fort$,code,write05,b_eof)
 206           format
cold     *         (' SYNT: STATION: ',a5,'S  Z'
     *         (' SYNT: STATION: ',a5,a4
...
c
               if(change_depth) then
                  do l=1,ndata
                     if(stat(istat)(1:5).eq.data(l)(2:6).and.
     *                  data(l)(80:80).eq.' ') then
...
c
 ...
 164     continue
         ...
         enddo
         do i=1,ndata
            if(stat(istat)(1:5).eq.data(i)(2:6).and.
     *      data(i)(80:80).eq.' ')then
...
1     continue
      if(operator.eq.'    ') then
         write(6,*) 'Give operator code, max 3 characters'
         read(5,'(a)') operator
         operator(4:4)=' '
         goto 1
      endif
...
c
      mxx=mxx*10.0**13    ! dreger scale and Dyne-cm to Nm
      mxy=mxy*10.0**13    ! dreger scale and Dyne-cm to Nm
      mxz=mxz*10.0**13    ! dreger scale and Dyne-cm to Nm
      myy=myy*10.0**13    ! dreger scale and Dyne-cm to Nm
      myz=myz*10.0**13    ! dreger scale and Dyne-cm to Nm
      mzz=mzz*10.0**13    ! dreger scale and Dyne-cm to Nm
      m0=m0/10.0**7       ! Dyne-cm to Nm
c
c   write mt in psmeca.in file
c
c      open(66,file='psmeca.in',status='unknown',POSITION='APPEND')
c      max=0.0
c      if(abs(mxx).gt.max) max=abs(mxx)
c      if(abs(mxy).gt.max) max=abs(mxy)
c      if(abs(mxz).gt.max) max=abs(mxz)
c      if(abs(myy).gt.max) max=abs(myy)
c      if(abs(myz).gt.max) max=abs(myz)
c      if(abs(mzz).gt.max) max=abs(mzz)
c      k=log10(max)
c      max=10.0**k
c      write(66,'(f8.2,1x,f7.3,a3,6(f6.3,1x),i2,1x,f8.2,1x,f7.3,a7)')
c     *vr,depth," 0 ",
c     *mzz/max,mxx/max,myy/max,mxz/max,-1.*myz/max,-1.*mxy/max,
c     *k-7,vr,depth," SEISAN"           !  -1.*m?? for spherical coordinates
c      close(66)                        !  k-7 is Dyne-cm for psmeca -Sm
c
c   return of only reading parmeters
c
      if(sfile.eq.' ') return
...
c   t or p +180
      ...
      subroutine rewrite_sfile_eev
     *(read01,sfil,base,proc_time,operator,data)
c
...
C  
         if(new_nordic_format) then
cxx            rea_old_format_required=.false.   ! since written with new format
...
C                                                                               
c-- FIND EARLIEST TIME if phase records available, used if no hr min                  
...
cxx      if(new_nordic_format) rea_old_format_required=.true.  ! since now processing
      return                                                                      
      END                                                                       
                                                                                
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      subroutine geo_location_one
...
c
c   it is assumed that the event has been read in rea.inc and only the 
c   locality line in the rea block is modified or added
c
c   the location file will be opened and read for each call
c
c   jh dec 2022
c
c-----------------------------------------------------------------
c
      ...
      character*1 ns,ew            ! directions
      character*2 nsw              ! direction NS, NW etc
      character*80 text,text1
      real elat,elon               ! lat lon
      real dist,azim,bazim         ! distance and azimuth and b azimuth
      real x                       ! help variable
 
      integer i,code,unit,seiclen
...
c
c  read to find nearest locality
c
      ns=' '
      x=30000.0
      do i=1,10000
         read(unit,'(a)',end=10) text
         if(text.eq.' ') goto 10
         read(text(1:24),*) elat,elon
         call azibazi(elat,elon,hyp_lat(1),hyp_lon(1),
     *   dist,azim,bazim)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine geo_location_many(nlocation,lat,lon,location)
c
c-----------------------------------------------------------------
c   Find geograpical location corresponding to epicenter and 
c   calculate distance.
...
c   locality line in the rea block is modified or added
c
c   the localities must be read before the call to the routine 
c
c   jh dec 2022
c-----------------------------------------------------------------
c
      ...
      integer nlocation            ! number of locations
      real lat(*),lon(*)           ! lat lon of locations
      character*56 location(*)     ! name of locations
...
c
c   find nearest locality
c

      ns=' '
      x=30000.0
      do i=1,nlocation
         call azibazi(lat(i),lon(i),hyp_lat(1),hyp_lon(1),
     *   dist,azim,bazim)

         if(dist*111.2.lt.x) then
            x=dist*111.2
            text(1:56)=location(i)
            if(azim.ge.337.5.or .azim.lt. 22.5) nsw='N '
            if(azim.ge. 22.5.and.azim.lt. 67.5) nsw='NE'  
            if(azim.ge. 67.5.and.azim.lt.112.5) nsw='E '
            if(azim.ge.112.5.and.azim.lt.157.5) nsw='SE'       

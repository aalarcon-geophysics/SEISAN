...
c 2020-06-20 lo : small bug fix, number in action 8 was not correct
...
c  action=3 used for events where ID and file name change.
c
c 2015-05-26 pv : first version
c 2020-06-22 pv : added action=3
c
...
c
      ...
      call computer_type(sun,pc,linux)
c     write(6,*)' debug: subroutine move_log_file'
c     write(6,*)' debug: sfile old:',sfile
c     write(6,*)' debug: sfile new:',newfile
...
c
     ...
     &' Event was deleted.'
c
c     if(action.EQ.3) write(write01,'(a,a)') 'The action was:',
c    &' Event ID and file name was updated.'
c     write(write01,'(a)')   'S-file content after action: '
c
...
c  action=8 used for events where ID and file name change.
c  action=9 iloc location added to sfil
c  action=999 dummy
c
c 2015-05-26 pv : first version
c 2020-11-23 pv : added iloc
c
...
c  action=8 used for events where ID and file name change.
        if(action.EQ.8) then
          write(write01,'(a,a)') 'The action was:',
     &    ' ID and file name change'
          write(write01,'(a)')   'S-file content after action: '
          call sei close( close$, write01, code )   ! Close
          call sfile_to_log_file(sfile)
        endif
c iloc location added
        if(action.EQ.9) then
          write(write01,'(a,a)') 'The action was:',
     &    ' iLoc location added to S-file'
          write(write01,'(a)')   'S-file content after action: '
          call sei close( close$, write01, code )   ! Close
          call sfile_to_log_file(sfile)
        endif

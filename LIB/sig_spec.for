...
c 2405 2019   jh: new format, spec_select
c 3004 2020   jh: in spec_value, no response correction was made if
c                 npole and nzero was zero, ca fall 2018 
c 1406 2021?? lo: changed call to four1 as data in that is real 
c 2609 2021   jh: changed the way Q(f) is calculated. before, at 1 hz, Q would be
c                 2Q0 and not Q0 as expected.            
c 1904 2022   jh: when using spec_select and rotate,the north channel was selected
c                 instead of the Z channel when Z was required
...
c                                                                               
c                                                                               
c   selects data for spectral analysis. Given a station and component,
c   the channel is selected from the waveform file and the time of the first
...
c  
      ...
      if(sel_crit.eq.1.or.sel_crit.eq.3) then                                 
         do i=nhead+1,nrecord
            ...
            if(rea_new_in) then
               read(data(i),'(1x,a5,10x,a1)') sta,phasex
            else   
...
c                                                                               
 6    continue
      ...
      if(new_nordic_format) then                                                               
         read(data(i),'(1x,a5,10x,a1,9x,2i2,f7.1)')                                
...
c
      ...
      if(rotate.and.wav_comp(chan)(4:4).ne.'Z') then     ! jh apr 22
...
c
     ...
     *       (q0*freq**qalpha))
              ...
              if(freq.lt.q_below_1hz) then
                 y_com(i)=y_com(i)*exp(pi*freq*travel_time/
     *           (q0*(q_below_1hz)**qalpha))
              else
                 y_com(i)=y_com(i)*exp(pi*freq*travel_time/
     *          (q0*freq**qalpha))
              endif
...
c
c   was like that before sep 2021. result is that q used at
c   1z would be 2Q0, not Q0. Even worse if q_below_iz was smaller than
c   1

c         if(q0.gt.0) then
c           if(q_below_1Hz.eq.0) then ! frequency dependent all freq
c              y_com(i)=y_com(i)*exp(pi*freq*travel_time/
c     *       (q0*freq**qalpha))
c           else ! constat q below q_below_1hz
c             y_com(i)=y_com(i)*exp(pi*freq*travel_time/
c     *      (q0*(1+(freq/q_below_1hz)**qalpha)))
c            endif
c         endif
c
c   correct for kappa
...
c      nsmooth=5
cjh      disp_vel=1
c      if (npole.eq.0.and.nzero.eq.0) then
c        rem_resp=0
c      else
        rem_resp=1
c      endif
...
c lo, this is probably not working June 2021, not sure who wrote this
c
      implicit none
      include 'seidim.inc'
      real y1(*),y2(*),yout(*)
...
c get real of ans, lo
      do i=1,n
        rans(i)=real(ans(i))
      enddo
c      call realft(ans,n,-1)
      call realft(rans,n,-1)
      return
      END
...
c lo changed call to four1 as data in that is real, but this was
c probably not correct as fft1 has two signals ???
      do i=1,n
        rfft1(i)=real(fft1(i))
      enddo
      call four1(rfft1,n,1)
      do i=1,n
        fft1(i)=cmplx(rfft1(i),0.0)
      enddo

...
c   apr 15 20 fh : change max_pz from 50 to 200
c   oct 28 20 jh : changed the RESP reading so files which are not generated
c                  rdseed also can be read, different column positions
c   jan    21 lo : fixed bug in above
c   oct     5 lo : fixed bug in reading of date in seed response files
c
c
...
c       write(6,*)' Response file: ',cal_file(1:60)
          seed_resp=.true.
c          write(*,*) ' debug lo seed response '
          sei_resp=.false.
          sac_resp=.false.
      endif   
c
...
c        read(line(26:),'(i4,1x,i3,1x,i2,1x,i2,1x,f4.1)')
c     &     year,doy,hour,min,sec
c        call dte(doy,day,month,year)
c        seed_start=0.
c        call timsec(year,month,day,hour,min,sec,seed_start)
c read end date and time:
c      elseif (line(1:7).eq.'B052F23') then
c        if (line(26:26).ne.'N') then
c          read(line(26:),'(i4,1x,i3,1x,i2,1x,i2,1x,f4.1)')
c     &      year,doy,hour,min,sec
c          call dte(doy,day,month,year)
c
c          call timsec(year,month,day,hour,min,sec,seed_end)
c        else
c          seed_end=0.
c        endif


        call remove_non_number_chars_resp(120,line)
c changed lo 5/10/2022
        sec=0.0
        min=0 
        hour=0
        doy=1
        read(line,*,end=500)year,doy,hour,min,sec
 500    read(line,*,end=501)year,doy,hour,min
 501    read(line,*,end=502)year,doy,hour
 502    read(line,*,end=503)year,doy
 503    read(line,*)year
...
c     &      year,doy,hour,min,sec
           ...
           hour=0
           doy=1
           read(line,*,end=505)year,doy,hour,min,sec
 505       read(line,*,end=506)year,doy,hour,min
 506       read(line,*,end=507)year,doy,hour
 507       read(line,*,end=508)year,doy
 508       read(line,*)year
           call dte(doy,day,month,year)
...
c
c        write(*,*) seed_start,event_time,seed_end
c        if (event_time.eq.0.) event_time=now    ! case from sys_resp
c        write(*,*) ' debug lo event time seed ',event_time
        if (event_time.ge.seed_start.or.    ! lo 10/09/2019
     &   event_time.eq.0.) then
          if (seed_end.eq.0..or.event_time.lt.seed_end) then
...
c         read(line(52:52),'(a1)') transfer_function_type(stage)

          do i=37,80,1
             if(line(i:i).ne.' ') then 
                transfer_function_type(stage)=line(i:i)
                goto 3330
             endif
          enddo
 3330     continue
...
c      elseif (line(1:7).eq.'B058F03'.and.line(52:52).eq.'0') then

c next part was no longer correct after flex format change, lo
c changed B058F03 part 21/01/2021
c      elseif (line(1:7).eq.'B058F03') then
c             onechar=' '
c             do i=37,80
...
c             enddo
c 3331        continue
c             if(onechar.eq.'0') then
cc        write(*,*) ' debug stage 0 '
c             flag=.false.
c          endif   
c lo 10/9/2019
c      elseif (line(1:7).eq.'B058F03'.and.line(52:52).eq.'1') then
...
c          enddo
c 3332     continue
c          if(onechar.eq.'1') then         
c             stage=1
c          endif
      elseif (line(1:7).eq.
     &   'B058F04'.and..not.flag) then
         call remove_non_number_chars_resp(120,line)
c        read(line(30:),*) overall_sens
         read(line,*) overall_sens
c        write(*,*) ' debug overall_sens = ',overall_sens        
      elseif (line(1:7).eq.'B058F03') then
         call remove_non_number_chars_resp(120,line)
         read(line(52:),*) i
         if (i.eq.0) then
           flag=.false.  ! this will be case for overall sensitivity at end
         elseif (i.eq.1) then
           stage=1   ! this is stage counter
         else
           stage=i     ! lo 10/04/2018
c           if (i.gt.stage) then
             flag=.true.
c           endif
         endif
c         write(*,*) ' debug lo ',i,stage,flag
      elseif (line(1:7).eq.'B053F05'.or.
     &    line(1:7).eq.'B043F06') then
          do i=45,90
             if(line(i:i).ne.' ') goto 3333
          enddo
 3333     continue
          in_units(stage)(1:20)=line(i:i+19)
c         read(line(52:),'(a)') in_units(stage)
      elseif (line(1:7).eq.'B053F06'.or.
     &    line(1:7).eq.'B043F07') then
          do i=45,90,1
              if(line(i:i).eq.' ') goto 3334
          enddo
 3334     continue
          out_units(stage)(1:20)=line(i:i+19)         
c          read(line(52:),'(a3)') out_units(stage)
      elseif (line(1:7).eq.'B053F07'.or.
     &    line(1:7).eq.'B043F08') then
          call remove_non_number_chars_resp(120,line)
c         read(line(52:),*) a0_norm(stage)
          read(line,*) a0_norm(stage)
c         write(*,*) ' debug A0_norm = ',a0_norm(stage)
      elseif (line(1:7).eq.'B053F08'.or.
     &    line(1:7).eq.'B043F09') then
          call remove_non_number_chars_resp(120,line)
c         read(line(52:),*) norm_f(stage)
          read(line,*) norm_f(stage)

c         write(6,*)'norm',stage,norm_f(stage)
...
c         write(*,*) ' debug zero ',stage,seed_np(stage),x,y
...
c        write(*,*) ' debug pole ',seed_np(stage),x,y
c      elseif (line(1:7).eq.'B058F04'.and.flag) then
cB058F04     Gain
       elseif ((line(1:7).eq.'B058F04'.or.
     &         line(1:7).eq.'B048F05').and.flag) then
c     &         line(1:7).eq.'B048F05').and.flag) then
          call remove_non_number_chars_resp(120,line)
c        read(line(52:),*) x
        read(line,*) x
        if (stage.eq.0) then
          write(*,*) ' something wrong, no stages ',line ! lo 10/9/2019
          stop
        endif
        seed_gain(stage)=x
...
c
 ...
 3335    continue
c        read(line(52:52),'(a1)') transfer_function_type(stage)
         transfer_function_type(stage)=onechar
...
c
      goto 555
      write(*,'(a,2f14.1)') 'seed start, seed end ',
     *seed_start,seed_end
      do i=1,stage
         write(6,*) 'stage',i
         write(6,*)'np,nz',seed_np(i),seed_nz(i)
          do j=1,seed_np(i)
            write(6,*)seed_p(i,j)
          enddo
          do j=1,seed_nz(i)
            write(6,*)seed_z(i,j)
          enddo
          write(6,*)'norm,a0_norm',norm_f(i),a0_norm(i)
          write(6,*)'seed_gain',seed_gain(i)
          write(6,*)'in and out units ', in_units(i)(1:8),out_units(1:8)
          write(6,*)'transfer function type ',transfer_function_type(i)
       enddo
 555   continue
c
...
c acceleration, lo 2/10/2013
            write(*,*) ' seed acceleration response, adding 2 zero '
            nzero=nzero+1  
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine remove_non_number_chars_resp(n,t)
c
c   replace non number chars with blanks for n length text string text
c   special to read RESP files
c
      implicit none
      character*120 t
      integer i,n
...
c
     ...
     *  t(i:i).ne.'.')   t(i:i)=' '
      enddo
c
c   check that E is a from a number
c
      do i=1,n-1
        if(t(i:i).eq.'e'.or.t(i:i).eq.'E') then
           if(t(i+1:i+1).ne.'-'.and.t(i+1:i+1).ne.'+') t(i:i)=' '
        endif
      enddo

...
c 2018-09-20 jh: reset the variance  matrix, values were carried 
c                over form one event to the next so vaues from a non
c                fixed depth event wer carried over to a fixed event,
c                where last 2 values should be zero. poited out by
c                fernando  carrilho
c 2020 07 20 jh: 9 char station, look for !nt
c 2020 10 05 jh: bug calculating direct ray when souce just below
c                bondary
c 2020 10 23 jh: AZ to BAZ,  more fix on BAZ
c 2020 11 12 jh: weight BAZ to 4 of app vel is 0
c 2020 11 17 lo: small change in azz part
c 2020 11 18 jh: implicit none
c 2020 11 27 lo: ignore col 53 when reading vapp, some baz extend into col 53 in nnsn database
c 2021 06 14 lo: change in do loop
c
...
c
c   these variables declared after implicit none
c
      integer nmoho,nconrad
      integer iulst              ! output unit
      real tmin
      real delta,dist            ! distance
      real ann
      integer iflag
      real azz0                  ! some azimuth
      real cfact,cthi
      real dd
      real dedeg
      real dellat,dellon
      ...
      real eta1,eta2
      real ea,eee,et,eb    
      real pm,sss,sum,sthi,thi,kkk
      real t                      ! some time
      real td
      real rearth                 ! radius of the earth

      integer i,ii,k,kmin,n2,na,nb,ne1,nep1,nn,n1,ne,ne2
...
c         eta1=eta1+parm(nl+ne1)  ! was like this before oct 5 2020
          eta1=eta1+parm(nl+ne1-1)
         ne1=ne1-1
...
c35    td=td+pm/(parm(i)*sqrt(1.-p*p*parm(i)*parm(i)))
c lo 14/6/2021
        td=td+pm/(parm(i)*sqrt(1.-p*p*parm(i)*parm(i)))
35    continue
      go to 37
38    continue
      if(iulst.gt.0)then
...
c***********************************************************************
          ...
          real alat1,alon1,az,alat2,alon2,dist,alata,alona,
     &    alatb,alonb
          integer nsol

          real degtorad,x1,y1,y2,x2,azz
          real arg,d,dedeg,del,delt,delt1,r
          real theta,dekm
...
c      write(*,*) ' debug lo number of rows ',n
...
c get eigenvalues (diag(a)) and eigenvectors (ve)
      call eigen(a,ve,m,0)
c      do i=1,4
c        write(55,*) ' write debug lo ',i,ve(i)
c      enddo
...
c
      ...
      integer m,n,np,iflagd,i,ii,j,jj,k,ndeg

      real xk,res,oterr,cres,errb,f1,f2,rs,sx,degfb
      real fdist
c      include 'comm1.inc'
...
c           for residuals
      oterr=sqrt(rs)      
c
c   null varience matrix, jh 2018
c
      do i=1,3
         do j=1,3
            var(i,j)=0.0
      ...
      do i=1,m
        ...
        do j=1,np
          ...
          f2=0.0
          sx=ss1(i)
c         write(55,*) ' debug lo ve,eig,alpha ',ve(ii),eig(j),alpha(j)
...
c BRL 6/98: added ips check
      ...
      real a,wt,am
      integer n,ips,i,j,knt,no


      no=n/2+1
      am=a(1)
      do 10 i=1,n
      if(wt(i).le.0.0.or.phase(i)(1:3).eq.'BAZ'.or.ips(i).eq.1)go to 10
      if(a(i).gt.am)am=a(i)
10    continue
      do 50 i=1,n
...
c
      ...
      integer nphase,iazflag,iulst,narriv1
      real coda,xlat,xlon,xmag,oldrms,azz
      real fmp1
      real orsec,tsec,veloc
      integer idy,ihrr,imn,iomin,imnth,iohr,iyr  ! times

      integer i,krecord,icoda
...
20    read(data(krecord),100,end=99,err=166) 
     &st(i)(1:5),pp,prm1(i),phased,
     &ihrr,imn,tsec,icoda,azimuth,veloc,cont
c 100  format(1x,a5,2x,2a1,a8,2i2,f6.1,1x,i4,13x,a5,f5.1,
c     &23x,a1) !nt st(i)(1:5)
clo 27/11/2020 col 52 to be ignored
 100  format(1x,a5,2x,2a1,a8,2i2,f6.1,1x,i4,13x,a5,1x,f4.1,
     &23x,a1) !nt st(i)(1:5)

      st(i)(6:9)=data5(krecord)(81:84)     !nt 
      if(st(i)(1:5).eq.' ') st(i)=' '      !nt 
...
c 10/94: changed position of pp & removed prm3(i)
     ...
     &  ihrr,imn,tsec,icoda,azimuth,veloc,cont !nt
...
c
c   if BAZ phase, read here. if app velocity is zero
c   assume something wrong and weight to zero


      if(phase(i)(1:3).eq.'BAZ') then
          read(azimuth,'(f5.1)') tp(i)
          if(appar_veloc(i).le.0.0) ip(i)=4    ! lo made le
      endif
...
c
c   do not add an extra phase if BAZ already is on a separate line
c   the extra phase can be BAZ or blank, other cases have not been tested
c   jh oct 23, 2020
c           
        if(phase(i)(1:3).eq.'BAZ') then
            read(azimuth,'(f5.1)')tp(i)
c           continue
        else          ! assume BAZ is together with a real phase
           i=i+1
           phase(i)='BAZ     '
           read(azimuth,'(f5.1)')tp(i)
           appar_veloc(i)=appar_veloc(i-1)
c         write(*,*) ' debug lo vapp ',st(i),appar_veloc(i)
           st(i)=st(i-1)
           prm1(i)=prm1(i-1)
           prm2(i)=prm2(i-1)
           prm3(i)=prm3(i-1)
...
c 10/94: set=0 if difference phase
           ...
           if(appar_veloc(i).le.0.0) ip(i)=4    ! lo 
        endif  
c        write(*,*) ' debug lo ',st(i),azz,appar_veloc(i)
c        if(appar_veloc(i).eq.0.0) then
c          ip(i)=4   ! lo moved here 17/11/2020
c        endif
      endif
...
c*******************************************************************
      ...
      real eta,eta1,eta2
      integer n,n1,nn,na,nb,nl
      real ea,eb,dl1,dll
      integer i

      

c      write(6,*) p,eta,n,etal,n1,nn
      nl=(nn+1)/2
      del=0.0d0
      na=n1
      ea=parm(nl+n1)-eta1
...
c        write(6,*)dble(parm(i))*p,1.0d0
         if(dble(parm(i))*p.ge.1.0d0)go to 11
c         if(dble(parm(i))*p.ge.0.90)go to 11
...
c and (modified) bisection function, rtbis, to find the root of qf
      ...
      implicit none
      integer nu1,nu2
      real alpha,f1,f2,facc,fdist,rtbis,qf

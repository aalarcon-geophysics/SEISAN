...
c  29-01-2021 jh   check for ind=0
c  27-09-2021 jh   change eval_q so q=q0 at f=fc and constant below fc
...
c the process is done in these steps: 
c
...
c change lot 05/2006
        if(ind.gt.0) then   ! jh added 29-1-21
...
c then from march 2021  eval_q=q0*(1+(f/fc)**qalpha)  but that
c gave q=2q0 at f=fc. So now Q(fc)=q0*f**fc for f<fc and
c Q0 q0*f**f above fc
c
...
c         eval_q=q0*(1+(f/fc)**qalpha) ! used before version 12.1
       endif

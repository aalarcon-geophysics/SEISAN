...
c     too small value
c   - maximum solutions out fixed to 2000
c   - How it works:
...
c
c  NO IMPLICIT NONE since original program did not have, so be carefull
c
c   changes
c
...
c jul    2016 jh: max solutions increased from 1000 to 2000, vpvs to 1.74, def
c                 bad polarities is 0
c jul 27 2016 jh: change output of amp misfit
c aug 8  2016 jh: amp err def to 0.1
c nov 30 2016 jh: fix so no overflow in av_amp_error if no amplitudes, fix to
c                 get output from composite solutions to hyp.out for easy
c                 plotting, hash_seisan to hash, program name to hash
c dec 20 2016 jh: argeument d will use defaults for all
c nov 20 2017 jh: get vpvs from input file focmec.dat instead of being hardwired. however
c                 if composite solution, or no amplitudes, vpvs is not available
c feb 25 2019 jh: new format, include seisan.inc
c sep 21 2020 jh: hash.out to hash_seisan.out, 
c                 program name to hash_seisan, hash is bash command
c
c      include 'param.inc'
...
c
      parameter (npick0=1000,nmc0=500,nmax0=2000) 
...
c variables for set of acceptable mechanisms, output of HASH subroutines
      ...
      logical all_def                                  ! if true, use all defaults
c
...
c
      ...
      call get_seisan_def      
c
c   make sure no hash.out from before
c
...
c
c   maximum number of output solutions now hardwired to 2000
c
c  
      maxout=2000
c
...
c
      all_def=.false.
      if(arg(1)(1:1).eq.'d') all_def=.true.
c
c   skip questions if all defaults, put in defaults
c
      if(all_def) then
         dang2=2.0
         nmismax=0 
...
c     
      ...
      if(text.eq.' ')then
       ...
       write(6,*) ' Max number of polarity errors, defualt is 0'
       read(5,'(a)') text
       ...
       write(6,*) ' Max average error in amp rat, log10, def 0.1'
       read(5,'(a)') text
       if(text.eq.' ') then
          qmismax=0.1
       else             
 ...
 25   continue                 ! get here if all defaults
...
c
       text='hyp.out'
cxx       call number_of_events(100,text,nevent)
...
c
 ...
 221   continue
       read(12,'(6x,f6.1,f8.1,a1,f8.3,11x,f5.2)',end=222)
     * p_azi_mc(k,1),p_the_mc(k,1),pickpol,sp_ratio(k),vpvs1
       if(vpvs1.gt.0.0) vpvs=vpvs1 ! fish out any one not zero
...
c
c   only use ratio sh to p identified as H in input file, discard sv   ! jh
c
 ...
 222   continue
       ...
       write(6,'(a,f6.2)') ' vpvs ratio is ',vpvs
       if(vpvs.eq.0.0) then
c
c   could be that there were no amplitudes
c
          if(nspr.eq.0) then
             vpvs=1.74
             write(6,*)'vpvse set to 1.74'
          else
             write(6,*)'define vpvs for amplitude data'
             stop
          endif
       endif
...
c     added avrage error
c

      call FOCALAMP_MC(p_azi_mc,p_the_mc,sp_ratio,p_pol,npol,nmc,
     &    dang2,nmax0,nextra,nmismax,qextra,qmismax,nf2,strike2,dip2,
     &    rake2,f1norm,f2norm,nspr,av_amp_error,vpvs)

c
c  jh  a fix to make sure no overflow in av_amp_error, should not
c      happen but it does if no amplitudes

      if(nspr.eq.0) av_amp_error=0.0
...
c
     ...
     *   var_est(1,i),var_est(2,i)
c  jh fix
c         if(av_amp_error.gt.1000.0) av_amp_error=999.0

         write(13,'(a,f5.2)')
     *  'Weighted fraction of pol misfits',mfrac(i)
         write(13,'(a,f5.2)')
c     *  'Average amplitrude error        ',mavg(i)  ! jh not sure why this was used
c

     *  'Average amplitude error         ',av_amp_error
         write(13,'(a,f5.2)')
...
c
c   add to hyp.out,  use first apperance, if hash_seisan.out emply, no solution
c

      rewind(13)
      read(13,'(a)',end=505) text
...
c
     ...
     *      str_avg(1),dip_avg(1),rak_avg(1),var_est(1,1),var_est(2,1)
            write(text(46:50),'(f5.2)') mfrac(1)     ! fit
            write(text(51:55),'(f5.2)') stdr(1)      ! station distribution ratio
            write(text(56:60),'(f5.2)') av_amp_error ! Ratio error
            text(71:80)='HASH     F'
...
c
     ...
     *          'This is a composite solution, number of events is',
     ...
     &      nrecord,type,exp,data,id)
            ...
            do i=2,nhead
c               write(6,'(a)') data(i)
                if(data(i)(71:80).eq.'HASH     F') then
                     ifoc=1
                     data(i)=text
                     goto 1212     ! only replace first
                   endif
             enddo
c            write(6,*) ifoc
             if(ifoc.eq.0) then
...
c
     ...
     &    faults,slips,nspr,av_amp_error,vpvs)
...
c                 sp_ratio=log10(4.9*s_amp/p_amp) ! jh : theoretical ratio
                 sp_ratio=log10(vpvs**3*s_amp/p_amp)   ! jh : different vp/vs compare to above
               end if
             ...
             if (p_pol(ista).ne.0) then   ! jh : only count if a polarity
               ...
               if (ipol.ne.p_pol(ista)) then
                  nmis(irot)=nmis(irot)+1   ! jh , number of misfits this angle                    
               end if
             end if
400         continue                        ! jh , end, station loop
...
615         continue
          ...
          if (sp_ratio(k).ne.0.) then
            s1=cos(2*theta)*cos(phi)  
            s2=-cos(theta)*sin(phi)
c            s2=0.0                     ! jh   null out sv
            s_amp=sqrt(s1*s1+s2*s2)
            sp_rat=log10(vpvs**3*s_amp/p_amp) ! now paramter nov 2017

            mavg=mavg+abs(sp_ratio(k)-sp_rat)

c            write(17,*) k,sp_ratio(k),sp_rat
...
c normal vectors and the normal vectors of each mechanism
      ...
      do 80 i=1,nfault
        ...
        call MECH_ROT(norm1_avg,temp1,norm2_avg,temp2,rota(i))   ! jh july 2016, was rota without index
...
c           ndep       =  number of source depths in table
      ...
      real z(2000),alpha(2000),slow(2000)

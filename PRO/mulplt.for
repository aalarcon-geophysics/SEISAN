...
c sep 21 2018     jh: fix problem with deliting last waveform file name in a list shown in mulplt
c                     when called from eev. increase wav file name to be deleted from 60 to 78.
c                     make sure filenames of 80 chan be used (78 if from eev)
c jan 20 2019     jh: new format  
c jul 26 2019     jh: mulplt_dist option was commented out? why
c aug 06 2019     jh: error when trying to make Y-phases for AZ
c jan 30 2020     jh: remove 'A' as a phase to give AMP, put network and location into
c mar 24 2020     lo: commented out writing of files 'Read headers from files'
c                     single trace mode
c oct 18 2020     jh: do no require rea_old_format_required to be true, operator in
c                     environment, needed for se 
c dec 8 2020      jh: mark picked events in nordic2 format, preserve distance order in
c                     three componenet mode             
c dec 22 2020     jh: fix for y-phase problem
c dec 31 2020     jh: more fix in 3 component mode, some cleaup
c jan 5  2021     Jh. put u function back
c jan 18 2021     pv: fix sorting with new format
c feb  1 2021     jh: fix problem with too many synt phases  in add_synt_readings
c feb  2 2021     lo: set arc_folder in arc channel selection
c feb 05 2021     jh: remove fix from feb 2
c feb 12 2021     jh: clean up and simplify file delete from s-file and filenr.lis
c mar 28 2021     jh: the fix from dec 31, 2020 was replaced by an earlier fix, how? now
c                     back to old version from jan 31 for that part.
c apr 12 2021     jh: rewrite of allchan function
c apr 30 2021     jh: bug in above, implemeted old filter in arc
c jan 21 2021     jh: wsize hardwired to 90 so could no be set in MULPLT.DEF
c                     now commented out
c jul 10 2022     jh: picking phases in multi screen would jump to single trace
c                     at next window
c dec 06 2023     jh: fix one warning about text allocation in plot title
...
c-- ---- new--- after registration                 
      ...
      integer channelname_all_old(max_trace)   ! save expanded selection from stations -----
...
c  temporary arc values
      character*80      arc_folder_temp(3000)             ! folder name
...
c
      ...
      rea_old_format_required=.false.

c
c   get agency for use with phase pick, only new format
c   assume STATION0.HYP
c
      if(new_nordic_format) call get_agency_hyp(' ',pick_agency)
...
c-- postscript hard copy                                          
      ...
      rotate=.false.      ! cfix
c	  
c-- x-window size in % max, set in MULPLT.DEF
c
c     wsize=90.0
c
...
c
      ...
      if( arg(i)(1:13) .eq. '-plotdefault' .or.
     &    arg(i)(1:4)  .eq. '-qdp' ) then
        default=.true.
        default_org=.true.
      endif
      if( arg(i)(1:6) .eq. '-sgram' ) then
         do_spectrogram=.true. 
      endif
      ...
      if( arg(i)(1:9) .eq. '-operator' ) then
         ...
         call put_env_op(operator)  ! for se, jh oc 2020
      endif
...
c           write(*,*) ' debug lo env default ',text
           if(text(1:1).eq.'o'.or.text(1:1).eq.'O') then
             default=.true.
             default_org=.true.
c             write(*,*) ' debug lo env default '
             filt_def=filt
...
c
c   added filter again jh apr30 2021
c
            write(6,*)                                                          
...
c xxx
             default_org=.true.   ! lo March 2020
...
c
                ...
                arc_folder(j)=arc_folder(i)   ! lo 1/2/2021
 2221           continue                
             enddo
...
c
             ...
             do i=1,j
               ...
               arc_folder_temp(i)=arc_folder(i)
             enddo
             arc_nchan=0
c
...
c                   x=-l 
               ...
               if (x.gt.0.and..not.arc_flag_temp(i)) then
                 ...
                 arc_folder(arc_nchan)=arc_folder_temp(i)
c virtual network
               elseif (x.eq.0) then
c                 x=-x
...
c                              
       ...
       enddo
c lo commented out 24 March 2020, not needed?
...
c                                                                               
          ...
          xhead(30:30)=' '
          xhead(31:31+seiclen(wav_file_text))
     *   =wav_file_text(1:seiclen(wav_file_text))
c
...
c         rea_nwav=1
         ...
         nrecord=4
         if(new_nordic_format) then
            call rea_event_in(0,.true.,data,k)
         endif
c
c   write s-file
c
...
c
c  added filter again jh apr 2021
c
      if(arc) then
         flow(9)=flow_def
         fhigh(9)=fhigh_def
...
c  component mode. so all component has already been chosen below
c  at 1106 so comming from there. 
c

c
c   since channels to plot for one or several stations has been select
c   when going inot allcomp mode, skip next section
...
c
 ...
 1103          continue
               if(nchan_all_current.ne.1.and.stat_current.eq.
...
c
c   next staition  (FINI) 
c
           if(choice.eq.'FINI') then
              nchan_all_current=nchan_all_current+1
c
c   if beyond last channel, go to multitrace mode again
c

              if(nchan_all_current.gt.nchan_all) then
...
c
c   check that next channel also is next station
c   else advance channel until a new station. only
c   do if not the first channel
c

 1005         continue                                               
              if(nchan_all_current.gt.1.and.stat_current.eq.wav_stat
     *           (channelname_allc(nchan_all_current))) then
                  nchan_all_current=nchan_all_current+1 
c
c   if beyond last channel, go to multitrace mode again
c
...
c
c   start counting channels for this station
c

           nchan_all_station=1        ! number of channels for one station, start counting

c
c   this is the current channel to plot
c   save it to compare to next stations to make sure it is a new station
c   also use to find other channels for the current  station
c

           stat_current=wav_stat(channelname_allc(nchan_all_current))
           comp_current=wav_comp(channelname_allc(nchan_all_current))
c
c  first channel is the one just active
c         
           channelname(nchan_all_station)=
     *     channelname_allc(nchan_all_current) 
 
c
c   find other channels for this station
c   

           do i=1,wav_nchan        ! check if more channels with same station code
              if(stat_current.eq.wav_stat(i).and.
     *        comp_current.ne.wav_comp(i)) then
                 nchan_all_station=nchan_all_station+1           ! one more channel
                 channelname(nchan_all_station)=i
               endif
           enddo
           nchan=nchan_all_station 

 1007      continue   ! get to here if channel already selcted for all channel mode
                      ! done in case one or several specific stations are selected
c
c   sort to get channels in correct order
c
           call channelsort(nchan)              

           if (fromse.and.port.gt.0.and.nchan.eq.0) then ! exit if from
                                                         ! se and no channels
             udptext='SBMESSAGE No waveform data'
             call send_udp_msg(port,udptext)
             udptext='EXIT'
...
c
         ...
         enddo
c
c check of spectrogram
c
         if (do_spectrogram) then
           choice='SGM'
           flow(9)=2.
           fhigh(9)=0.
           filt=9
...
c
     ...
     *        wav_network(current_chan),
     *        wav_location(current_chan),                           
     +        iswitch)
...
c--------------------------------------------------------------------------
c
c  come here from above if back to multi mode from 3 comp when reaching last channel
c
 1106      continue

           if(choice.eq.'ALLC'.or.
     *     (choice.eq.'FINI'.and.allcomp.and.all_from_single)) then
c     *     (choice.eq.'FINI'.and.allcomp.and.all_from_single).or.
c     *     choice.eq.'ALCC') then
c
...
c
c   go back to old multi channel selection if all components originate in
c   multi channel mode, else go back to channel selection used for
c   single channel mode 
c

c
c   go back to the single channel where all chan mode was called from
c
...
c
c   go back to multi channel mode, restore the channels in multi channels mode
c
                    do l=1,old_nchan
                       channelname(l)=old_chan(l)
                    enddo
                 ...
                 choice=' '              
c
c   was not in all channels mode so start all channel mode
c
              else
                 allcomp=.true.
c
c  next is if more than one station in all channel mode
c
c                 if(choice.eq.'ALCC') allcomp_many=.true.  
c
...
c
c   no channels selcted, use all in view, y has been called without
c   without selecting one or more channels
c
                if(nchan_selected.eq.0) then
                  nchan_selected=nchan
c
c  since options u (ALCC) has not been used, force into loop
c  where you use all stations in view
c
c                  if(.not.choice.eq.'ALCC') choice='FINI' ! force selection 
                  choice='FINI' ! force selection             
                  nchan_all=nchan  
c
c  save selection into array with channels to used in all channels mode
c       
                  do i=1,nchan
                     channelname_allc(i)=channelname(i)
                     channelname_all(i)=channelname(i)
                  enddo
c
c   above nchan_all_current will have one added
c   so zero here to start with no 1
c
              nchan_all_current=0  
...
c
c   if one or more stations selected. find all channels for those
c 
                 nchan_all=0
...
c
c   check if no selected before
c
                          do l=1,nchan_all
                             if(channelname_all(l).eq.i) goto 6655
                          enddo
 ...
 6655                     continue                                                     
               ...
               nchan=nchan_all
c
c   put back in original array
c
                  do i=1,nchan
                    channelname(i)=channelname_all(i)
                  enddo                    
...
c
c  back to plotting, here go directly and  skipping
c  next or back in all channel mode
c
              goto 1007   ! above near 1100
           endif
c
c---------------------------------------------------------------------------
c check if help
...
c changed 22 March 2020 lo
           if(choice.eq.'SGM') then
              if (do_spectrogram) then    ! switch mode off
                do_spectrogram=.false.
                choice='    '      ! clear choice
                goto 1100                   ! replot		   
              else
...
c
     ...
     &              code )         ! Condition not applicable.
                    do i=1,arc_nchan
c                       write(any_unit,'(a5,a3,a2,a2,1x,2f15.3)')
c     *                 arc_stat(i),arc_comp(i),arc_net(i),arc_loc(i),
c     *                 arc_start(i),arc_stop(i)
                       write(any_unit,'(a5,a3,a2,a2)')
...
c
 ...
 5566          continue
c
c  put  theoretical arrival times from location into end of data array
c
...
c
c  put  theoretical arrival times into end of data array
c
              call make_res_file(nrecord,data,nsynt) ! calulate residual
              synt_head=data(1)(1:10)
              call add_synt_readings(nsynt,synt_head)
...
c
c----------------------------------------------------------------
c   check if multi file name delete, then display delete box
c----------------------------------------------------------------
c
           if(choice(1:4).eq.'DEL ') then
               if(rea_nwav.gt.0) then
                  if(rea_nwav.gt.1) then
                     call wav_select_del_new(n_wav_del)
                  else
                     rea_wav(1)=' '
                     n_wav_del=1
                  endif 
...
c     *     WRITE(6,'(a,$)')
c     *  '  SURE YOU WANT TO DELETE WAVEFORM FILE NAME(S) (Y/N) ??????? '        
c           READ(5,'(A1)') answer                                             
...
c
c
c   first check if from s-file
c
                if (opmode.eq.0) then    ! lot 17/05/2006
                  write(6,'(a)')
     *            'Deleting wavform file name(s) from S-file'
                  write(6,'(a)')
     *            'No waveform files on disk are deleted (y/n)?'
                  READ(5,'(A1)') answer                                             
...
c
c   one input file from filenr.lis or individual input
c   
                  write(6,*)
     *            'Sure you want to delete waveform file from disk(y/n)'  
                  READ(5,'(A1)') answer                                             
                  IF(answer.EQ.'Y'.or.answer.eq.'y') then                    
                     if(sun.or.linux) then
                        write(long_text,'(a,a)')'rm ',wav_filename(1)    ! was 1:60 before sep 18, jh
                     else
                        write(long_text,'(a,a)')'del ',wav_filename(1)   ! was 1:60 before sep 18, jh
                     endif
                     write(6,'(a,a)') 'Deleting: ',
     *               wav_filename(1)                                ! was 1:60 before sep 18, jh
                     write(6,*)long_text,seiclen(long_text)
                     call systemc( long_text,seiclen(long_text) )
...
c           endif       ! endif of confirm delete
c
...
c
          ...
          i=0
          do k=1,nrecord
               if (data1(k)(10:10).ne.'Y') then   !   new fix
                 i=i+1
                 data(i) = data1(k)
               endif
           enddo
...
c
 3737         continue           ! from just below
              ...
              if(arc)then
                 ...
                 write(6,*)'No output, just ARC line virtual network: v'
              endif
     ...
     *        question.ne.'a') then
              ...
              if(question.eq.'n'.or.question.eq.'a'.or.question.eq.'v')
     *        no_arc_extract=.true.
...
c
     ...
     *        question.ne.'n'.and.question.ne.'v') then
                 write(any_unit,*) nchan
                 do i=1,nchan
                    write(any_unit,'(i3,a)') channelname(i),' 0.0 0.0'
                 enddo
...
c-----------------------------------------------------------------
             ...
             if(question.eq.'a'.or.question.eq.'v') then
               if (question.eq.'v') then
                 write(*,*) ' give name of virtual network (e.g., _GSN)'
                 read(5,'(a)') qvnet
                 if (seiclen(qvnet).le.0) qvnet='*'
               else
                 qvnet='*'
               endif
c
...
c
c   end of multitrace options, very far back up 
c
      endif
...
c
c   handle default filter for single for first plot
c
      filt_single=0
      if(def_single) then
         flow(9)=flow_def
...
c
c   replace parameters for current channel due to rotation, time
...
c                          
         ...
         head(1:80)=wav_file_text(1:80)
         head(81:85)=wav_stat(ichan)
         head(87:96)=wav_comp(ichan)(1:2)//wav_comp(ichan)(4:4)//' '//
     *   wav_location(ichan)//' '//wav_network(ichan)

         if(wav_rot_comp(ichan).ne.' ') 
     *   head(89:89)=wav_rot_comp(ichan)
         if(rotate) then
            write(head(98:124),'(i4,4i2,f6.3)')wav_out_year(1),
     *      wav_out_month(1),wav_out_day(1),
     *      wav_out_hour(1),
     *      wav_out_min(1),wav_out_sec(1)                 
         else
            write(head(98:124),'(i4,4i2,f6.3)')wav_year(ichan),
...
c udp to se
      ...
      subroutine channelsort(number_of_channels)       
c                                                                               
c                                                                               
c     Routine to sort channels in channelname, sort for stat and  comp.  
c     This is a simplificition of channelselect if only sorting by stat and comp.
c      
c                                                                               
c     written by  j. havskov  dec 2020                                   
c                                                                               
c     Input:   number_of_channels so sort as given in channelsname im common                             
                    
c     Output:  channelname    : sorted channels in common                  
...
c-- channels                             
      integer channelname_work(max_trace)			
c-- number of channels in input channelname                  
...
c
      do i=1,number_of_channels
          station(i)=wav_stat(channelname(i))
          comp(i)=wav_comp(channelname(i))
...
c
c   copy to work array in new order
c                                                      
         do i=1,number_of_channels
            channelname_work(i)=channelname(order_out(i))
         enddo
c
c   copy back to original array
c
         do i=1,number_of_channels
            channelname(i)=channelname_work(i)
         enddo
...
c     Only sorting of data can also be done.            
c                                                                               
...
c
     ...
     *         .and.comp(i)(1:1).eq.data(j)(7:7).
     *          and.comp(i)(4:4).eq.data(j)(9:9))  then   
                  ph(i)='*'
...
c
c   optionally deselect all channels too far, was commented out
c   put back in jh jul 2019 
c
       call mulplt_dist(nchan,station)      	  
...
c
c   select Z for rest of channels
c
...
c         write(*,*) ' pv1: ',i,data(i)
          ...
          if(ldum)then
            if(new_nordic_format)then
              read(data(i)(27:28),'(i2)') hr1
              read(data(i)(29:30),'(i2)') mi1
              read(data(i)(32:37),'(f6.3)') se1
            else
          ...
          if(data2(i)(2:16).EQ."STAT SP IPHASW ") ldum=.TRUE.   ! fix new
          if(data2(i)(2:15).EQ."STAT COM NTLO ") ldum=.TRUE.   ! fix new
        enddo
...
c added lo 8/6/20
         if(data(i)(1:23).eq.'MLHF HIGH CUT AND POLES'.and.
     *      data(i)(41:50).ne.'          ') then
            read(data(i)(41:60),'(2f10.1)',err=99) mlhf_high,xin
            mlhf_high_pole=xin
            mlhf_high_pole=4
         endif
         if(data(i)(1:22).eq.'MLHF LOW CUT AND POLES'.and.
     *      data(i)(41:50).ne.'          ') then
            read(data(i)(41:60),'(2f10.1)',err=99) mlhf_low,xin
            mlhf_low_pole=xin
            mlhf_low_pole=4
         endif
...
c                                    
      implicit none
      include 'seidim.inc'
      include 'rea.inc'     ! due to new format
...
c
      ...
      if(rea_new_out) then
         data(iline)=
     *   ' STAT COM NTLO IPHASE   W HHMM SS.SSS   PAR1  PAR2 AGA'    ! new fix
     *   //' OPE  AIN  RES W  DIS CAZ7'
      else  
         data(iline)(1:57)=
     *   ' STAT SP IPHASW D HRMM SECON CODA AMPLIT PERI AZIMU VELO '               
...
c
c   skip blank påases   jh 8 2019
c
        if (text(34:34).eq.' ') goto 30

        data_out(k)=' '
        data_out(k)(2:6)=text(2:6)

        if(new_nordic_format) then
             rea_stat(max_data/2-k+1)=text(2:6)
             rea_onset(max_data/2-k+1)='Y'
             rea_phase(max_data/2-k+1)=text(34:41) ! output phase
             ...
             if(time.ge.0.0) then
                ...
                rea_hour(max_data/2-k+1)=hour
                rea_min(max_data/2-k+1)=min
                rea_sec(max_data/2-k+1)=sec
            else
                rea_hour(max_data/2-k+1)=hour
                rea_min(max_data/2-k+1)=min
                rea_sec(max_data/2-k+1)=time
            endif
        else
...
c                                                                               
c                                                                               
c     Routine to select continuous databases.
c                                                                               
...
c          call input_box(nfile_page,on,10,89.0,20.0,
c     *                     box_text_local,35,
c     *                     3.0,max_ypos+30.0,title,last_char,1,0)
c lo 22 March 2020
          call input_box(nfile_page,on,10,90.0,15.0,
     *                     box_text_local,35,
     *                     3.0,685.,title,last_char,1,0)
c
c   Quit...
c
         if(last_char.eq.'Q'.or.last_char.eq.'q') then
...
c     written by  j. havskov                                     
c                                                                               
c     Input:   n_wav_file           : number of input file                             
c              wav_filename         : all waveform file names       
c
c     Output:  n_wav_out            : number of wav files selected     
c                                   : selected files
...
c
      ...
      do i=1,n_wav_file
        if(wav_filename(i)(1:1).ne.dchar) then
           k=k+1
           box_text(k)(1:78)=wav_filename(i)(1:78)
        endif
      enddo
c
...
c
      knew=k   ! number of wav files availabel for this event
c
c   display boxes
c
      title=
     *'SELECT FILES FOR DELETION'
      ...
      if(knew.gt.22) then
         call input_box(knew,on,2,650.0,20.0,box_text,78,
     *                    300.0,max_ypos-50.0,title,last_char,0,0)
      else
         call input_box(knew,on,1,650.0,20.0,box_text,78,
     *                    300.0,max_ypos-50.0,title,last_char,0,0)
      endif
c
c   Quit...
...
c
      ...
      do i=1,knew
         if(on(i).eq.1) then
            wav_filename(k)=' '
            wav_filename(k)(1:78)=box_text(i)(1:78)
...
c
 ...
 200       continue
           ...
           if(data(k)(80:80).eq.'6') then
              do l=1,n_wav_out  ! files to delete
                 m=seiclen(wav_filename(l)(1:78))

                 if(wav_filename(l)(1:m).
     *           eq.data(k)(2:m+1)) then 
                    data(k)(1:1)='*'
                 endif
              enddo
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine wav_select_del_new(n_wav_del)                              
c                                                                               
c                                                                               
c     Routine to show wav file names and select filenames to delete
c     and delete  the  names in memory.             
c                                                                               
c     written by  j. havskov, 2021                                     
...
c-- Counters                                               
      ...
      do i=1,rea_nwav
         box_text(i)(1:78)=rea_wav(i)(2:79)
      enddo	  
...
c
      title=
     *'SELECT FILE NAMES FOR DELETION'

      if(rea_nwav.gt.22) then
         call input_box(rea_nwav,on,2,650.0,20.0,box_text,78,
     *                    250.0,max_ypos-50.0,title,last_char,0,0)
      else
         call input_box(rea_nwav,on,1,650.0,20.0,box_text,78,
     *                    250.0,max_ypos-70.0,title,last_char,0,0)
      endif
c
c   Quit...
c
...
c
c  delete selected files from memory
c
      n_wav_del=0
      do i=1,rea_nwav
         if(on(i).eq.1) then
            rea_wav(i)=' '
            n_wav_del=n_wav_del+1
         endif
      enddo                    
...
c
      ...
      integer read_unit
      logical b_flag
      integer code,nstat,nphase,id,i
...
c
c   if new format, always put data in memory
c
c             if(new_nordic_format) then   ! fix new, already in indata
c                rea_nrecord=nrecord
c                rea_nhead=nhead
c                call rea_event_in(0,.true.,data,code)
...
c
      ...
      include 'libsei.inc'
      include  'seisan.inc'
      include 'rea.inc'   ! for new format, to get rea_new_out
      character*80 data2(max_data)
      integer write_unit
      logical b_flag
      integer code,i,nrecord_old,k
      integer seiclen
...
c
c   convert to new format if required     ! fix new, old format needed in mulplt?
c
              do i=1,nrecord        ! save in case new format, so internal format remains old
                data2(i)=data(i)
              enddo

              if(rea_new_out) then
c
c    put in array data in new format, all parameters should be there
c    the array data is now in new format
c 

                 call rea_event_out(0,.true.,data2,code) !  
                 nrecord=rea_nrecord
                 nhead=rea_nhead          
              endif      
              ...
              do i=1,nrecord
                 if(.not.(data2(i)(k:k).eq.'Y'.and.i.gt.nhead))   
     *           write(write_unit,'(a80)',err=10) data2(i)
              enddo
              goto 1
 10           continue
c
...
c
c   put possible syntethic readings into data array
c   put at end of array
c   or for new format
c   put at end of rea variables
c
...
c
c  delete all from data and down, old format
c
...
c
 ...
 1        continue
          read(read1,'(a)',end=99) text1   
          if(text1.eq.' ') goto 99          
          if(new_nordic_format) then        ! iasp.out is in old format
             rea_stat(max_data/2-k)=text1(2:6)
             rea_onset(max_data/2-k)=text1(10:10)
             rea_phase(max_data/2-k)=text1(11:18)
             read(text1(19:29),'(2i2,f6.2)') 
     *       rea_hour(max_data/2-k),rea_min(max_data/2-k),
     *       rea_sec(max_data/2-k)
          else
             data(max_data-k)=text1
          endif
             
          if((k.ge.max_data-300).or.
     *       (k.ge.(max_data/2-300).and.new_nordic_format)) then  ! leave room for at least 300 phase lines
              write(6,*)
...
c   the same day. the synt data is at end of array and is moved up to the data 
c
...
c
c   read and write to remove
c

               do k=1,nsynt
                  rea_stat(rea_nphase+k)=rea_stat(max_data/2-k+1)
                  rea_onset(rea_nphase+k)=rea_onset(max_data/2-k+1)
                  rea_phase(rea_nphase+k)=rea_phase(max_data/2-k+1)
                  rea_hour(rea_nphase+k)=rea_hour(max_data/2-k+1)
                  rea_min(rea_nphase+k)=rea_min(max_data/2-k+1)
                  rea_sec(rea_nphase+k)=rea_sec(max_data/2-k+1)

               enddo
               rea_nphase=rea_nphase+nsynt
            else
               data(nrecord+nsynt)=data(nrecord) ! assuming it is blank
               do k=1,nsynt

...
c   2019 04 09   jh : new format, only include seisan.inc
c   2020 05 08   jh : write out events used
c   2020 11 13   jh : new spectral format
c   2022  8 15   lo : added mag range for relationship
c
...
c
      ...
      real minmag,maxmag     ! ignore magnitues that are below minmag or above maxmag
      character*80 line
...
c
     ...
     *' For spectral values, also possible to chose channel and P or S'
         ...
         write(6,*)'Both station and component must be given'
         write(6,*)'Component is 4 chars, e.g. S  Z, SH Z'
...
c
     ...
     *   spec_stat2,spec_comp2,spec_type2
      ...
      write(6,*)' Give acceptable range of magnitudes for mag relation:'
     &           // ' min,max (return for all)'
      read(5,'(a)') line
      if (line.ne.' ') then
        read(line,*) minmag,maxmag
      endif
c      write(*,*) ' debug lo ',minmag,maxmag
...
c			   
     ...
     *         .and.data(i)(51:54).ne.'    ') then
            ...
            write(6,*)' No new magnitudes generated, must be set up in'
     *                  ,' mag.par'
     ...
     *    ' on coda and/or amplitudes if possible'
            ...
            write(6,*)' No magnitude regression done on coda',
...
c
      ...
      open(15,file='mag_used.out',status='unknown')
c
...
c
      outstring=' '
      do i=2,nhead
        if(data(i)(2:13).eq.'SPEC AVERAGE'.or.
     *  data(i)(2:6).eq.'AV-SD') then
           nspec=nspec+1
           outstring(1:79)=data(1)(1:79)
           outstring(80:80)=' '
           outstring(105:105)=' '
           outstring(106:183)=data(i)(2:79)
        endif
...
c
             do i=1,rea_nphase
                if(spec_stat1(1:5).eq.rea_stat(i)(1:5).
     *          and.spec_comp1.eq.
...
c
             do i=1,rea_nphase
                if(spec_stat2(1:5).eq.rea_stat(i)(1:5).
     *          and.spec_comp2.eq.
...
c-------------------------------------------------------------------
c   case where no agencies are chosen and no specific channels either
c-------------------------------------------------------------------
...
c         write(*,*) ' debug lo ',xm1,xm2,minmag,maxmag
      
          if (xm1.ge.minmag.and.xm1.le.maxmag.and.   ! make sure magnitudes are in acceptable range
     &        xm2.ge.minmag.and.xm2.le.maxmag) then
            nmagrel=nmagrel+1
            mag1(nmagrel)=xm1
            mag2(nmagrel)=xm2
c         write(*,*) ' debug lo ',xm1,xm2,nmagrel,minmag,maxmag
            date(nmagrel)=data(1)(1:15)
c
c   and write out event used
c
            do i=1,nrecord
              write(15,'(a80)') data(i)
            enddo
...
c
         ...
         write(6,*)
         write(6,*) 
     *  'File with events used is mag_used.out'
         write(6,*) 

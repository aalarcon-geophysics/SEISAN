...
c  19-3-2019  jh: delete was never implemented, test with new format
c
      ...
      character*1  xphase(1000)  ! all phases
      ...
      character*23 spec          ! spec amplitude and frequency
      character*80 ratio         ! ratios for time and spectral domain
      ...
      character*13 del(1000)     ! data to delete
      ...
      integer ich,i           
      integer nsignal,nfit ! number of points in data sets              
      character*1 phase
...
c
      open(1,file='plotratio.inp',status='unknown') 
c
...
c         write(*,*) ' debug ',phase
         ...
         read(1,'(a)',end=999) spec
         read(1,'(a)',end=999) ratio
...
c   main title
c     
c2015  8 9 2257  8.0 LQ 66.637  12.883  2.0  BER 46 1.9 3.0LBER 2.9WBER 3.1LNAO   Phase S
         call XCHARS(header(2:79),104,x00,765.0) 
c
c  plot stat comp0
c
      
         text=stat//comp(1:2)//comp(4:4)//' '//phase
         call XCHARS(text,11,x0+xsize/2.0,Y0+ysize-15.0)
...
c
         call xchars(ampper,6,x0+20.0,y0+75.0)
         call xchars(ratio,27,x0+20.0,y0+60.0)
         call XCHARS(spec,23,x0+20.0,Y0+45.0) 
         call XCHARS(flimits,12,x0+20.0,Y0+30.0)
         call xchars(ampper(7:25),19,x0+20.0,y0+15.0)
...
c
         infile=' '
         infile=stat(1:seiclen(stat))//'.'//comp//'.sp.'//phase
...
c
         infile=' '
         infile=stat(1:seiclen(stat))//'.'//comp//'.ga.'//phase
         open(3,file=infile(1:seiclen(infile)),action='read',
     *   status='old')
         i=1
 3       continue
         read(3,*,end=4)xf(i),yf(i)
c         xf(i)=alog10(xf(i))
         xf(i)=xf(i)
         i=i+1
         goto 3
...
c
 100     continue
c
c   read amplitude trace
c

c         if(ampper.eq.' ') goto 200   ! no amplitude trace

         infile=stat(1:seiclen(stat))//'.'//comp//'.td.'//phase
         open(3,file=infile,status='old')
         read(3,*,end=8) nwa,rate,i1,i2 
         i=1
 7       continue
         read(3,*,end=8)x(i)
         i=i+1
...
c   plot signal
c
         call plot_trace(nwa,x,rate,i1,i2,xsize,40.0,x0,y0-40.0)
...
c
         text=
     *'q to quit, f is next'//
     *', d inside frame to delete amplitude and/or spec'
...
c
c   find which channel and if spec or amplitude (wa) 
c
              do i=kplot,iplot
...
c                    del(idel)(10:10)='W'    ! spectra
                    del(idel)(10:13)='AS'//xphase(i)//'G'
     ...
     *           xampper(i).ne.' ') then
                    ...
                    del(idel)(1:5)=xstat(i)
                    del(idel)(6:9)=xcomp(i)
                    del(idel)(10:13)='AT'//xphase(i)//'G'
c                    del(idel)(10:10)='L'    ! amplitude
                    call xmovabs(xpos(i),ypos(i))
...
c
c   check if delete and save, why repeat here?
c
      if(char(ich).eq.'d') then
c
...
c
     ...
     *   'You have marked the following channels/data for delete'
         write(6,*)
         do i=1,idel
            write(6,'(a,2x,a)') del(i)(1:9),del(i)(10:13)
         enddo
         write(6,*)
...
c
      ...
      write(text,'(a,f5.1)') 'Win= ',n/rate
      call XCHARS(text,10,x0+10.,y0+30.)
c
c   plot frame
c
      call xmovabs(x0,y0)
...
c
      ...
      character*13 del(*)                 ! station, comp and phase indicator to delete
...
c----------------------------------------------------
c    
           if(rea_phase(i)(1:4).eq.del(k)(10:13).and.rea_stat(i).eq.
     *     del(k)(1:5).and.rea_comp(i).eq.del(k)(6:9)) then

...
c 2019 02 25 jh: new format, use rea structure
c
      program pinv
c
...
c
      ...
      idat=1
c
c   read file to get format and data in rea structure
c
      call rea_event_in(3,.true.,data,k)
      rewind 3

      if(new_nordic_format) then
         do i=1,rea_nphase
            if(rea_polarity(i).eq.'D'.or.rea_polarity(i).eq.'C'.
     *       and.rea_phase(i)(1:1).eq.'P') then
               if(rea_polarity(i).eq.'C') then
                 cd(idat)=1.0
               else
                 cd(idat)=-1.0
               endif 
               ...
               tak(idat)=rea_ain(i)
c
c   pinv must have ain less than 90
c

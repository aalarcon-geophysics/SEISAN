...
c  2022-04-07  fh : convert slowness in s/deg to apparent velocity in km/s and
c                   vice versa
c  2022-06-13  fh : new format, added seisan.inc and get_seisan_def and compile
c
      ...
      real dist2deg
      real kmperdeg                       ! km per degree along great circle
...
c
  40  continue
      write(*,*) ' converting Nordic to ISF '
c
c-----------------------------------------------------------------
...
c write phases
c
      kmperdeg=3.14159265*6371.*2./360. ! km per degree along great circ
...
c        if (rea_baz_res(i).gt.-999.) azimres=rea_baz_res(i)
          ...
          if (rea_vel(i).gt.0.)
     &    slow=1./rea_vel(i)*kmperdeg ! in Nordic format app-veloc. km/s
          amp=rea_amp(i)
          per=rea_per(i)
...
c          if (azim.ne.ISF_NULL) rea_az(rea_nphase)=azim
          ...
          if (slow.gt.0.) rea_vel(rea_nphase)=1./slow*kmperdeg

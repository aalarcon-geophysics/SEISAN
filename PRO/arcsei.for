...
c sep 24 2020 jh : fix read statement, but canno compile due to missing dirf_pc
c                  commented out
     ...
     &           // ' AND CONVERSION TO SEISAN FORMAT '
      ...
      write(6,*)
     *' Program does not work anymore, routine dirf_pc is missing'
      write(6,*)' Contact Lars Ottemoller if you need this program'
      stop
c
c set defaults
c
...
c
      if (arcfetch_end(1:2).eq.'++') then
        ...
        read(5,*) n_cont_windows
c        read(5,'(i)') n_cont_windows
        if (n_cont_windows.le.0) then
          write(*,*) ' BAD SELECTION '
          STOP
        endif
        read(arcfetch_end(2:),*) cont_length
c        read(arcfetch_end(2:),'(i)') cont_length
...
c
        nfiles=0
c        call dirf_pc('arcsei_temp\\*.rt',files,nfiles)
        do i=1,nfiles
c
...
c         call dirf_pc('arcsei_temp\\*.sei',files,nfiles)
     ...
     &   //files(i)(1:seiclen(files(i)))
         ...
         call systemc(sys_line,seiclen(sys_line))
c         call dirf_pc('*.'//arcsei_code//'*',files,nfiles)

...
c 13 02 2019 jh: new nordic format
c 21 03 2019 jh: del amplitudes AM,AT,AS
c 05 05 2021 jh: del phases from particular stations
c
c
      ...
      integer delp,dels,delsp,delml,delia,delstat,
     *delal,delfps,delam,delbaz,delend    ! flags for delete
...
c---time of making update
      ...
      character*5 statdel(100)            ! stations to delete
      integer nstatdel                    ! number of stations to delete
...
c
 12   continue
      ...
      write(6,'(a,a)')
     *' 1: Lines with P-phase             ',delete(delp+1) 
      write(6,'(a,a)')
     *' 2: Lines with S-phase             ',delete(dels+1)
      write(6,'(a,a)')
     *' 3: Lines with SPEC-phase          ',delete(delsp+1) 
      write(6,'(a,a)')
     *' 4: Lines with IAML-phase          ',delete(delml+1)      
      write(6,'(a,a)')
     *' 5: Lines with IASP-AMP phase      ',delete(delia+1) 
      write(6,'(a,a)')
     *' 6: Lines with AM, AT and AS phase ',delete(delam+1) 
      write(6,'(a,a)')
     *' 7: Lines with END phase           ',delete(delend+1) 
      write(6,'(a,a)')
     *' 8: Lines with BAZ phase           ',delete(delbaz+1) 
      write(6,'(a,a)')
     *' 9: Lines with any phase           ',delete(delal+1) 
      write(6,'(a,a)')
     *'10: Lines with fp solutions        ',delete(delfps+1)
      write(6,'(a,a)')
     *'11: Phases with given stations     ',delete(delstat+1)
      ...
      if(text.ne.' ') then
         ...
         if(ndel.eq.11) then
           write(6,*)'Give stations to delete, terminate with enter'
 15        continue
           read(5,'(a)') text
           if(text.ne.' ') then
              nstatdel=nstatdel+1
              statdel(nstatdel)=text(1:5)
              goto 15
           endif
        endif
...
c
c   delete AM, AT ansd AS-phases
c----------------------------------------------------
c
      if(delam.eq.1) then
         do i=1,rea_nphase
           if(rea_phase(i)(1:2).eq.'AM'.or.rea_phase(i)(1:2).eq.'AT'
     *     .or.rea_phase(i)(1:2).eq.'AS') then
              rea_phase(i)(1:6)='DELETE'
              nd=nd+1
           endif
         enddo
...
c
      if(delend.eq.1) then
         do i=1,rea_nphase
           if(rea_phase(i)(1:3).eq.'END') then
              rea_phase(i)(1:6)='DELETE'
              nd=nd+1
           endif
         enddo
...
c
      if(delbaz.eq.1) then
         do i=1,rea_nphase
           if(rea_phase(i)(1:3).eq.'BAZ') then
              rea_phase(i)(1:6)='DELETE'
              nd=nd+1
           endif
         enddo
...
c
c   delete particular stations from phases, not SPEC phases 
c----------------------------------------------------------
c
      if(delstat.eq.1.and.nstatdel.gt.0) then
         do i=1,rea_nphase
           do k=1,nstatdel
              if(rea_stat(i).eq.statdel(k)) then
                 if(rea_phase(i)(1:4).ne.'SPEC') then
                    rea_phase(i)(1:6)='DELETE'
                    nd=nd+1
                 endif
              endif

...
c  apr 18 22 jh: if same channels but not same time, it could be different segments
c                of data for same channels. so in miniseed they would be read as 
c                one channel. so number of channels in file name must be corrected.
c                make mseed output defualt.
c  aug 12 23 jh: enable output of small numbers (under 1) in seisan format
c
      implicit none
c
...
c-- number of duplicate channels
      integer ndup
c-- number of same channels different time
      integer nsame,ksame 
c-- corrsponding channels
      character*5 same_stat(1000)
      character*4 same_comp(1000) 
      logical same_check              ! true if dup	  
c-- maximum and minumum of above and their difference
      double precision first_time,last_time, total_window	  	  
c-- counters
      integer i,j,k,l,ich,khead,n
...
c
 4646 continue   ! from just below
      write(6,*)' Output format, seisan or mseed (def=enter) ?'
      read(5,'(a)') out_format
      if(out_format.eq.' ') out_format='mseed'
...
c
c   check if this stat comp already has been checked
c
          same_check=.false.
          if(ksame.ge.1) then
            do n=1,ksame
               if(same_stat(n).eq.wav_stat(i).
     *         and.same_comp(n).eq.wav_comp(i))
     *         same_check=.true.
            enddo
          endif
         do j=i,wav_nchan
...
c
     ...
     *    and.i.ne.j.and.wav_abs_time(i).ne.wav_abs_time(j).and.
     *    .not.same_check) then   ! check if duplicate
c
c   save which channel
c
              ksame=ksame+1
              same_stat(ksame)=wav_stat(i)
              same_comp(ksame)=wav_comp(i)
...
c
c
c  if same channels but not same time, it could be different segments
c  of data for same channels. so in miniseed they would be read as 
c  one channel. so number of channels in file name mut be corrected.
c
       if(nsame.gt.0) then 
         write(6,*)
     *  'Number of channels from same channel, different time',nsame
         k=wav_nchan-nsame
         write(outfile(27:29),'(i3.3)')k
         write(6,'(a,a)')
     *   ' Output file name is corrected to: ', outfile(1:29)
         write(6,'(a)')
     *   ' Correction due to same channel, different time'                                             
       endif
c

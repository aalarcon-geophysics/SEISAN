...
c   2020-01-17 jh: add lock file eev.lock
c   2020-06-14 jh: small change to lock 
c   2020-11-27 jh: simplify lock
...
c
      ...
      open(122,file='eev.lock',status='old',err=2323)
c
c   file is there
c
      write(6,*)      
      write(6,*)' **********************************WARNING*********'//
     *'**************************'
      write(6,*)' ** Another EEV user in same directory or EEV'//
...
c
c   lock file not there, make it
c
      open(122,file='eev.lock',status='new')
      write(122,*)'eev lock file'
      close(122)
...
c read waveforms in background
      udptext=' '
...
c
         if(event_no.eq.0.and.status.eq.3) goto 999
         if(status.eq.9) goto 999   ! something wrong
      ENDIF                                                                     
c
c
c-------------------------------------------------------                  
...
c
 ...
 1818 continue
      ...
      read(5,'(a)') text
      if(text(1:1).eq.'q'.or.text(1:1).eq.'Q') goto 999
 1820 continue
...
c       
     ...
     *eq.'quit'.or.keys(1:4).eq.'QUIT') goto 999                                             
c
...
c
 999  continue
      ...
      open(122,file='eev.lock')
      close(122,status='delete')   ! delete lock file
      stop
      end

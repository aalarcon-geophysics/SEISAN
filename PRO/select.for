...
c                  distance range
c sep 30 2017  jh: eventtype Q will now only select events marked bY Q, not blanks as 
c                  before. If only blanks are to be select, use B (new)
c nov 20 2018  jh: fix to get componets 1 and 2 in codaq output file, fixed for 3
c                  paticular stations, see portugal
c dec 20 2018  jh  replace search for a string in waveform file name lines with a search
c                  for a general text string in whole s-file. 
c mar 1  2019  jh: new nordic format
c xxx    2019  xx: somebody added to selcrit_prompt??
c jan 13 2020  jh: bug with reading from file, def endtime to 2025
c feb 05 2020  jh: add option to select moment tensors, bug with distance selection
c                  if blank distance on phase line
c feb 13 2020  jh: enable search for 8 char phases in both new and old format
c sep 18 2020  jh: search for errors was wrong. if a blank error, it was read as 0.0.
c                  this cannot be the case if written by seisan, but from otheres it
c                  is possible. same with gap. also all E lines were searched, now 
c                  only the first.
c dec 8  2020  Jh: allow for multiple E-lines, only use the one associated with prime 
c                  hypocenter.
c mar 4  2021  jh: make sfiles in default instead of cat files
c may 10 2021  jh: problem with fps selection togeter with other parameters
c aug 20 2021  jh: select only events with no loaction
c sep 29 2021  jh: initialize fpquality and volcano types
c jan 14 2022  jh: fix selection screen so lines not too long from phases and
c                  volcanic subclasses
c mar 27 2023  jh: bug when selcting polarities in new format, fix dim warning
c apr 14 2023  lo: add option 26 to select events within distance from point
c apr 18 2023  jh: increase dim of statselect etc
...
C  STATIONS CODES TO SELCT BY
      CHARACTER*7	STATSELECT(150)
      character*8       stat_phase(150)  ! phases to select with stations
      integer mindist(150),maxdist(150)   ! distance range for station 
      logical hyp_dist                  ! if true, use hypocentral distance
      character*1500 text1500           ! long text for codaq output
...
c  logical to indicate fault plane solution found
      logical fpsolutf
c  logical to indicate moment tensor solution found
      logical mom_tensorf
...
c  logical to indicate hypocenter error depth found
      logical hedepthf
c  logical to check for distance from point, and variables
      logical ldistfp
      real distfp(3)
...
c   text
      character*80 text
      logical case               ! true if case independent search
c   dir name for EPI now cat
      character*80 dir
c   fault plane solution and moment tensor solution
      logical fpsolut,mom_tensor
c   earthquake felt
      logical eqfelt
      character*1 lcase          ! lower case function
      logical header_check       ! if or not all header lines checked
      logical wave               ! true if correct wave form file name found
      character*84 text_check    ! string to identify text in s-file 
      ...
      character*8 phase(6)       ! phase names to select
      ...
      integer k1,k2,k3           ! position of phase and polarity on phase line
      integer kk
...
C
     ...
     *                minhedepth,maxhedepth,ypol,iant,mnupol
      common/epicrit2/fpsolut,mom_tensor,eqfelt,statselect
...
c
 ...
 384        continue
            write(6,*) ' Type of base: CAT (c) or Sfiles (s) (enter):'
            read(5,'(a)') text
            if(text(1:1).eq.'s'.or.text.eq.' ') 
     *      basetype='SFILES'
            if(text(1:1).eq.'c') 
     *      basetype(1:3)='CAT'
            if(text(1:1).ne.' '.and.text(1:1).ne.'c'.
     *      and.text(1:1).ne.'s') goto 384
         endif
      endif
C
...
C
     ...
     &                    b_flag )                   ! Flag end of file.
            ...
            nrecord=0 ! do not accept blank lines in phases
...
C
c
c   position of phase on phase line
c
      k1=11       
      k2=14
...
C
      ...
      if(header_check) then    ! find if all headers are checked
...
c
      ...
      if(mingap.ne.0.0.or.maxgap.ne.360.0) then
         ...
         if(.not.compact) then
c            do i=1, nhead
c              if(data(i)(80:80).eq.'E'.and.data(i)(2:4).eq.'GAP') 
c     *           then
c                 if(data(i)(6:8).ne.' ')read(data(i)(6:8),'(i3)') igap   ! do not use if blank jh sep 2020
c                 if(float(igap).le.maxgap.and.float(igap).ge.mingap)
c     *           accepted=.true.
c                 goto 3333   ! only first errror line
c              endif
c            enddo
c 3333       continue

             if(hyp_gap(1).ge.0.0.
     *       and.hyp_gap(1).ge.mingap.and.hyp_gap(1).le.maxgap) 
     *       accepted=.true.
         endif
      endif
      endif
...
c
     ...
     *        (data(j)(k1:k1).eq.'P'.or.data(j)(k1:k1).eq.'S')) then
c
...
C
      ...
      kk=8   ! initially assume 8 char phase
      ...
      IF(.not.compact.AND.STAT_present.GT.0) THEN
         ...
         if(nstat.ge.0) then
            DO I=1,STAT_PRESENT
                DO J=NHEAD+1,NRECORD-1
                   if(new_nordic_format) then
                      stationcode=data(j)(2:7)//data(j)(9:9)  ! only use 2 chars comp
                   else
...
c
c    if distance is required but phase has no distance, skip for all following cases
c
                   if(maxdist(i).lt.99999.and.dist.eq. -1.0) goto 1616

                   if(data(j)(k1+4:k1+7).eq.' ')kk=4   ! 4 chars or less for phase, 
                                                       ! will cover old format with W or D
...
c
     ...
     *                  .or.maxdist(i).eq.99999).
     *                 and.((data(j)(k1:k1+kk-1).
     *                 eq.stat_phase(i)(1:kk).and.
     *                 stat_phase(i).ne.' ').or.(stat_phase(i)(1:4).eq.
     *                 '____'.and.data(j)(k1:k1+3).eq.' ')
...
c
     ...
     *                 ((dist.ge.mindist(i).and.dist.le.maxdist(i))
     *                  .or.maxdist(i).eq.99999).             ! no selection by distance
     *                 and.((data(j)(k1:k1+kk-1).eq.
     *                 stat_phase(i)(1:kk).and.
     *                 stat_phase(i).ne.' ').or.(stat_phase(i)(1:4).eq.
     *                 '____'.and.data(j)(k1:k2).eq.' ')
     *                 .or.stat_phase(i).eq.' ')) then
...
c--------------------------------------------------
       ...
       if (accepted) then
          if (.not. compact .and. mnupol .gt. 0) then
              cnupol = 0
              k3=17
              if(new_nordic_format) k3=44

              do j = 1,nrecord
                 if ((data(j) (80:80) .eq. ' ' .or. 
     *               data(j) (80:80) .eq. '4').and.
     *               data(j)(k1:k1).eq.'P') then
                     if (data(j) (k3:k3) .eq. 'C' .or.
     *                   data(j) (k3:k3) .eq. 'D') 
     *                   cnupol = cnupol + 1
                 end if
              end do
...
c
        ...
        kk=8    ! assume 8 chars phase possible
        if (accepted) then
           if (phase(1).ne.' ') then
              do j = nhead+1,nrecord-1
                  do k=1,6
                     if(data(j)(k1+4:k1+7).eq.' ')kk=4   ! 4 chars or less for phase, will cover old format with W or D
                     if((data(j)(k1:k1+kk-1).eq.phase(k)(1:kk).and.
     *               phase(k).ne.' ').or.
     *               (data(j)(k1:k1+3).eq.' '.
     *                and.phase(k)(1:4).eq.'____'))
     *               goto 3746
                   enddo
              end do
...
c              do j = 1,nrecord
c                 if (data(j) (80:80) .eq. 'E' .or.
c     *               data(j) (80:80) .eq. 'e') then
c                     filevar=-1
c                     if(data(j)(24:30).ne.' ') ! no error, do not use, jh sep 2000
c     *               read (data(j) (24:30),'(f7.0)') filevar
c                     if (filevar .ge. minhelat .and.
c     *                   filevar .le. maxhelat) then
c                         helatf = .true.
c                         goto 2221
c                     end if
c                 end if
...
c 2221         continue
              ...
              if(hyp_lat_err(1).ge.0.and.hyp_lat_err(1).ge.minhelat.
     *        and.hyp_lat_err(1).le.maxhelat) helatf=.true.
...
c     *               data(j) (80:80) .eq. 'e') then
c                     filevar=-1
c                     if(data(j)(31:38).ne.' ') ! no error, do not use, jh sep 2000
c     *               read (data(j) (31:38),'(f8.0)') filevar
c                     if (filevar .ge. minhelon .and.
c     *                   filevar .le. maxhelon) then
c                         helonf = .true.
c                         goto 2222  ! only first error line
c                     end if
c                 end if
c              end do
c 2222         continue

              if(hyp_lon_err(1).ge.0.and.hyp_lon_err(1).ge.minhelon.
     *        and.hyp_lon_err(1).le.maxhelon) helonf=.true.
...
c     *               data(j) (80:80) .eq. 'e') then
c                     filevar=-1.0
c                     if(data(j)(39:43).ne.' ')   ! no error, do not use, jh sep 2000
c     *               read (data(j) (39:43),'(f5.0)') filevar
c                     if (filevar .ge. minhedepth .and.
c     *                   filevar .le. maxhedepth) then
c                         hedepthf = .true.
c                         goto 2223
c                     end if
c                 end if
...
c 2223         continue
              ...
              if(hyp_depth_err(1).ge.0.and.hyp_depth_err(1).
     *        ge.minhedepth.
     *        and.hyp_depth_err(1).le.maxhedepth) hedepthf=.true.
...
c
c   moment tensor solution
c

      if (accepted) then
          if (.not. compact .and. mom_tensor) then
              do j = 1,nrecord
                 if (data(j) (80:80) .eq. 'M') then
                    mom_tensorf=.true.
                    goto 4444
                 endif
c
c   since at least one moment tensor found, do not check more
c

              enddo
 4444         continue
              if (.not. mom_tensorf) accepted = .false.
          end if
      end if
c
c   felt earthquake
...
c
      if (accepted) then
          if (.not.compact.and.text_check.ne.' ') then
              wave=.false.
              k=sei clen(text_check)
              m=k
c
c   check if case independent, in that case convert search string
c   and s-file line string to lower chars before comparing
c
              case=.false.
              if(k.gt.4) then
                 if(text_check(k-3:k).eq.'case') then
                    do i=1,k
                       text_check(i:i)=lcase(text_check(i:i))
                    enddo
                    case=.true.
                 endif
              endif
              do j = 1,nrecord                
                  read(data(j)(1:80),'(a)') text  
                  if(case) then
                     do i=1,80
                        text(i:i)=lcase(text(i:i))
                     enddo
                     m=k-4
                  endif 
                  do i=1,sei clen(text)-m+1
                    if(text(i:i+m-1).eq.text_check(1:m))wave=.true.
                  enddo
              end do
...
c                       if(statselect(1)(7:7).eq.'A') 
c     *                 write(90,'(200a15)')
c     *                                    ('   Z    N    E ',i=1,ncoda)


c
c   case of all components
c
                       text1500=' '
                       if(statselect(1)(7:7).eq.'A') then
c
c   fix for portugal obs to get component 1 and 2 instead of N and E
c
c
c   write to long string since not all stations might be obs's 
c
c   the obs stations are ob.., ox.. or NRT
c
                              m=1
                              do k=1,ncoda
c
c   for other obs stations, replace stations here
c
                                if(coda_stat(k)(1:2).eq.'ob'.or.
     *                         coda_stat(k)(1:2).eq.'ox'.or.coda_stat(k)
     *                          (1:3).eq.'NRT') then
                                   write(text1500(m:m+14),'(a15)')
     *                             '   Z    1    2 '
                                else
                                  write(text1500(m:m+14),'(a15)')
     *                             '   Z    N    E '
...
c
     ...
     &               code )                ! Returned condition (n/a).
      ...
      read(w1,'(31x,8x,f8.3,1x,f7.3,1x,f5.1)') 
     &   distfp(1),distfp(2),distfp(3)
      if (distfp(3).gt.0.) ldistfp=.true.
      read(w1,'(31x,14x,l1)')mom_tensor 
      read(w1,'(31x,14x,l1)')header_check 
      read(w1,'(31x,1x,a)')  text_check
      read(w1,'(31x,f15.3)') mingap
      read(w1,'(31x,f15.3)') maxgap
      read(w1,'(31x,1x,6a8)')   phase
...
c
c   if start time has no year, put in 1980
c
       if(starttime(1:4).eq.' ') then
          starttime(1:4)='1980'
          if(endtime(1:4).eq.' ') endtime(1:4)='2025'
       endif
c
...
CC2000   if(cen1.lt.19) then
            stime=starttime
c            stime(1:2)='19'      ! make sure calculation goes from 1900,
...
c   add century
c
c         call abstim('19991231235960',0,timcen)  ! time of one century
c         time1=time1+cen1*timcen
c         time2=time2+cen2*timcen
...
c-if enter, default is choosen
     ...
     *    option(1:3) .eq. '23 ' .or. option(1:3) .eq. '24 ' .or.
     *    option(1:3). eq. '25 '.or. option(1:3) .eq. '26 ') then
         call draw_screen(option)
     ...
     *         option(1:3) .ne. '24'  .and.
     *         option(1:3) .ne. '25'  .and.
     *         option(1:3) .ne. '26'       ) then
           option(1:1) = '?'
         call draw_screen(option)
      end if    
      end do
...
c      fpsolut = .false.
     ...
     *fpquality,fpsolut 
      write(w1,'(a,8x,f8.3,1x,f7.3,1x,f5.1)') 
     &  ' Distance from point          :',distfp(1),distfp(2),distfp(3)
      write(w1,'(a,14x,l1)') ' Momet tensor solution        :',
     *mom_tensor 
      write(w1,'(a,14x,l1)') ' Check all header lines       :',
     *header_check 
      write(w1,'(a,1x,a)')   ' Search for text string       :',
     *text_check
      write(w1,'(a,f15.3)')  ' Minimum gap                  :',
     *mingap               != 0
      write(w1,'(a,f15.3)')  ' Maximum gap                  :',
     *maxgap               != 999.0
      write(w1,'(a,1x,6a8)') ' Phases                       :',
     *phase
     ...
     * all_stat,hyp_dist
      if(stat_present.gt.0) then
         do i=1,stat_present
           write(w1,'(1x,a7,2i7,1x,a8)')statselect(i),mindist(i),
...
c
      ...
      logical fpsolut,mom_tensor  ! fault plane solution and moment tensor 
      ...
      character*7 statselect(150)  ! station code
      character*8 stat_phase(150)  ! phase to select with station
      integer mindist(150),maxdist(150)  ! distance range for station	
     ...
     *        inde2, inde3        ! aux index
      ...
      character*1 mom_tensorin    ! ------------ moment tensor
...
c-formats used in the subroutine
 ...
 905  format ('  Event ID. Any combinations of E, B, P, V, S, Q, E.G. '
     *        ,'QE for Quakes and Explosions.'
     *       /,'  Return for all types, B for only blanks.',
 ...
 908  format ('  Lower Latitude Limit, 99 for no location only,'//
     *           ' return for default: ',$)
...
c-checking if option equal 1, fault plane solution

c         fpsolut = .false.  ! was here before may10 2021?
c         fpquality='     '

         if (option(1:3) .eq. '1  ') then
c-writing message and reading value for fault plane solution

             fpsolut = .false.  ! was commented out here
             fpquality='     '  ! ----------------------
...
c check if option 26
         else if (option(1:3) .eq. '26 ') then
             write(*,*) ' enter lat,lon,dist '
             read(5,*) distfp(1),distfp(2),distfp(3)
             ldistfp=.true.

c-checking if option equal 25
         else if (option(1:3) .eq. '25 ') then
c-writing message and reading value for moment tensor
             mom_tensor = .false.
             write (6,*) ' '
             write (6,962)
             read (5,'(a)') reading
             call aligncha(reading)
             mom_tensorin = reading
             if (mom_tensorin .eq. 'y' .or. mom_tensorin .eq. 'Y') then
                 mom_tensor = .true.
                 stcodes = 0
             else if (mom_tensorin .eq. 'n' .or. mom_tensorin .eq. 'N')
     *       then
                 stcodes = 0
             else 
                 stcodes = 2
...
c                       magtypes(ind) = 'S'
c commented the following out as both exist, lo 6 oct 2022
c                    else if (magtypes(ind) .eq. 'w') then
c                        magtypes(ind) = 'W'
                    end if
                end if
             end do
...
c                       eventtype(ind) = 'S'
                        ...
                        eventtype(ind) = 'Q'
                   else if (eventtype(ind) .eq. 'b') then
                        eventtype(ind) = 'B'
                    end if
c               end if
             end do
...
c-writing message and reading value for minimum latitude
             write (6,*) ' '
             write (6,'(a,$)')
     *           '  Lower Latitude Limit, 99 for no location only,'//
     *           ' return for default: '
             read (5,'(a)') reading
             call alignnum(reading)
...
c-writing message and reading value for maximum latitude
             if(lolat.ne.99.0) then  ! do not ask if only events with no location
             write (6,*) ' '
             write (6,909)
             read (5,'(a)') reading
             call alignnum(reading)
...
c-writing main screen. When default, value is not printed
     ...
     *            stat_present .le. 30) then
         ...
         if(text_check.ne.' ') then                                
            write(6,'(6x,a,a,7x,a)') '21  - Search for text string',
     *      ' in S-file ',text_check(1:seiclen(text_check))
         else
            write(6,'(6x,a,a,7x)')    '21  - Search for text string ',
     *      'in S-file'
         endif
     ...
     *      '     '
         ...
         write(text,
     *  '(6x,a,a,7x,6a8)')   '23  - Phases                 ',
     *      '     ',phase
         write(6,'(a)') text(1:seiclen(text))
         write(text,
     *  '(6x,a,a,7x,10a6)')   '24  - Volcanic subclasses    ',
     *      '     ',volcanictype
         write(6,'(a)') text(1:seiclen(text))
           if (mom_tensor) then
             write (6,'(6x,a,21x,a)') '25   - Moment tensor solution  ',
     *                                ' Yes'
         else
             write (6,'(6x,a)') '25  - Moment tensor solution'
           if (ldistfp) then
             write (6,'(6x,a,f8.3,1x,f7.3,1x,f5.1)') 
     &       '26  - Distance from point ',distfp(1),distfp(2),distfp(3)
           else
             write (6,'(6x,a)') '26  - Distance from point'
           endif
         end if
...
c
c      if(century.eq.0) century=19
c      year=year+1900
      call timsec(year,month,day,hour,minutes,sec,time)                        
c      time=time+century*timcen
...
C
 ...
 50   CONTINUE
C
C   SELECT ON EVENTTYPE, A BLANK IN NORDIC FILE MEANS normally EARTHQUAKE,
C   a confirmed earthquake is  Q IN SELECTION CRITERIAS. IF EVENTTYPE(1)
C   IS BLANK, ALL EVENT TYPES ARE USED
c
c   if eventtype(1) is '_', is same as blank
C
      if(eventtype(1).eq.'_') eventtype(1)=' '
c      write(*,*) ' debug eventtype(1) =',eventtype(1)
	  
      IF(EVENTTYPE(1).NE.' ') THEN
	     DO I=1,5
                    if(eventid.eq.' '.and.eventtype(i).eq.'B') goto 55    ! select only blank
		    IF(EVENTID.EQ.EVENTTYPE(I)) GOTO 55
c            IF(EVENTID.EQ.' '.AND.EVENTTYPE(I).EQ.'Q') 
c     *      GOTO 55
         ENDDO
         GOTO 99
      ENDIF
...
c
      if (string(24:30).eq.'       ') then
          if(lolat.eq.99.0) goto 111    ! select events with no location
c changed lo 08 March 2001 for seisweb
 ...
 111  continue
c
c check for distance from point, lo 11042023
c
c event coordinates are lati and long
c      write(*,*)  ' debug lo ldistfp ',ldistfp
      if (ldistfp) then
        call delaz(lati*pi/180.,long*pi/180.,
     &    dekm,dedeg,az,distfp(1)*pi/180.,distfp(2)*pi/180.)
c        write(*,*) dekm,lati,long,distfp(1),distfp(2)
        if (dekm.gt.distfp(3)) goto 99
      endif
c

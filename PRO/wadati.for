...
c  apr 19 03  jh : as pointed out by fernando carrilho, weiting was not used properly
c                  for s in wadati mode
c  apr 09 19  jh : new format, a few changes
c                                                                               
c
      implicit none
...
c 
        ...
        if(data(i)(17:17).eq.'P'.and.iw.le.min_weight) then  ! if valid P
           ...
           do k=nhead+1,nrecord
             read(data(k)(25:25),'(i1)') iw     ! read assigned weight, i to k, apr 03             
             if(data(k)(17:18).eq.phase.and.data(k)(2:6).eq.stat.

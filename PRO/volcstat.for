...
c   20190529 jh  - new format, only seisan.def
c   20210917 rcs - works with GMT 6.1.1
c
      implicit none
      include 'seidim.inc'
      include 'seisan.inc'
...
c
     ...
     &                    code)                  ! Returned condition.
      ...
      gmt_text=' '
C RCS  -----------------------------------------
C      gmt_text='gmtset FONT_ANNOT Helvetica,black'
      gmt_text='gmtset FONT_ANNOT_PRIMARY 10p,Helvetica,black'
      call systemc(gmt_text,seiclen(gmt_text))
      write(write03,*) gmt_text(1:seiclen(gmt_text))
C RCS  -----------------------------------------
C      gmt_text=' '
C      gmt_text='gmtset ANOT_FONT_SIZE 10p'
C      call systemc(gmt_text,seiclen(gmt_text))
C      write(write03,*) gmt_text(1:seiclen(gmt_text))
      gmt_text=' '
C RCS  -----------------------------------------
C      gmt_text='gmtset LABEL_FONT Helvetica'
      gmt_text='gmtset FONT_LABEL 10p,Helvetica,black'
      call systemc(gmt_text,seiclen(gmt_text))
      write(write03,*) gmt_text(1:seiclen(gmt_text))
C RCS  -----------------------------------------
C      gmt_text=' '
C      gmt_text='gmtset LABEL_FONT_SIZE 10p'
C      call systemc(gmt_text,seiclen(gmt_text))
C      write(write03,*) gmt_text(1:seiclen(gmt_text))
      gmt_text=' '
...
c
          gmt_text=' '
C RCS  -----------------------------------------
C          gmt_text='gmtset D_FORMAT %lg'
          gmt_text='gmtset FORMAT_FLOAT_OUT %lg'
          call systemc(gmt_text,seiclen(gmt_text))

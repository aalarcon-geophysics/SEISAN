...
c   may  7 20 jh  : also select by agency, use all header lines, only
c                   use first instance 
c   aug 24  20 jh : use negative magnitudes
c --------------------------------------------
...
c  number of magnitudes each interval, cumulative and log of same
      ...
      character*3 agency          ! desired agency of magnitude
      character*3 aga(3)          ! mag agency
...
c      if(magtype.eq.'s') magtype='S'
      ...
      write(6,*)' Magnitude agency, enter for any'
      read(5,'(a)') agency
c
...
c      
      if(compact) then
         read(data(1),'(55x,3(f4.1,a1,a3))')  ! neg mag
     *   (mg(i),mtyp(i),aga(i),i=1,3)
c 3       continue

         do 2 i=1,3
            if(magtype.eq.mtyp(i).and.   ! neg mag
     *        (aga(i).eq.agency.or.agency.eq.' ')) then
              nm=nm+1
              mag(nm)=mg(i)
...
c
              ...
              xyear=year+(month-1)/12.0+(day-1)/
     *        365.0+(hour-1)/(24.0*365.0)
...
c
         ...
         do  i=1,rea_nmag
            if(magtype.eq.hyp_mag_type_all(i).and.
     *        hyp_mag_all(i).ne.0.0.and.
     *        (hyp_mag_agency_all(i).eq.agency.or.agency.eq.' ')) then
                 nm=nm+1
                 mag(nm)=hyp_mag_all(i)

                 write(6,*) hyp_mag_all(i),
     *           hyp_mag_type_all(i),hyp_mag_agency_all(i)
c
c   get time
c
                 read(data(1),'(1x,i4,1x,2i2,1x,i2)') 
     *           year,month,day,hour
                 xyear=year+(month-1)/12.0+(day-1)/
     *           365.0+(hour-1)/(24.0*365.0)
                 if(xyear.gt.max_year) max_year=xyear
...
c
 10   continue
      ...
      write(6,*)'  Duration of catalog in years', xyear
...
c   corresponding to magnitude -2-8 in 0.1 magstep intervals  ! minus mag
c
      kl=0
c
c  blank arrays
...
c
      do 15 i=1,nm
         k=(mag(i)+0.01)/magstep+1+2.0/magstep   ! minus mag
         nmag(k)=nmag(k)+1
 15   continue
c
c  cumulative sum
...
cold            xmag=(i-1)*magstep
            xmag=(i-2)*magstep-2.0    ! minus mag
...
c
      ...
      do 17 i=1,100
         if(nmag(i).ne.0) then
            write(6,202) float(i-1)*magstep-2.0,nmag(i),cmag(i)  ! neg mag
            write(write01,202) float(i-1)*magstep-2.0,nmag(i),cmag(i) ! neg mag
...
c
      ...
      do 1 i=1,4
        ...
        call xmovabs(210.0,iy) ! neg mag
        write(text(1:4),'(i4)') k
        call xchars(text,4,125.0,iy-5)
 ...
 22   continue	  
      ...
      do 2 i=1,99
         if(mag.eq.10) then
            ...
            write(text(1:2),'(i2)') jmag-2   ! neg mag
            call xset_color(color_frame)
            call xchars(text(1:2),2,ix-12.0,170.0) ! neg mag
            call xmovabs(ix,200.0)
...
c
      ...
      x1=k*(m1+2)/magstep+200   ! neg mag
      x2=k*(m2+2)/magstep+200   !neg mag
...
c
 ...
 250  format(2f8.2)
      call xchars(text,50,250.0,700.0)

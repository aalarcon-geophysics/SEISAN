...
c jh 2019 04 09 : new format, few changes
c jh 2021 09 04 : increase some dimensions
c
...
c-- event arrays
      character*80      data(10000)    ! jh may 2007, increased
...
c station names
      character*5 pathstations(30000),stations(30000)
...
c
      ...
      do c=nhead+1,nrecord
        if(new_nordic_format) then
           if (data(c)(17:17).eq.'P'.or.
     &         data(c)(17:17).eq.'S') then
               phases=phases+1
           endif

        else

           if (data(c)(11:11).eq.'P'.or.
     &         data(c)(11:11).eq.'S') then
               phases=phases+1
           endif
        endif
...
c      write(*,*) ' debug ',nphase,phases,nhead,nrecord
...
c        write(*,'(F8.3,1X,F7.3,1X,F10.5)') lon,lat,magout
     ...
     &      year,month,day,hour,min,sec
        ...
        enddo
        write(13,'(a22,1x,f8.3,1x,f8.3,1x,f10.5,1x,g9.2,1x,i4,1x,f5.2)') 
     &    time,lon,lat,magout,mom,phases,magnitude
c     &    time,lon,lat,magout,mom,phases

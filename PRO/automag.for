...
c  2019 02 26 jh: new format, minor changes with components. location and 
c                 network not implemented since no corresponding response files
c  2019 06 04 jh: network and location used is transferred
c  2019 06 18 jh: operator and agency
c  2020 11 09 jh: rea_com for new spectral format, location and network for spectra
c  2020 01 29 jh: get agency from seisan.inc instead of from station file
c                 increase dim of mw
c  2021 02 22 jh: time before s from 2 to 4 sec, rem taper, already done in remove_response
c  2021 10 13 jh:  write out avarages sd of mw etc
c  2022 01 02 jh: mark amplitudes as automatic, distance where S is considered Lg
c                 500km instead of 200km, variable extra_time_before_s which is a
c                 function of distance to make sure S is in window, measure s/n
c                 of wa readings and reject if bad
c  2022 02 03 jh  add agency and operator from prompt, wav base from prompt
c  2022 02 20 lo  changed s/n check for WA and implemented use of 'h' to
c                 use both horizontals; time before S is now .1 times duration
c  2022 02 21 lo  removed gain correction for additional filter
c  2022 02 23 lo  remove trend for wa and noise
c  2022 02 24 lo  convert whole trace to WA then extract and read amplitude
c  2022 08 04 jh  write station for wa trace was commented out so plotspec crashed ???
c                 automag.list not correct if low s/n so crash in plotspec
c
c minor issues: average calculation, noise level 0 in some cases
...
c
c  WA window: 50 s
c  WA window also filtered according to MULPLT.DEF, no correction for this filter
c  S-phase only
c  Frequency range for spectral analysis: 0.05 to (samp_rate)/2.5
c  WA period must be less than 2 s
...
c  for grid search, number of steps, start and step
      ...
      real av_f0_st,sd_f0                 ! and sd---------- with ok stress drop
      real av_st,sd_st                    ! average and sd ------ stress drop
      logical grid                        ! if true, grid search
      character*140 result(50000)         ! store results
      ...
      real extra_time_before_s         ! extra----- depending on distance
...
c
      ...
      real av_res_st,sd_res            ! average residual and sd of runs with ok stress drop
      real av_mw                       ! average mw of all runs
      real med_mw                      ! median mw
      real av_mw_st,sd_mw              ! average mw and sd of all runs with ok stress drop
      real grid_st(10000)              ! stress drop values in grid search
      real grid_res(10000)             ! residual ------------------------
      real grid_mw(10000)              ! mw --------------------
      real grid_f0(10000)              ! f0
      integer nmw                      ! number of calculated mw
      real mw(10000)                   ! individual mw
      ...
      real wa_noise,wa_signal          ! wa amplitudes
      real noise,signal                ! wa av spectral amplitudes
      real wa_period
      ...
      character*3 agency               ! agency, for station file
      character*3 operator             ! operator
c
      integer i,k,kk,l,m,n,j,kq0,kqalpha,kkappa  ! countera etc
...
c     
      duration_spec=50.0              ! duration spec window
      duration_wa=50.0
      ...
      operator=' '
c
c   get operator, set in eev, if oprated outside eev, operator on promp
c   can be used
c
      call get_env_op(operator)
c
c   get agenc, can also come from prompt
c
c      call get_agency_hyp(hyp_model(1),agency)  ! from station file
       agency=seisan_agency                      ! from SEISAN.DEF
c
...
c 
        ...
        do i=2,nars
          if(arg(i)(1:1).eq.'s'.and.arg(i-1)(1:1).ne.'-') then
            spec=.true.
            read(arg(i+1),*) duration_spec
            if(duration_spec.eq.0.0) spec=.false.
          endif
          if(arg(i)(1:1).eq.'p'.and.arg(i-1)(1:1).ne.'-') then
            spectrum_type='P'
            spec=.true.
            wa=.true.
            if(duration_wa.eq.0.0) wa=.false.
          endif
          if(arg(i)(1:1).eq.'w'.and.arg(i-1)(1:1).ne.'-') then
            wa=.true.
            read(arg(i+1),*) duration_wa
            if(duration_wa.eq.0.0) wa=.false.
          endif
          if(arg(i)(1:1).eq.'n'.and.arg(i-1)(1:1).ne.'-') then
            comp_select='N'
          endif
          if(arg(i)(1:1).eq.'e'.and.arg(i-1)(1:1).ne.'-') then
            comp_select='E'
          endif
          if(arg(i)(1:1).eq.'h'.and.arg(i-1)(1:1).ne.'-') then
            comp_select='h'   ! N,E,1,2
          endif
          ...
          if(arg(i)(1:8).eq.'-wavbase') then
              n_wav_bases=n_wav_bases+1
              wav_base(n_wav_bases)=arg(i+1)
          endif
        enddo
        endif
      endif
c      time_before_s=4.0               ! start window at least 4 s before S
      time_before_s=0.1*duration_wa   ! lo, start window 10% of wa window before S
c      duration_wa=duration_wa+time_before_s

c     write(6,*)spec,spectrum_type,wa,comp_select,operator,agency
c
...
c
 ...
 20   continue   ! here from end for next event
      nmw=0 ! moved here lo, 21/2/2022; to get magnitudes per event
      nnml=0
      nml=0
c
...
c
c      call get_agency_hyp(hyp_model(1),agency)  ! from station file
c      agency=seisan_agency                      ! from SEISAN.DEF
c
...
c
         ...
         do i=1,m
           do j=1,wav_nchan
              if (comp_select.eq.'h') then   ! horizontals
                if (wav_comp(j)(4:4).eq.'N'.or.
     &              wav_comp(j)(4:4).eq.'E'.or.
     &              wav_comp(j)(4:4).eq.'1'.or.
     &              wav_comp(j)(4:4).eq.'2') then
                  cmp=wav_comp(j)(4:4)
                else 
                  cmp='0'
                endif
     ...
     *         wav_comp(j)(1:1).eq.'H')) then   !afad fix
                 ...
                 if (comp_select.eq.'h') then   ! include both horizontals
                   k=k+1
                   statsel(k)=stat(i)
                   location(k)=wav_location(j)
                   network(k)=wav_network(j)
                   if (wav_comp(j)(4:4).eq.'N') then 
                     compsel(k)=wav_comp(j)(1:3)//'E'
                   elseif (wav_comp(j)(4:4).eq.'E') then
                     compsel(k)=wav_comp(j)(1:3)//'N'
                   elseif (wav_comp(j)(4:4).eq.'1') then
                     compsel(k)= wav_comp(j)(1:3)//'2'
                   elseif (wav_comp(j)(4:4).eq.'2') then
                     compsel(k)=wav_comp(j)(1:3)//'1'
                   endif
                 endif
                 goto 42
              endif
...
c
      ...
      enddo
      write(6,'(a,a)')' No S-phase or weight 4 for station ',stat(m) 
c
c  S from P since no S
c
      extra_time_before_s=0.0
      do i=1,rea_nphase
        if(rea_phase(i)(1:1).eq.'P'.and.rea_stat(i).eq.stat(m)) then
           ...
           if(rea_weight_in(i).eq.'4') then
              write(6,*)'Zero weight on P, no S, skip station'
              goto 500
           endif
c
c  direct phase or deep event
c
           if(hyp_depth(1).ge.50.0.or.rea_dist(i).le.500.0) then
...
c  lg phase
c
           if(hyp_depth(1).lt.50.0.and.rea_dist(i).gt.500.0) then
...
c
c   since theoretical S is not very accurate, allow extra slack in
c   start time depending on distance
c
           extra_time_before_s=rea_dist(i)/100.0         
...
c
c estimate absolute ptime to be used for noise
c
      ptime=(s_time-hyp_abs_time(1))/1.78+hyp_abs_time(1)
...
c
c   limit trace to 20s after window end
c
        kk=wav_out_first_sample(k)+wav_out_nsamp(k)+20*wav_out_rate(k)
        if(kk.ge.wav_nsamp(k)) kk=wav_nsamp(k)
        wav_nsamp(k)=kk
c
c convert whole trace to WA, stored in signal2
c
c       write(*,*) ' debug lo ',wav_nsamp(k),wav_out_nsamp(k)
       do i=1,wav_nsamp(k)
         signal2(i)=signal1(i)
       enddo
c
       drate=wav_out_rate(k)   ! double precision rate
c       call rtr(signal2,wav_nsamp(k),drate)
c   
...
c
c        write(*,*) ' debug filter nsamp ',wav_nsamp(k)
          call recfil(signal2,wav_nsamp(k),signal2,'BU      ',
     *    0.0,0.0,recfil_pole,
     *    filt_name,flow,fhigh,1.0/wav_out_rate(k),1)  
      endif               
c
c   correct for instument, no filtering
c   lo: could move this ahead of filter?
c
      call remove_resp(signal2,y_com,wav_nsamp(k),wav_out_rate(k),1,
     *0.0,0.0,pl,ph,
...
c
      ...
      av=av/wav_nsamp(k)
      do i=1,wav_nsamp(k)
        signal2(i)=signal2(i)-av
      enddo
c
c   save trace in file
c
c             text=stat(m)(1:seiclen(stat(m)))//'.'//comp(m)//'.fu'
c             do i=1,10
c               if(text(i:i).eq.' ') text(i:i)='_'
c             enddo
c            open(33,file=text(1:13),status='unknown')
c             write(33,*)wav_nsamp(k),wav_out_rate(k),ifirst,ilast
c             enddo
c             close (33)
...
c  following was commented out?? jh aug 22
...
c
c  if period more then 2 sec, consider unreliable
c
          if(per.lt.2.0) then
c
c  correct for filter if used, removed lo
c
            
c             if(pole_low.gt.0.or.pole_high.gt.0) then
c               gain=1.0
c               if(flow.gt.0.0.and.pole_low.gt.0) then
c                  call bworth(1.0/per,
c     *            flow,-pole_low,hs) 
c                  gain=gain*cabs(hs)
c               endif
c               if(fhigh.gt.0.and.pole_high.gt.0) then
c                  call bworth(1.0/per,
c     *            fhigh,pole_high,hs) 
c                  gain=gain*cabs(hs)
c               endif
c               amp=amp/gain   ! lo moved to after S/N comparison
c               write(6,*) 'wa: gain,period',gain, per
c             endif       
             wa_signal=amp
             wa_period=per
c
...
c
             abstime=s_time+ifirst/wav_out_rate(k)
     *       -time_before_s-extra_time_before_s
             call sectim(abstime,
...
c 
c get WA amplitude from start of trace to see if S reading higher
c see commments for processing above

c
c start for noise time
c
c             write(*,*) ' ptime abstime ',ptime,
c     &          wav_abs_time(wav_first)
             wav_out_nchan=1
             wav_out_chan=k
             wav_out_start(k)=ptime-wav_abs_time(wav_first)
     &         -12.    ! start 12 sec before P
             wav_out_duration(k)=10.
             call wav_get_interval
             write(*,*) ' first noise ',wav_out_first_sample(k),
     &          wav_out_nsamp(k)
             l=0
c extract signal
             do i=wav_out_first_sample(k),
...
c remove trend
c
c             call rtr(wav_y1,l,drate)

c             do i=j,j+int(10.*wav_out_rate(k))    ! 10 seconds
cwav_out_nsamp(k)   ! taken from start of trace, if pre p time short this may be included, but should not matter
c               wav_y1(l)=signal1(i)
c               l=l+1
c             enddo
             wav_out_nsamp(k)=l
             nsamp=l
c             write(*,*) ' debug lo nsamp = ',nsamp
             av=0.0
             do i=1,wav_out_nsamp(k)
               av=av+wav_y1(i)
             enddo
...
c                call recfil(wav_y1,nsamp,wav_y1,'BU      ',
c     *          0.0,0.0,recfil_pole,
c     *          filt_name,flow,fhigh,1.0/wav_out_rate(k),1)
c             endif
c
c             call remove_resp(wav_y1,y_com,nsamp,wav_out_rate(k),1,
c     *       0.0,0.0,pl,ph,
c     *       zero,pole,nzero,npole,norm)

c
c   save trace in file
c
             text=stat(m)(1:seiclen(stat(m)))//'.'//comp(m)//'.no'
             do i=1,10
               if(text(i:i).eq.' ') text(i:i)='_'
             enddo
             open(33,file=text(1:13),status='unknown')
c  next line was commented out ?? jh aug 22
             write(33,*)wav_out_nsamp(k),wav_out_rate(k),ifirst,ilast
             do i=1,wav_out_nsamp(k)
               write(33,*) wav_y1(i)
             enddo
...
c             write(*,*) ' now autoamp nsamp ',wav_out_nsamp(k),wav_y1(l)
             call auto_amp
     *(wav_y1,wav_out_nsamp(k),wav_out_rate(k),fcmin,fcmax,maxcros,
     *yfirst,ylast,ifirst,ilast)
c        write(*,*) ' debug amp ',
c     *abs(yfirst-ylast)/2.0,ifirst,ilast,yfirst,ylast
             amp=0.
             per=0.
             if (ifirst.gt.0) then
                amp=abs(yfirst-ylast)/2.0   ! first and last sample number for 2 extreemes
...
c  if period more then 2 sec, consider unreliable
c
c                write(*,*) ' debug lo period ',per
c                if(per.lt.10.0) then
cc
cc  correct for filter if used
cc
c             write(*,*) 'debug lo ',pole_low,pole_high,flow,fhigh
c                   if(pole_low.gt.0.or.pole_high.gt.0) then
c                     gain=1.0
c                     if(flow.gt.0.0.and.pole_low.gt.0) then
c                        call bworth(1.0/per,
...
c                        gain=gain*cabs(hs)
c                     endif
cccc lo                     amp=amp/gain
c                     write(*,*) ' debug gain ',gain
c                   endif
c                 else
c                   write(*,*) ' period noise ',per
c                   amp=0.
c                 endif
             endif
             wa_noise=amp
      write(*,*) ' wa signal, noise, ratio = ',wa_signal,wa_noise,
     &          wa_signal/wa_noise

c ------------------------------------------------------------------------
c lo skip this part, 19/2/2022
      goto 888
c
c   check signal to noise ratio. first spectral amplitude of noise
c   at start of trace in frequecy band around wa frequency
c   use  a  fixed window of 10s, start a few s after start
c   to avoid low amp in beginning due to  fit
c
             nsamp=10.0*wav_rate(k)
             do i=1,nsamp
                wav_y1(i)=signal1(i+int(10.0*wav_rate(k)))
             enddo
c
c   use 1-5 hz for s/n insted of period of pic since oftne in noise
c
             call spec_value_one(wav_y1,nsamp,wav_rate(k),
c     *       (1.0/per)*0.7,(1.0/per)*1.3,wa_noise)  
     *       2.0,6.0,noise)  
c             write(6,*)'noise=',noise
c
c  now amplitude around wa pick
c
             l=1
             do i=wav_out_first_sample(k)+ifirst-nsamp/2,
     *       wav_out_first_sample(k)+ifirst+nsamp/2
               wav_y1(l)=signal1(i)
               l=l+1
             enddo 

             call spec_value_one(wav_y1,nsamp,wav_rate(k),
c    *       (1.0/per)*0.7,(1.0/per)*1.3,signal)  
     *       2.0,6.0,signal)  
c             write(6,*)'signal=',signal
             write(*,*) ' spec signal, noise = ',signal,noise,
     &         signal/noise
 888  continue
c ------------------------------------------------------------------------
c
c   calculate s/n ratio
c
            if(wa_signal.ne.0.0.and.wa_noise.ne.0.) then
                write(6,'(a,f5.1)') ' wa-amp s/n= ',wa_signal/wa_noise
                if((wa_signal/wa_noise).lt.1.5) then
c                if((signal/noise).lt.1.5) then
                   write(6,*) 's/n ratio smaller than 1.5, skip wa'
                   write(55,*)' '   ! signal no spectrum   added jh aug 22
                   goto 400
                endif
            endif
...
c
             ...
             rea_com(rea_nphase)=comp(m)(1:2)//comp(m)(4:4)   ! for new format
             ...
             rea_amp(rea_nphase)=wa_signal
             rea_per(rea_nphase)=wa_period
             rea_location(rea_nphase)=location(m)
             rea_network(rea_nphase)=network(m)
             rea_agency(rea_nphase)=agency 
             rea_operator(rea_nphase)=operator  
             rea_auto(rea_nphase)='autopick'
c
c   check if phase was there from before, then delete
c
             do l=1,rea_nphase_org                         ! fix new comp, co to com

               if(new_nordic_format) then
                  if(rea_stat(l).eq.stat(m).and.rea_com(l).
     *            eq.rea_com(rea_nphase)
...
c
c  if a P-spectrum, also check that the window does not go into the S
...
c
     ...
     *        year,doy,month,day,hour,min,sec)
              ...
              rea_stat(k)=stat(m)
              rea_comp(k)=comp(m)
              rea_location(k)=location(m)
              rea_network(k)=network(m)

              rea_com(k)=comp(m)(1:2)//comp(m)(4:4)
              rea_phase(k)= '     '
...
c
c source dimension and spectral phase
c
              if(phase.eq.'P') then
...
c-----------
     ...
     *           stress_high) then
                    nfit_st=nfit_st+1
c                    av_res_st=av_res_st+res_m_a(2)
                    grid_st(nfit_st)=rea_sdrop(k)
                    grid_res(nfit_st)=res_m_a(2)
                    grid_f0(nfit_st)=f0
                    grid_mw(nfit_st)=rea_mw(k)
c                    av_f0_st=av_f0_st+f0
c                    av_st=av_st+rea_sdrop(k)
c                    av_mw_st=av_mw_st+rea_mw(k)
                 endif                  
             else
...
c
     ...
     *          eq.rea_comp(k).and.rea_network(l).eq.rea_network(k).
     *          and.rea_location(l).eq.rea_location(k)
     *          .and.rea_phase(l)(1:4).eq.'SPEC'.
     *           and.rea_spec_phase(l).eq.phase) then
                   rea_phase(l)(1:6)='DELETE'
                 endif
             enddo
            else
...
c      write(6,'(a,i7)')     ' Total number of readings for ml',nml
c      write(6,'(a,i7)')     ' Total number of calculated ml  ',nnml
c      if(nnml.gt.0) then
c         call mdian(ml,nml,med_ml)
c         write(6,'(a,2f7.2)')' Average and median ml          ',
c     *   av_ml/nnml,med_ml
c      endif
...
c         av_res_st=av_res_st/nfit_st
c         av_mw_st=av_mw_st/nfit_st
c         av_st=av_st/nfit_st
c         av_f0_st=av_f0_st/nfit_st
	  call sdv(nfit_st,grid_res,av_res_st,sd_res)
	  call sdv(nfit_st,grid_st,av_st,sd_st)
	  call sdv(nfit_st,grid_mw,av_mw_st,sd_mw)
	  call sdv(nfit_st,grid_f0,av_f0_st,sd_f0)
         

         write(result(nresult)
     *   ,'(a,f6.1, a,f6.3,a,f6.3,a,i6,a,f6.3,a,f6.2,a,f6.2,a,f6.1,
     *    a,f6.3,a,f6.1,a,f6.1,a,f6.1)')
     *   'q0=',q0,' qa=',qalpha,' ka=',kappa,' nf=',
     *    nfit_st,' re=',av_res_st,' mw=',av_mw_st,' f0=',av_f0_st,
     *   ' st=',av_st,
     *   ' sd-res=',sd_res,' sd-mw=',sd_mw,
     *   ' sd-f0=',sd_f0,' sd-st=',sd_st
...
c
     ...
     * ,nminsp,minsp
      ...
      write(6,'(a)')     ' Output of wa traces in STAT.COMP.wa'
      write(6,'(a)')     ' Output of summary file in automag.lis'
      if(grid) write(6,'(a)') 
...
c
 ...
 99      continue
      ...
      subroutine spec_value_one(y,nsamp,rate,f_low,
     &  f_high,level)
c
c subroutine to take out spectral level at given frequencies
c no response correction
c simplified from spec_value by lo
c
c  jh jan 2022
c 
c input:  y      - input time signal
c         nsamp   - number of samples
c         rate    - sample rate
c         nsmooth - number of times to smoothe
c         f_low,f_high - frequency limits for spectrum
c         farray  - frequency value array
c         nfreq   - number fo frequencies 
...
c
      ...
      real rate               ! sampel rate
      real y(*)               ! the signal
      complex y_com(100000)   ! complex spectrum
      ...
      real level              ! spectral level at f_low to f_high
      real ttime
...
c
      ...
      do b = 2,((ipow/2) + 1)
       ...
       if(ff .ge.f_low.and. ff .le. f_high) then
         c = c + 1
         y(c) = ((1/rate)**2)*(y_com(b)*conjg(y_com(b)))
         y(c) = sqrt(y(c)) 
         level=level+y(c)        
       endif
      enddo   

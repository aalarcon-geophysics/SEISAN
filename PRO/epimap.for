...
c 2018 12 04 jh  : no error message if depth field is blank
c 2020 02 11 jh  : skip events with no epicenter, were plotted at 0,0
c 2020 03 29 jh  : filetype was determiend using first 10 lines, not
c                  enough for large events from isc, now 50 lines
c                  are checked
c 2020 12 07 jh  : multiple, H and E lines, remove warnings due to rank mismatch. 
c                  mostly, replace map_plot with map_proj_one and a few others
c 2021 01 19 jh  : change format of locality file, option no plot to not plot info on left
c                  rewind plot file (at 2000 continue), sometimes plot appeared 2 times 
c                  eps file, if no plot, no options only quit with any char
c 2022 23 11 jh  : move numbers on profile y-axis right
C
...
c---tec xy for epicenters
      ...
      integer ne,nh    ! number of E and H lines
c
c   polygon arrays...
c   -----------------
c
...
c-----variables for plotting positions------------------                        
      real ix,iy 
      logical no_plot      ! do not plot some features if true                                                               
...
c   get arguments
c
      no_plot=.false. ! plot all, if true do not plot info on left
      call get_arguments(nars,args)
      do i=1,nars
       if(args(i).eq.'noplot')    no_plot=.true.
      enddo
...
c
c   seems that on some systems, if no rewind here, figure comes twice
c
      rewind(write2)     ! jh feb 2021
c                                                                               
...
c
C   =======================================================================================                                                                             
C   Input an event read loop, back here to read next event
C   =======================================================================================
C  
C  
...
c
               ...
               DO I = 2, NHEAD                            ! Loop records.
                  ...
                  IF(DATA(i)(80:80).EQ.'E'.and.           ! If ellipses
     *            data(i)(10:10).eq.data(1)(6:6).and.     ! same location method
     *            (ne.eq.1.or.data(i)(12:14).eq.data(1)(46:48))) then ! same agency or only one    
...
c
c   check if an epicenter is present, if not skip event
c
            if(data(1)(25:38).eq.' ') goto 9
...
c
     ...
     *          data(i)(6:6).eq.data(1)(6:6).and.   ! same method
     *          (nh.eq.1.or.data(i)(61:63).eq.data(1)(46:48))) then  ! same agency or only one H line
...
c
c
c   make sure no error message if depth field is blank
c
         if(data(1)(39:43).eq.' '.and.data(1)(25:37).ne.' ') 
     *   data(1)(39:43)='  0.0' !  jh dec 2018
...
c8585  read( read1, '(a)', iostat=code ) chr_text     ! Get entry.
      read( read1, '(a)', iostat=code ) text         ! Get entry.
      call seicode( fort$,code,read1,b_flag)         ! Process outcome.
c
c   End of file...
c   --------------
...
c
         ...
         read(text(1:24),*) slat(1),slon(1)
         chr_text(1:56)=text(25:80)
        
c         call sei left( chr_text )                   ! Left justify record.
c         jx = index( chr_text, '  ' )                ! Search for double space.
c         call sei left( chr_text(jx+1:) )            ! Left justify lat string.
c         kx = jx + index(chr_text(jx+1:),' ')        ! & next delimiter.
c         call sei left( chr_text(kx+1:) )            ! Left justify long string.
c         slat(1) = geo deg(chr_text(jx+1:kx), code ) ! Get latitude.
c         slon(1) = geo deg(chr_text(kx+1:),   code ) ! & longitude.
c         chr_text(jx:) = ' '                         ! & isolate text.
c
...
c            write(6,*) jx,strike(jx),dip(jx),rake(jx),ix,iy 
            call plot_foc(strike(jx),dip(jx),rake(jx),ix,iy,25.0,0)
         endif    
c
c    Ellipse values...
c    -----------------
...
c               call hyperr( slon, slat,                   ! Ellipses.
               call hyperr( slon(1), slat(1),                   ! Ellipses.
...
c
        ...
        goto 2738
c
c   stop here if no plot
c
 2737  continue        
        
       text = 'Any char to stop'
       call tchars(text,seiclen(text),border_x$,ix)
       call xscursr(ichar,ix,iy)
       goto 999
...
c     call ancho(7)
      ...
      enddo
cx
c      xlonmz(1)=xlonmz(1)-1.0
cc      xlonmz(2)=xlonmz(2)+1.0
c      ylatmz(1)=ylatmz(1)-1.0
c      ylatmz(2)=ylatmz(2)+1.0

c      xlonmz(1)=xlonm(1)-1.0
c      xlonmz(2)=xlonm(2)+1.0
c      ylatmz(1)=ylatm(1)-1.0
c      ylatmz(2)=ylatm(2)+1.0
...
c
      do i=1,n
c      CALL MAP_PROJ(xzoom(i),yzoom(i),x,y,1)
       CALL MAP_PROJ(xzoom,yzoom,x,y,1)
      bx(i) = x(1) + REFX0$
      by(i) = y(1) + REFY0$
        if(i.lt.n)then
...
c
         ...
         call map_proj_one(x(ix), y(ix), xl, yl) ! Project this point.
...
c      call map_proj( x(ix), y(ix), xl, yl, 1 )  ! Project this point.
      call map_proj_one( x(ix), y(ix), xl, yl) 
      b_out = xl .lt. 0.0     .or.              ! Outside drawing area?.
     &        yl .lt. 0.0     .or.              !
     &        xl .gt. xsize$  .or.              !
     &        yl .gt. ysize$                    !
...
c       xd = X0 -25.0                               ! Left edge of tick.
       xd = X0 + 10.0                               ! Left edge of tick.
...
C                                                                               
      ...
      SUBROUTINE MAP_PROJ_one(LON,LAT,X,Y)                                        
C                                                                               
C  PERFORMS THE TRANSFORMATION FROM LATITUDE-LONGITUDE TO X-Y                   
C   X-Y ARE IN VERTUAL UNITS WITH ORIGO IN THE LOWER LEFT HAND CORNER OF MAP.   
C   INPUT:  LAT,LON 1 VALUE TO BE TRANSFORMED                               
C   OUTPUT: X,Y: one TRANSFORMED VALUES                                           
C                                                                               
C   FOR REST OF PARAMETERS, SEE EXPLANATION OF COMMON BLOCK IN                  
C   ROUTINE MAP_GRID                                                            
C                                                                               
...
C
      ...
      call map_proc( encode$+proj$,            ! Encode projection.
     &               lat,   lon,         ! From lat-longs.
     &               vy,       ux,             ! To map co-ordinates.
     &               REFLATN$, REFLONG$,       ! Reference lat-longs.
     &               code )                    ! Returned condition.
      call sei code( stop$, code, 0, b_flag )  ! Process outcome.
C
         X=(ux*rad_to_deg$-XMIN$)*SCALE$    ! Convert to screen co-ordinates.
         Y=(vy*rad_to_deg$-YMIN$)*SCALE$    !
 51   CONTINUE                                                                  
      RETURN                                                                    
      END                         
...
c
      CALL MAP_PROJ_one(xlon,xlat,xx(1),yy(1))                                            
      xx(1)=xx(1)+REFX0$                                                            
      yy(1)=yy(1)+REFY0$
...
c         call map_proj( xx(i), yy(i),      ! Get screen co-ordinates.
c     &                  x(1),   y(1),      !
c     &                  1 )                ! For 1 point.

         call map_proj_one( xx(i), yy(i),      ! Get screen co-ordinates.
     &                  x(1),   y(1))

...
class Bulletin(object):
    ...
    def _printOriginAutoloc3(self, org, extra=False):
        ...
        for dist, arr in dist_arr:
            ...
            comp = wfid.channelCode()
            try:    toang =  arr.takeOffAngle()
            except: toang = 0.0                 
            loca = wfid.locationCode()            
            ...
            if self.polarities:
                ...
                line = "    %-5s %-2s %-3s %5.1f %s %5.1f  %-7s %s %s %1s%1s %3.1f  %s %-5s %-2s\n" \
                    % (sta, net, comp, dist, azi, toang, pha, tstr, res, status, flag, wt, pol, sta,loca)                
        ...
        txt += "    sta  net cmp  dist azi toang  phase   time         res     wt  "        
        ...
        for typ in mags:
            for mag in mags[typ]:
                ...
                if amp:
                    ...
                    if typ in ["mb", "mB", "Ms", "ML", "MLv", "Mwp", "Ms(BB)"]:
                ...
                line = "    %-5s %-2s %-3s %5.1f %s  %-6s %5.2f %5.2f   %s %8s %4s \n" % \
                        (sta, net, comp, dist, azi, typ, val, res, tt, a, p)
                lines.append( (dist, line) )

        lines.sort()

        txt += "%d Station magnitudes:\n" % nmag
        txt += "    sta  net cmp  dist azi  type   value   res                      time      amp per\n"        
        for dist,line in lines:
            txt += line

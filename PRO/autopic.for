...
ccjan 4 2012   jh: add argument to auto_tr
c apr 8 2019   jh: new format and some bugs
c feb 7 2022   jh: bug writing baz, add operator and agency, wavbase
c jul15 2022   jh: bug with rea_weigh_in, not kept of previous data. if parameter
c                  CRAT = 0, no coda calculation, add parameterr MaxWin to limit the 
c                  analysis windo to MaxWin from start. This  to avoid a second 
c                  event much later to be detedted as s.
...
c-- station and comp used in seisinc      
      ...
      character*3 jcom(max_trace)
c-- component of station                  
      character*4 incomp		
c-- component of station                              
      character*2 comp(3)
      character*3 com(3)
c-- netwotk and location
...
c-- used to get parameters                                 
      real array(11)				
      real max_window         ! max length of window (s)
...
c-- minimum allowed apparent P-velocity
      ...
      character*3 operator, agency
c
c   following 2 lines for reading s-file
c
...
c         
      ...
      do i=1,nars
         if(arg(i)(1:3).eq.'-ag') agency=arg(i+1)(1:3)
...
c                                                                               
        station=wav_stat(k)
        incomp=wav_comp(k)
        network=wav_network(k)
        location=wav_location(k)
        year=wav_year(k)
        month=wav_month(k)
        hour=wav_hour(k)
        min=wav_min(k)
...
c-- Just format change  
        com(1)=incomp(1:2)//incomp(4:4)	 ! 3 char                  
        call component(incomp,comp(1))	 ! 2 char
...
c
        ...
        if(max_window.eq.0.0) then
            nsamp=wav_nsamp(k)
        else
            nsamp=max_window*srate
            if(nsamp.gt.wav_nsamp(k)) nsamp=wav_nsamp(k)
        endif 
        do j=1,nsamp
          data(j)=signal1(j)
        enddo
...
c	
     ...
     *  network,location,operator,agency)
c
...
C                                                                               
  20  CONTINUE                                                                  
     ...
     +           NCHAN,SRATE,HYPOCEN,IFILTERSWITCH,IOUT,CODAEPO)                
      ...
      REAL*8 EPOTIM                                                             
cbug      REAL*4 DATA(MSMP,NCHAN), SRATE, SLAT, SLON   
      REAL*4 DATA(MSMP,3), SRATE, SLAT, SLON                              
...
c
         	...
         	read(string,'(11f7.0)')array(1),array(2),array(3),                     
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine read_write_phas(un,sfile,station,slat,slon,comp,com,
...
c                                                                               
        ...
        if(new_nordic_format) then   
           if(phase(i).ne.' ') then
              ...
              rea_com(rea_nphase)=com
              rea_network(rea_nphase)=network
              rea_location(rea_nphase)=location
              rea_onset(rea_nphase)='E'
              rea_phase(rea_nphase)(1:2)=phase(i)(2:3)
              rea_weight_in(rea_nphase)=weight
              rea_hour(rea_nphase)=hour(i)
              rea_min(rea_nphase)=min(i)
              rea_sec(rea_nphase)=sec(i)
              rea_auto(rea_nphase)='A'
...
c   time of END
c
                 call sectim(dur,year,j,month,day,rea_hour(rea_nphase),
     *           rea_min(rea_nphase),rea_sec(rea_nphase))
                 rea_coda(rea_nphase)=idur
                 rea_auto(rea_nphase)='A'
                 idur=0       ! do not write for s-phase
              endif
c
c   baz
c
              if(bazi.ge.0.0.and.apvel.gt.0.0) then
                 rea_nphase=rea_nphase+1
                 call rea_phase_clear(rea_nphase)
                 rea_nrecord=rea_nrecord+1
                 rea_stat(rea_nphase)=station
                 ...
                 rea_location(rea_nphase)=location
                 rea_onset(rea_nphase)=' '
                 rea_phase(rea_nphase)(1:5)='BAZ-P'
                 rea_auto(rea_nphase)='A'
                 rea_weight_in(rea_nphase)=' '
                 rea_hour(rea_nphase)=hour(i)
                 rea_min(rea_nphase)=min(i)
                 rea_sec(rea_nphase)=sec(i)
                 rea_baz_obs(rea_nphase)=bazi
                 rea_vel(rea_nphase)=apvel
                 rea_operator(rea_nphase)=operator
                 rea_agency(rea_nphase)=agency
                 bazi=-1.0   
              endif  
...
c     +     duration,amplitude,period,azi(i),vel(i),snr(i)                              
     ...
     +     duration,amplitude,period,azi(i),vel(i)
      ...
      if(new_nordic_format) then 
          call rea_event_out(read1,.true.,cards,code) 
      endif

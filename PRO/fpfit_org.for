...
c sep 24 2020 jh: change names of cvrtop, delaz and back, conflicting names
c mar  2 2020 jh: fix problem with wrong output in fpfit.fps and fpfit.pol
...
c
      ...
      subroutine delaz_fpfit(lat, lon, delta, az0, az1, x, y)
c
...
c
      ...
      if ((ct1 - ct0) .eq. 0. .and. (lon - olonsv) .eq. 0.) then
        ...
        call 
     *  cvrtop_fpfit (st0*ct1 - st1*ct0*cdlon, st1*sdlon, sdelt, az0)
        delta = atan2(sdelt, cdelt)
        call 
     *  cvrtop_fpfit (st1*ct0 - st0*ct1*cdlon, -sdlon*st0, sdelt, az1)
      ...
      return
       ...
       subroutine back_fpfit (delta, az0, lat, lon)
c
...
c
      ...
      call cvrtop_fpfit 
     *(st0*cdelt - ct0*sdelt*cz0, sdelt*sin(az0), st1, dlon)
      lat = atan2(ct1, st1)
      lon = olonsv + dlon
...
c
      ...
      subroutine cvrtop_fpfit(x, y, r, theta)
c
...
c
c 20    read (iunit, 30, end = 1000) test   ! gfortran
 20    read (iunit, 30, end = 1000,err=1000) test    ! new
...
c
      ...
      call delaz_fpfit (alat1, alon1, delta, az0, bazm, xpos, ypos)
      call back_fpfit (shift, az0, plunge, azimth)

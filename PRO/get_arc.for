...
c  23 05 2019 jh: new format, only recompile
c  17 03 2021 jh: fixed bug where get_arc breaks when one event doesn't have
c                 any waveforms available in archive
c  08 10 2021 jh: remove  ARC references
c  
c--------------------------------------------------------------------------c
c
c
...
c
      ...
      do i=1,nchan
         k=chan(i)
c         write(3,'(a5,a3,a2,a2,2f15.0)') arc_stat(k),arc_comp(k),
         write(3,'(a5,a3,a2,a2)') arc_stat(k),arc_comp(k),
     *   arc_net(k),arc_loc(k)
c ,arc_start(k),arc_stop(k)
      enddo
      close(3)
c
c-----------------------------------------------------------
...
c
      do i=1,rea_nwav
        if(k.le.78) then
          ...
          write(*,*) ' string too long ',k,long_text(1:k)
        endif
      enddo
c
      rea_nwav=rea_nwav+1
